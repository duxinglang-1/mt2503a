...
/* under construction !*/
        ...
        #if defined(__NVRAM_SECURE_DATA_STORAGE__) || defined(__NVRAM_BACKUP_PARTITION__)
...
static kal_bool nvram_create_report_file()
{
    ...
    if (handle > FS_NO_ERROR)
    {
        ...
        for (index = LIDChanged; index > 0; index--)
        {
            ...
            */
            if (!nvram_util_get_data_item(&ldi, LIDChangeArray[index]))
            {
                continue;
            }

            if (ldi->description == NULL)
            {
                continue;
            }
...
//    *CodeFileSizeTotal = 2 * BytesPerCluster * (1 + (NVRAM_EF_SYS_SIZE - 1) / BytesPerCluster);
    ...
    *CodeFileSizeTotal = 2 * NVRAM_RoundUp(NVRAM_EF_SYS_SIZE, BytesPerCluster);
    ...
    do
    {
        ...
        {
            *CodeFileSizeTotal += (NVRAM_IS_ATTR_MULTIPLE(ldi->attr) ? 2 : 1) * NVRAM_RoundUp(onefilesize, BytesPerCluster);
        }
...
//        *CodeFileSizeTotal += BytesPerCluster * (1 + (infolid_size - 1) / BytesPerCluster);
        ...
        *CodeFileSizeTotal += (NVRAM_RoundUp(package_size, BytesPerCluster) + NVRAM_RoundUp(infolid_size, BytesPerCluster));
    }
#endif
}
...
static kal_bool nvram_malloc_disk_table(void)
{
    ...
    /* Step 1: Use FS_FindFirst to parse the nvram folder to get the file number and file size */
...
//                diskSize_of_folder[folder_idx] += BytesPerCluster * (1 + (fileinfo.FileSize - 1) / BytesPerCluster);
                diskSize_of_folder[folder_idx] += NVRAM_RoundUp(fileinfo.FileSize, BytesPerCluster);
    ...
    do
    {
        ...
        else
        {
            ...
            if (NVRAM_IS_ATTR_MULTIPLE(ldi->attr)) /* Process Multiple */
            {
                ...
                if (!nvram_search_disk_table(nvramname1, folder_index) ||
                    !nvram_search_disk_table(nvramname2, folder_index))
                {
...
kal_bool nvram_write_system_record(nvram_lock_state_enum lock_state)
{
    ...
    //NVRAM_BLOCKING_ASSERT(write_status == KAL_TRUE, 0, 0, 0);
...
static kal_bool nvram_supplementary_check(void)
{
    ...
    if (!nvram_supplementary_check_data_item(NVRAM_EF_CAT_TIMESTAMP_LID))
    {
        nvram_trace(TRACE_FUNC, FUNC_NVRAM_SUPPLEMENTARY_CHECK, 3);
        temp_result = nvram_reset_data_items(NVRAM_RESET_CAT, NVRAM_APP_RESERVED, NULL, 0, 0);
        nvram_ptr->sw_status |= NVRAM_SW_CAT_CHANGE;
    }
...
kal_bool nvram_init(task_indx_type task_indx)
{
    ...
    // Force link g_SEC_RO
    {
        extern const SEC_RO_ST g_SEC_RO;
        nvram_ltable_register((nvram_ltable_entry_struct*) &g_SEC_RO);
    }
    #endif
    ...
    /* Initial pseudo merge first */
        ...
        {
            nvram_del_rename_package_file(KAL_TRUE);    // Delete PACKALID
            nvram_del_rename_package_file(KAL_FALSE);   // Delete INFOALID
        ...
        /* Always remove SDS category from NVRAM_EF_SYS_LID during NVRAM upgrade
           to ensure the version default is getting from code instead of SDS */
        sys_lid_cat = ldi->category;
        ldi->category = sys_lid_cat & ~NVRAM_CATEGORY_BACKUP_SDS;
        ...
        if (strncmp((kal_char*) nvram_ptr->old_coded_version, (kal_char*) nvram_ptr->coded_version, CODED_DATA_VERSION_SIZE) != 0)
        {
            ...
            if (ld_init_result)
            {
                ...
                /* NVRAM_EF_SYS_LID needs to be reset, because system verno is different.
                   To avoid side effects in falvor/branch verno (record 2,3), reset it after nvram_supplementary_check */
        ...
        else    /* version compatible */
        {
            ...
            ld_init_result = nvram_supplementary_check();
            /* NVRAM_EF_SYS_LID will be reset in nvram_supplementary_check() on branch/flavor verno change */
        ...
        /* restore NVRAM_EF_SYS_LID's category */
        ldi->category = sys_lid_cat;
