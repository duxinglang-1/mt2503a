            ...
            if (lid_info.area != NVRAM_AREA_INVALID)
            {
                ...
                do
                {
                    ...
                    if (NVRAM_IS_ATTR_PACKAGE(ldi->attr))
                    {
                        ...
                        if (!strncmp(filename_compare, lid_info.itemname, sizeof(filename_compare)))       /* Compatible */
                        {
                            ...
                            {
                                lid_is_valid = KAL_FALSE;
                            }
                            break;  /* break out for loop */
                        }
                        else if (!strncmp(lid_info.itemname, ldi->fileprefix, FILE_PREFIX_LEN))
                        {
                            lid_is_valid = KAL_FALSE;
                            break;
                        }
...
kal_bool nvram_del_rename_package_file(kal_bool is_exist_package_file)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
kal_uint32 nvram_calculate_filename_chksum(nvram_ltable_entry_struct *ldi)
{
    ...
    *(word_chksum + 1) = nvram_util_caculate_checksum((kal_uint8*)ldi->fileverno, 3);
    *word_chksum = nvram_util_caculate_checksum((kal_uint8*)ldi->fileprefix, 4);
