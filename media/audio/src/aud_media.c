...
/* If __MED_IN_ASM__ is defined, we should use MMA interface to play mpl files */
        ...
        case MED_TYPE_F4A:
			...
			#if defined(__VIBRATION_SPEAKER_SUPPORT__)&& defined(__MTK_TARGET__)
                {   //set volume again to avoid gain change by vibration dummy audio
                    kal_uint8 vol_lvl = aud_get_volume_level(aud_context_p->audio_mode, AUD_VOLUME_MEDIA);
                    if (!(VIBR_IsVibrating() && vol_lvl == 0))
                    {   // Prevent set silence volume & cause vibration stop
                        aud_set_active_ring_tone_volume();
                    }
                }
            #endif
