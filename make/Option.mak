...
ifeq ($(strip $(WAPI_SUPPORT)),TRUE)
  ifdef WIFI_AUTHENTICATION_LEVEL
    ifeq ($(strip $(WIFI_AUTHENTICATION_LEVEL)),WIFI_FEATURE_SCAN_ONLY)
      $(warning ERROR: Please turn off WAPI_SUPPORT for WIFI_FEATURE_SCAN_ONLY !)
      DEPENDENCY_CONFLICT = TRUE
    endif
    ifeq ($(strip $(WIFI_AUTHENTICATION_LEVEL)),WIFI_AUTHENTICATION_PSK_ONLY)
...
#Add this configuration for enable full image FOTA
ifdef FUNET_ENABLE
  ifneq ($(strip $(FUNET_ENABLE)),NONE)
    ifneq ($(strip $(CARD_DOWNLOAD)),NONE)
...
FALL_DETECTION_SUPPORT = TRUE
FALL_DETECTION_SENSOR_USE = LSM6DS3


ifdef FALL_DETECTION_SUPPORT
  ifeq ($(strip $(FALL_DETECTION_SUPPORT)),TRUE)
    COM_DEFS += FALL_DETECT_SUPPORT
   	COMPLIST +=  august
    CUS_REL_SRC_COMP += august
    ifeq ($(strip $(FALL_DETECTION_SENSOR_USE)),LSM6DS3)
      COMPLIST +=  lsm6ds3
      CUS_REL_SRC_COMP += lsm6ds3
      COM_DEFS += FALL_DETECT_SENSOR_USE_LSM6DS3
    else 
      $(warning ERROR: Fall Detection Sensor is not selected!)
    endif
  endif
endif

AW9106_LED_SUPPORT = TRUE

ifeq ($(strip $(AW9106_LED_SUPPORT)),TRUE)
	COMPLIST +=  led
	COM_DEFS += LED_AW9106_SUPPORT
endif

# *************************************************************************
# OPT Feature Options Section
# *************************************************************************
# opt project begin

#OPT_PROJECT
ifdef OPT_PROJECT
  COM_DEFS += $(OPT_PROJECT)
endif

ifdef OPT_CUSTOMER
  COM_DEFS += $(OPT_CUSTOMER)
endif

ifdef OPT_CUSTOMER_DRV
  COM_DEFS += $(OPT_CUSTOMER_DRV)
endif

ifdef OPT_CUSTOMER_VERSION
  COM_DEFS += $(OPT_CUSTOMER_VERSION)
endif

#OPT_MMI
#for MMI resource folder such as audioplayer skin, theme..
ifneq ($(strip $(OPT_MMI_RESOURCE)),NONE)
  OPT_MMI_RESOURCE := $(strip $(OPT_MMI_RESOURCE))
endif

ifdef OPT_MMI_REF_LIST_TXT
    ifneq ($(strip $(OPT_MMI_REF_LIST_TXT)),NONE)
      OPT_MMI_REF_LIST_TXT := $(strip $(OPT_MMI_REF_LIST_TXT))
    endif
endif

#for audio resource folder
#ifneq ($(strip $(OPT_AUDIO_RESOURCE)),NONE)
#  OPT_AUDIO_RESOURCE := $(strip $(OPT_AUDIO_RESOURCE))
#endif

ifeq ($(strip $(OPT_USE_COMMON_IMG)),TRUE)
  COM_DEFS 	+= __OPT_USE_COMMON_IMG__
endif

ifdef OPT_IMAGE_COMMON
  COM_DEFS += $(OPT_IMAGE_COMMON)
endif

ifneq ($(strip $(OPT_IMAGE_RESOURCE)),NONE)
  OPT_IMAGE_RESOURCE := $(strip $(OPT_IMAGE_RESOURCE))
endif

#OPT_DRIVER
ifneq ($(strip $(OPT_CODEGEN)),NONE)
  OPT_CODEGEN := $(strip $(OPT_CODEGEN))
endif

ifneq ($(strip $(OPT_RF_RESOURCE)),NONE)
  OPT_RF_RESOURCE := $(strip $(OPT_RF_RESOURCE))
endif

ifdef OPT_NVRAM_VERSION
  ifneq ($(strip $(OPT_NVRAM_VERSION)),NONE)
    COM_DEFS 	+= __OPT_NVRAM_VERSION__
    OPT_NVRAM_VERSION := $(strip $(OPT_NVRAM_VERSION))
  endif
endif
...
ifeq ($(strip $(AUGUST_ALL_FUNC_SUPPORT)),TRUE)
  COM_DEFS 	+= __AUGUST_ALL_FUNC_SUPPORT__
    COM_DEFS 	+=__ST_LSM6DS3__
endif
