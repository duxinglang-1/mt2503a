...
NEED_COPY_LCDRESOURCE_DIR += $(FIXPATH)\plutommi\Customer\LcdResource\MainLcd$(strip $(MAIN_LCD_SIZE))\FTO
    ...
    ifdef OPT_MMI_RESOURCE
            ifneq ($(strip $(call Upper,$(OPT_MMI_RESOURCE))),NONE)
              NEED_COPY_LCDRESOURCE_DIR += $(FIXPATH)\$(MMIDIR)\OPT_MMI\OPT_MMI_RESOURCE\$(call Upper,$(strip $(OPT_MMI_RESOURCE)))
            else ifneq ($(strip $(call Upper,$(FINGER_TOUCH_SUPPORT))),NONE)
    ...
    ifdef OPT_MMI_RESOURCE
        ifneq ($(strip $(call Upper,$(OPT_MMI_RESOURCE))),NONE)
          NEED_COPY_LCDRESOURCE := $(call CheckNeedCopyDir,$(NEED_COPY_LCDRESOURCE_DIR),.txt) $(call CheckNeedCopyDir,$(NEED_COPY_LCDRESOURCE_DIR),.c) $(call CheckNeedCopyDir,$(NEED_COPY_LCDRESOURCE_DIR),.h)
        else ifeq ($(call Upper,$(strip $(CUSTOM_RELEASE))),FALSE)
          NEED_COPY_LCDRESOURCE_DIR += $(FIXPATH)\$(MMIDIR)\Customer\LcdResource\MainLcd$(strip $(MAIN_LCD_SIZE))\FTE
...
ifneq ($(filter L1S BASIC,$(call Upper,$(PROJECT))),)
  ...
  ifeq ($(strip $(MMI_VERSION)),COSMOS_MMI)
    ...
    ifdef OPT_MMI_REF_LIST_TXT
        ifneq ($(strip $(call Upper,$(OPT_MMI_REF_LIST_TXT))),NONE)
          NEED_COPY_REFLIST_LIST := $(FIXPATH)\$(MMIDIR)\OPT_MMI\OPT_MMI_REF_LIST_TXT\$(call Upper,$(strip $(OPT_MMI_REF_LIST_TXT)))\ref_list.txt
        else
  ...
  ifneq ($(strip $(MMI_VERSION)),EMPTY_MMI)
    ...
    NEED_COPY_MMISWITCH_LIST += $(FIXPATH)\$(MMIDIR)\OPT_MMI\OPT_FEATURES_SWITCH\MMI_features_switch$(strip $(ORIGINAL_CUSTOMER)).h
    ifdef FLAVOR
...
# It seems only NEPTUNE resgen needs it
	@echo [Dependency] $(NEED_COPY_REFLIST) >>$(basename $@).log
     ifdef OPT_MMI_REF_LIST_TXT
        ifneq ($(strip $(call Upper,$(OPT_MMI_REF_LIST_TXT))),NONE)
	@copy /y $(NEED_COPY_REFLIST) $(FIXPATH)\$(MMIDIR)\Customer\CustResource\$(strip $(MMI_BASE))\ref_list.txt >NUL
        endif
    endif
...
ifdef OPT_CODEGEN
    ifneq ($(strip $(OPT_CODEGEN)),NONE)
	@echo OPT_COPY Start copy code gen file
	@if exist $(FIXPATH)\custom\OPT_DRIVER\OPT_CODEGEN\$(call Upper,$(strip $(OPT_CODEGEN))).dws \
		(@echo OPT_COPY Copying code gen file $(OPT_CODEGEN).dws...) & \
	     	(copy /y $(FIXPATH)\custom\OPT_DRIVER\OPT_CODEGEN\$(call Upper,$(strip $(OPT_CODEGEN))).dws $(FIXPATH)\custom\codegen\$(strip $(BOARD_VER))\codegen.dws)   
    endif
endif

ifdef OPT_RF_RESOURCE
    ifneq ($(strip $(OPT_RF_RESOURCE)),NONE)
	@echo OPT_COPY Start copy rf resource files
	@if exist $(FIXPATH)\custom\OPT_DRIVER\OPT_RF_RESOURCE\$(call Upper,$(strip $(OPT_RF_RESOURCE)))  \
		(@echo OPT_COPY Copying rf resource files from $(OPT_RF_RESOURCE)...) & \
	    	(xcopy /y $(FIXPATH)\custom\OPT_DRIVER\OPT_RF_RESOURCE\$(call Upper,$(strip $(OPT_RF_RESOURCE)))  $(FIXPATH)\custom\l1_rf\$(RF_MODULE))
    endif
endif

ifdef OPT_NVRAM_VERSION
    ifneq ($(strip $(OPT_NVRAM_VERSION)),NONE)
	@echo OPT_COPY Start copy nvram version file
	@if exist $(FIXPATH)\custom\OPT_DRIVER\OPT_NVRAM_VERSION\$(call Upper,$(strip $(OPT_NVRAM_VERSION))).h \
		(@echo OPT_COPY Copying nvram version file $(OPT_NVRAM_VERSION).h...) & \
		(copy /y $(FIXPATH)\custom\OPT_DRIVER\OPT_NVRAM_VERSION\$(call Upper,$(strip $(OPT_NVRAM_VERSION))).h $(FIXPATH)\custom\common\$(strip $(MMI_BASE))\OPT_NVRAM_VERNO.h > nul)
    endif
endif

	
ifeq ($(strip $(OPT_USE_COMMON_IMG)),TRUE)
	@echo OPT_COPY Start copy image resource files
	@if exist $(FIXPATH)\$(MMIDIR)\OPT_MMI\OPT_IMAGE_RESOURCE\$(OPT_IMAGE_RESOURCE)\
		(@if /I $(RUN_RESGEN) EQU TRUE \
			(@echo OPT_COPY Copying image resource files from $(OPT_IMAGE_RESOURCE)...) & \
			(rd /s /q $(FIXPATH)\$(MMIDIR)\OPT_MMI\OPT_IMAGE_RESOURCE\$(OPT_IMAGE_RESOURCE)\MainLCD\) & \
			(rd /s /q $(FIXPATH)\$(MMIDIR)\OPT_MMI\OPT_IMAGE_RESOURCE\$(OPT_IMAGE_RESOURCE)\SUBLCD\) & \
			($(FIXPATH)\$(MMIDIR)\Customer\ResGenerator\7za.exe x $(FIXPATH)\$(MMIDIR)\OPT_MMI\OPT_IMAGE_RESOURCE\$(OPT_IMAGE_RESOURCE)\image.zip -o$(FIXPATH)\$(MMIDIR)\OPT_MMI\OPT_IMAGE_RESOURCE\$(OPT_IMAGE_RESOURCE)\) & \
			(rd /s /q $(FIXPATH)\$(MMIDIR)\Customer\Images\PLUTO_TEMP\) & \
			(md $(FIXPATH)\$(MMIDIR)\Customer\Images\PLUTO_TEMP\) & \
			(xcopy /y /e $(FIXPATH)\$(MMIDIR)\OPT_MMI\OPT_IMAGE_RESOURCE\$(OPT_IMAGE_RESOURCE) $(FIXPATH)\$(MMIDIR)\Customer\Images\PLUTO_TEMP\) & \
			(del /q $(FIXPATH)\$(MMIDIR)\Customer\Images\PLUTO_TEMP\image.zip) \
		)
endif

ifdef OPT_AUDIO_RESOURCE
ifneq ($(strip $(OPT_AUDIO_RESOURCE)),NONE)
	@echo OPT_COPY Start copy audio resource files
	@if exist $(FIXPATH)\$(MMIDIR)\OPT_MMI\OPT_AUDIO_RESOURCE\$(call Upper,$(strip $(OPT_AUDIO_RESOURCE))).zip  \
		(@if /I $(RUN_RESGEN) EQU TRUE \
			(@echo OPT_COPY Copying audio resource file from $(OPT_AUDIO_RESOURCE).zip...) & \
			(copy /y $(FIXPATH)\$(MMIDIR)\OPT_MMI\OPT_AUDIO_RESOURCE\$(call Upper,$(strip $(OPT_AUDIO_RESOURCE))).zip $(FIXPATH)\$(MMIDIR)\Customer\Audio\PLUTO\audio.zip) \
		)
endif		
endif
