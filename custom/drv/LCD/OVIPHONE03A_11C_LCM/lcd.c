...
void init_lcd_interface_old(void)
{

#if 1//defined(THOR61A_EVB_BB)
  ...
  SET_LCD_SERIAL_IF_WR_1ST(0,5);//
  SET_LCD_SERIAL_IF_WR_2ND(0,5);
  SET_LCD_SERIAL_IF_RD_1ST(0,50);//
  SET_LCD_SERIAL_IF_RD_2ND(0,50);
...
{
#if 0
init_lcd_interface_old();
return;

#else
#if defined(THOR61A_EVB_BB)
#else
    PMU_CTRL_LDO_BUCK_SET_EN ldoSetEn;
    PMU_CTRL_LDO_BUCK_SET_VSIM_GPLDO_EN ldo_vsim_gpldo_en;
    PMU_CTRL_LDO_BUCK_SET_VOLTAGE val;
...
void LCD_Init_ST7789S(kal_uint32 bkground, void **buf_addr)
{
        ...
        extern const char gpio_lcm_rst_pin;
        kal_uint32  reset_pin = gpio_lcm_rst_pin -0x80;
    
        lcd_reset_hnd = DclGPIO_Open(DCL_GPIO, reset_pin);
        DclGPIO_Control(lcd_reset_hnd, GPIO_CMD_DISABLE_PULL, NULL);        // disable PD/PU
        DclGPIO_Control(lcd_reset_hnd, GPIO_CMD_SET_DIR_OUT, NULL);     // change to output
        DclGPIO_Control(lcd_reset_hnd, GPIO_CMD_SET_MODE_0, NULL);      // Set to mode 0
        
        DclGPIO_Control(lcd_reset_hnd, GPIO_CMD_WRITE_HIGH, NULL);
        Delayms(200);     // keep 200ms high
        
        DclGPIO_Control(lcd_reset_hnd, GPIO_CMD_WRITE_LOW, NULL);
        Delayms(10);
        DclGPIO_Control(lcd_reset_hnd, GPIO_CMD_WRITE_HIGH, NULL);
        Delayms(120);
    
        DclGPIO_Close(lcd_reset_hnd);
#endif
    ...
    LCD_DataWrite_ST7789S(0xc0);// 00 C0 40 60
    ...
    LCD_DataWrite_ST7789S(0x0f);  // 60Hz while FPA and BPA = 0x0C
...
Delayms(120);
LCD_CtrlWrite_ST7789S(0x36);
LCD_DataWrite_ST7789S (0x00);//C0 C8  
LCD_CtrlWrite_ST7789S(0x3a);
LCD_DataWrite_ST7789S (0x06);//05-RGB565  06-RGB666
LCD_CtrlWrite_ST7789S(0x21);
LCD_CtrlWrite_ST7789S(0xE7);
LCD_DataWrite_ST7789S (0x00);	 //10h-2 data;00h-1data
LCD_CtrlWrite_ST7789S(0x2a);
LCD_DataWrite_ST7789S (0x00);
LCD_DataWrite_ST7789S (0x00);
LCD_DataWrite_ST7789S (0x00);
...
LCD_DataWrite_ST7789S (0x33);
LCD_DataWrite_ST7789S (0x4f);
LCD_DataWrite_ST7789S (0x04);
LCD_DataWrite_ST7789S (0x0d);
LCD_DataWrite_ST7789S (0x19);
...
{  
  ...
  LCD_CtrlWrite_ST7789S(0x36);
  LCD_DataWrite_ST7789S(0x00);// 00 C0 40 60
  ...
  LCD_DataWrite_ST7789S(0xCB);
    ...
    LCD_CtrlWrite_ST7789S(0xB2);    
    LCD_DataWrite_ST7789S(0x0C);   //porch setting
    LCD_DataWrite_ST7789S(0x0C);  
    LCD_DataWrite_ST7789S(0x00);   
    LCD_DataWrite_ST7789S(0x33);   
    LCD_DataWrite_ST7789S(0x33); 
    ...
    LCD_CtrlWrite_ST7789S(0xB7);     
    LCD_DataWrite_ST7789S(0x02);   //64;VGH&VGL,12V&-8V

    LCD_CtrlWrite_ST7789S(0xC2);    
    LCD_DataWrite_ST7789S(0x01); 

    LCD_CtrlWrite_ST7789S(0xBB);    
    LCD_DataWrite_ST7789S(0x35);   // 0x3C 28;VCOM

    LCD_CtrlWrite_ST7789S(0xC3);     
    LCD_DataWrite_ST7789S(0x19);  //0x27

    LCD_CtrlWrite_ST7789S(0xC4);    
    LCD_DataWrite_ST7789S(0x20);  //0x3F

    LCD_CtrlWrite_ST7789S(0xC5);    
    LCD_DataWrite_ST7789S(0x20);   //vcom shift.0V  20

    LCD_CtrlWrite_ST7789S(0xD0);   
    LCD_DataWrite_ST7789S(0xA4);  
    LCD_DataWrite_ST7789S(0xA2);   //AVDD&AVCL,6.8v&-4.8v
    										  
    LCD_CtrlWrite_ST7789S(0xD2);     
    LCD_DataWrite_ST7789S(0x4C); 

    LCD_CtrlWrite_ST7789S(0xE8);    
    LCD_DataWrite_ST7789S(0x83);   //Booster CLK Select

    LCD_CtrlWrite_ST7789S(0xE9);   
    LCD_DataWrite_ST7789S(0x09);   //EQ
    LCD_DataWrite_ST7789S(0x09);   
    LCD_DataWrite_ST7789S(0x08); 
    ...
    LCD_DataWrite_ST7789S(0x00);   //TEWrite


    LCD_CtrlWrite_ST7789S(0xC6);     
    LCD_DataWrite_ST7789S(0x0F);   //0x09orig [DB7-5] 0forDotInv,1forColumnInv ,0xEF  列翻 

    LCD_CtrlWrite_ST7789S(0xE0);    
    LCD_DataWrite_ST7789S(0xD0);   //V0[7-4] & V63[3-0]
    LCD_DataWrite_ST7789S(0x06);   //V62[5-0]
    LCD_DataWrite_ST7789S(0x0C);   //V61[5-0]
    LCD_DataWrite_ST7789S(0x09);   //V59[4-0]
    LCD_DataWrite_ST7789S(0x09);   //V57[4-0]
    LCD_DataWrite_ST7789S(0x25);   //J1[5-4] & V50[3-0]
    LCD_DataWrite_ST7789S(0x2E);   //V43[6-0]
    LCD_DataWrite_ST7789S(0x33);   //V27[6-4] & V36[2-0]
    LCD_DataWrite_ST7789S(0x45);   //V20[6-0]
    LCD_DataWrite_ST7789S(0x36);   //J0[5-4] & V13[3-0]
    LCD_DataWrite_ST7789S(0x12);   //V6[4-0]
    LCD_DataWrite_ST7789S(0x12);   //V4[4-0]
    LCD_DataWrite_ST7789S(0x2E);   //V2[5-0]
    LCD_DataWrite_ST7789S(0x34);   //V1[5-0]
...
void LCD_BlockWrite_ST7789S(kal_uint16 startx,kal_uint16 starty,kal_uint16 endx,kal_uint16 endy)
{
 ...
 while (LCD_IS_RUNNING){};
    ...
    SET_LCD_ROI_CTRL_NUMBER_OF_CMD(1);
