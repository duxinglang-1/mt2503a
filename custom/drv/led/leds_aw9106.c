         ...
         unsigned char reg_addr, unsigned char reg_data)
{
    ...
    while(cnt < AW_I2C_RETRIES) {
        ...
        if(ret < 0) {
            kal_prompt_trace(MOD_UEM,"%s: i2c_write cnt=%d error=%d\n", __func__, cnt, ret);
        } else {
            break;
        }
        cnt ++;
        ...
        unsigned char reg_addr, unsigned char *reg_data)
{
    ...
    while(cnt < AW_I2C_RETRIES) {
        ret = i2c_smbus_read_byte_data(reg_addr, reg_data);
        if(ret < 0) {
            kal_prompt_trace(MOD_UEM,"%s: i2c_read cnt=%d error=%d\n", __func__, cnt, ret);
        } else {
            return 1;//*reg_data = ret;
            break;
        }
        cnt ++;
        msleep(AW_I2C_RETRY_DELAY);
         ...
         unsigned char reg_addr, unsigned char mask, unsigned char reg_data)
{
    ...
    aw9106_i2c_read( reg_addr, &reg_val);
    reg_val &= mask;
    reg_val |= reg_data;
    aw9106_i2c_write( reg_addr, reg_val);
...
static void aw9106_brightness_work(struct work_struct *work)
{
    ...
    aw9106_i2c_write( REG_WORK_MODE_P0, 0x00);   // led mode
    aw9106_i2c_write( REG_WORK_MODE_P1, 0x00);   // led mode

    aw9106_i2c_write( REG_EN_BREATH, 0x00);      // disable breath

    aw9106_i2c_write( REG_CTRL, 0x03);           // imax

    for(i=0; i<6; i++) {
        aw9106_i2c_write( REG_DIM00+i,
...
static void aw9106_led_blink(struct aw9106 *aw9106, unsigned char blink)
{
    ...
    if(aw9106->cdev.brightness > aw9106->cdev.max_brightness) {
        ...
        aw9106_i2c_write( REG_EN_BREATH, 0x3f);      // enable breath

        aw9106_i2c_write( REG_CONFIG_P0, 0x03);      // blink mode
        aw9106_i2c_write( REG_CONFIG_P1, 0x0f);      // blink mode

        aw9106_i2c_write( REG_FADE_TIME,
            (aw9106->fall_time<<3)|(aw9106->rise_time));    // fade time
        aw9106_i2c_write( REG_FULL_TIME,
        ...
        for(i=0; i<6; i++) {
            aw9106_i2c_write( REG_DIM00+i, 0x00);    // dimming
        }
...
static int aw9106_read_chipid(struct aw9106 *aw9106)
{
    ...
    while(cnt < AW_READ_CHIPID_RETRIES) {
        ret = aw9106_i2c_read( REG_ID, &reg_val);
        if (ret < 0) {
            dev_err(aw9106->dev,
                ...
                const char *buf, size_t count)
{
    ...
    if(2 == sscanf(buf, "%x %x", &databuf[0], &databuf[1])) {
        aw9106_i2c_write( (unsigned char)databuf[0], (unsigned char)databuf[1]);
    }
...
static int __init aw9106_i2c_init(void)
{
    ...
    if(ret){
        kal_prompt_trace(MOD_UEM,"fail to add aw9106 device into i2c\n");
        return ret;
    }
