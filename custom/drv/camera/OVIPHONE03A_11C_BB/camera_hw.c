...
//	    	CameraSccbInit(CAMERA_SW_SCCB, CAM_SCCB_SCL_MODE, CAM_SCCB_SDA_MODE, CAM_SCCB_SW_I2C_DELAY);

		CameraSccbInit(CAMERA_HW_SCCB, CAM_SCCB_SCL_MODE, CAM_SCCB_SDA_MODE, CAM_SCCB_SW_I2C_DELAY);

//QQ		CamDclPMU_Enable(VCAMD, PMU_VOLT_02_800000_V);	/* PMU Enable VCAM_D, 2.8V */ /*camera interface fix 2.8V. DOVDD= 2.8,DVDD = 1.5 (LDO)*/
		CamDclPMU_Enable(VCAMA, PMU_VOLT_02_800000_V);	/* PMU Enable VCAM_A, 2.8V */
	}
	else
	{
//QQ		CamDclPMU_Disable(VCAMD);		/* PMU Disable VCAM_D, 2.8V */
