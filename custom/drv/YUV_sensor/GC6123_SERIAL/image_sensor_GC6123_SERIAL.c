...
/*	GC6123_SERIAL_SET_PAGE2;
	...
	CamWriteCmosSensor(0x03, 0x24); //20
	CamWriteCmosSensor(0xfd, 0x04); 
	CamWriteCmosSensor(0xf1, 0x07); //00

//=================QQ======================
//	if ((int_lane_num == 1) && (ddr_enable == KAL_FALSE))
	{ //QQ for 1 lane
//		CamWriteCmosSensor(0x03, 0x20); //20
//		CamWriteCmosSensor(0xfd, 0x00); //00
//		CamWriteCmosSensor(0xf1, 0x05); 
	}
//	else if ((int_lane_num == 2) && (ddr_enable == KAL_FALSE))
	{//QQ for 2 lane
//		CamWriteCmosSensor(0x03, 0x24); //20
//		CamWriteCmosSensor(0xfd, 0x04); 
//		CamWriteCmosSensor(0xf1, 0x07); //00
	}
//	else if ((int_lane_num == 2) && (ddr_enable == KAL_TRUE))
	{
		CamWriteCmosSensor(0x02, 0x20); //LSB first 	
		CamWriteCmosSensor(0x03, 0x24); //20
		CamWriteCmosSensor(0xfd, 0x04); 
		CamWriteCmosSensor(0xf1, 0x07); //00
	}
