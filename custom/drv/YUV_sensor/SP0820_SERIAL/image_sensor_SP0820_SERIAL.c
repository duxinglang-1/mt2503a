 ...
 * Filename:
 * ---------
 *   image_sensor_SP0820_SERIAL.c  V1.0
 *
 * Project:
 * --------
 *   Maui_sw
...
{ 
		...
		CamWriteCmosSensor(0x32,0x07);
		dbg_print("SP0820_SERIALInitialSetting\r\n");
...
static kal_uint32 SP0820_SERIALPowerOn(void)
{
  ...
  kal_sleep_task(10);
   ...
   /* set sensors chip enable pin to low to activate SP0820 */
   CamPdnPinCtrl(SP0820_SERIALSensor.SensorIdx, 1);   //add 20121017
   
    kal_sleep_task(10);                                //add 20121017
   CamPdnPinCtrl(SP0820_SERIALSensor.SensorIdx, 0);   //need to check power on sequency //
   kal_sleep_task(10);
   
  /* Reset the sensor */
  CamRstPinCtrl(SP0820_SERIALSensor.SensorIdx, 1);
  kal_sleep_task(1);
  CamRstPinCtrl(SP0820_SERIALSensor.SensorIdx, 0);
  kal_sleep_task(4);
  CamRstPinCtrl(SP0820_SERIALSensor.SensorIdx, 1);
  kal_sleep_task(2);
  
   CameraSccbSetAddr(CAMERA_SCCB_SENSOR, SP0820_SERIAL_WRITE_ID_0);
   kal_sleep_task(5);
       
   SensorId = CamReadCmosSensor(0x02);
	   
    #ifndef WIN32
       dbg_print("SP0820_SERIALPowerOn  SensorId=%x\r\n", SensorId);
    #endif	       
       kal_prompt_trace(MOD_ENG,"SP0820_SERIALPowerOn SP0820SensorId = %x\r\n",SensorId);
...
//  GPIO_ModeSetup(SP0820_PCLK_PIN,2);   	//CMPCLK	
//  GPIO_ModeSetup(SP0820_DATA0_PIN,2);   	//Data0
