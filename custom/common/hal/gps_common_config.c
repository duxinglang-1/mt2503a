...
//define 32K clock output mode refer to BB chip. for new chip, please get spec from WCP1SE8/DRV1 team
#if defined(MT6250)
    #define gps_f32k_clk_mode 4
#else
#endif
...
//use drv tool generated eint pin to prevent one EINTx--> multiple GPIO.
extern const char gpio_gps_eint_pin;
extern const char gpio_gps_eint_pin_M_EINT;
extern const unsigned char  GPS_EINT_NO;
...
#elif defined(MT6252)	//configure lpo clock output
{
	switch (numLpo)
	{
		case (GPS_LPO_GPIO_NUM | 1)://CLK0
			modeLpo = 3;
			GPS_GPIO_SetClkOut(0, mode_f32k_ck);
			GPS_GPIO_SetClkDiv(0, 0);	
			break;

		case (GPS_LPO_GPIO_NUM | 3)://CLK1
			modeLpo = 5;
			GPS_GPIO_SetClkOut(1, mode_f32k_ck);
			GPS_GPIO_SetClkDiv(1, 0);	
			break;
			
		case (GPS_LPO_GPIO_NUM | 23)://CLK2
			modeLpo = 5;
			GPS_GPIO_SetClkOut(2, mode_f32k_ck);
			GPS_GPIO_SetClkDiv(2, 0);	
			break;
		case (GPS_LPO_GPIO_NUM | 33)://CLK2
			modeLpo = 1;
			GPS_GPIO_SetClkOut(2, mode_f32k_ck);
			GPS_GPIO_SetClkDiv(2, 0);	
			break;
		case (GPS_LPO_GPIO_NUM | 43)://CLK2
	  case (GPS_LPO_GPIO_NUM | 64)://CLK2
		case (GPS_LPO_GPIO_NUM | 70)://CLK2
			modeLpo = 3;
			GPS_GPIO_SetClkOut(2, mode_f32k_ck);
			GPS_GPIO_SetClkDiv(2, 0);	
			break;
			
		case (GPS_LPO_GPIO_NUM | 8)://CLK3
		case (GPS_LPO_GPIO_NUM | 27)://CLK3
			modeLpo = 3;
			GPS_GPIO_SetClkOut(3, mode_f32k_ck);
			GPS_GPIO_SetClkDiv(3, 0);	
			break;

		case (GPS_LPO_GPIO_NUM | 26)://CLK3
			modeLpo = 3;
			GPS_GPIO_SetClkOut(4, mode_f32k_ck);
			GPS_GPIO_SetClkDiv(4, 0);	
			break;

		case (GPS_LPO_GPIO_NUM | 45)://CLK3
			modeLpo = 3;
			GPS_GPIO_SetClkOut(5, mode_f32k_ck);
			GPS_GPIO_SetClkDiv(5, 0);	
			break;

		case (GPS_LPO_GPIO_NUM | 34)://CLK3
			modeLpo = 1;
			GPS_GPIO_SetClkOut(6, mode_f32k_ck);
			GPS_GPIO_SetClkDiv(6, 0);	
			break;
	}
}

#elif defined(MT6253T) || defined(MT6253) || defined(MT6253E) || defined(MT6253L)  
{
	switch (numLpo)
	{
...
#elif defined(MT6250)
{
    switch (numLpo)
    {
          case (GPS_LPO_GPIO_NUM | 0): //CLKM0
              modeLpo = 1;
              GPS_GPIO_SetClkOut(0, gps_f32k_clk_mode);
              //GPS_GPIO_SetClkDiv(0, 0);
              break;
        
          case (GPS_LPO_GPIO_NUM | 11): //CLKM1
              modeLpo = 6;
              GPS_GPIO_SetClkOut(1, gps_f32k_clk_mode);
              break;
        
          case (GPS_LPO_GPIO_NUM | 21): //CLKM2
              modeLpo = 1;
              GPS_GPIO_SetClkOut(2, gps_f32k_clk_mode);
              break;
        
          case (GPS_LPO_GPIO_NUM | 48): //CLKM3
              modeLpo = 2;
              GPS_GPIO_SetClkOut(3, gps_f32k_clk_mode);
              break;
        
          case (GPS_LPO_GPIO_NUM | 50):  //CLKM4
              modeLpo = 2;
              GPS_GPIO_SetClkOut(4, gps_f32k_clk_mode);
              break;
        
          case (GPS_LPO_GPIO_NUM | 4):  //CLKM5
              modeLpo = 3;
              GPS_GPIO_SetClkOut(5, gps_f32k_clk_mode);
              break;
        case (GPS_LPO_GPIO_NUM | 18): //CLKM6
              modeLpo = 4;
              GPS_GPIO_SetClkOut(6, gps_f32k_clk_mode);

              break;
        case (GPS_LPO_GPIO_NUM | 24): //CLKM7
              modeLpo = 4;
              GPS_GPIO_SetClkOut(7, gps_f32k_clk_mode);
...
#elif defined(MT6260)
{
    switch (numLpo)
    {
          case (GPS_LPO_GPIO_NUM | 21): //CLKM0
              modeLpo = 3;
              GPS_GPIO_SetClkOut(0, mode_f32k_ck);
              //GPS_GPIO_SetClkDiv(0, 0);
              break;        
          case (GPS_LPO_GPIO_NUM | 45): //CLKM1
              modeLpo = 3;
              GPS_GPIO_SetClkOut(1, mode_f32k_ck);
              break;
     }
...
#elif defined(MT6261)
{
    switch (numLpo)
    {
          case (GPS_LPO_GPIO_NUM | 18): //CLKM0
              modeLpo = 3;
              GPS_GPIO_SetClkOut(0, mode_f32k_ck);
              //GPS_GPIO_SetClkDiv(0, 0);
              break;        
          case (GPS_LPO_GPIO_NUM | 25): //CLKM1
              modeLpo = 3;
              GPS_GPIO_SetClkOut(1, mode_f32k_ck);
              break;
     }
...
#elif defined(MT6255)
{
    switch (numLpo)
    {
          ...
          case (GPS_LPO_GPIO_NUM | 79):  //CLKM5
              ...
              GPS_GPIO_SetClkDiv(5, 0);
              break;
        case (GPS_LPO_GPIO_NUM | 61): //CLKM6
              modeLpo = 3;
              GPS_GPIO_SetClkOut(6, mode_f32k_ck);
              GPS_GPIO_SetClkDiv(6, 0);
              break;
        case (GPS_LPO_GPIO_NUM | 62): //CLKM7
              modeLpo = 3;
              GPS_GPIO_SetClkOut(7, mode_f32k_ck);
              GPS_GPIO_SetClkDiv(7, 0);
              break;
        case (GPS_LPO_GPIO_NUM | 104): //CLKM8
              modeLpo = 4;
              GPS_GPIO_SetClkOut(8, mode_f32k_ck);
              GPS_GPIO_SetClkDiv(8, 0);
              break;
...
void GPS_GetUartMode(kal_uint8 numUart)
{
...
#elif defined(MT6252) //configure uart
	switch (numUart)
	{	                
		case (GPS_UART_GPIO_NUM | 5):
            gps_urxd_mode = 5;
            gps_utxd_mode = 5;
            gps_uart_port = uart_port3;       
            break;
            
		case (GPS_UART_GPIO_NUM | 20):
            gps_urxd_mode = 1;
            gps_utxd_mode = 1;
            gps_uart_port = uart_port3;       
            break;
            
		case (GPS_UART_GPIO_NUM | 22):
            gps_urxd_mode = 1;
            gps_utxd_mode = 1;
            gps_uart_port = uart_port2;       
            break;
    ...
    default:
        ...
        switch (numUart)
        {                   
            case (GPS_UART_GPIO_NUM | 0):
                gps_urxd_mode = 5;
                gps_utxd_mode = 5;
                gps_uart_port = uart_port3;       
                break;
               
            case (GPS_UART_GPIO_NUM | 23):
                gps_urxd_mode = 1;
                gps_utxd_mode = 1;
                gps_uart_port = uart_port2;       
                break;
                
            case (GPS_UART_GPIO_NUM | 48):
                gps_urxd_mode = 3;
                gps_utxd_mode = 3;
                gps_uart_port = uart_port3;       
                break;
    
            case (GPS_UART_GPIO_NUM | 57):
                gps_urxd_mode = 4;
                gps_utxd_mode = 4;
                gps_uart_port = uart_port2;       
                break;
    
            case (GPS_UART_GPIO_NUM | 60):
                gps_urxd_mode = 3;
                gps_utxd_mode = 3;
                gps_uart_port = uart_port3;       
                break;
...
#if defined (MT6260)||defined (MT6250)|| defined (MT6252)|| defined (MT6255)||defined (MT6256)||defined (MT6261)/* avoid one EINT number -> multiple GPIO PIN */
        //new eint configure metod:
        if (GPS_EINT_NO != EINT_CHANNEL_NOT_EXIST)
        {
           if (gpio_gps_eint_pin != 0xFF)
           {	//use drv tool generated eint pin to prevent one EINTx--> multiple GPIO.
              handle=DclGPIO_Open(DCL_GPIO,(gpio_gps_eint_pin&0x7f));
              DclGPIO_Control(handle,gpio_gps_eint_pin_M_EINT,0);
           }
           eint_channel = GPS_EINT_NO;
        }
        else
        {
        		eint_channel = 0xFF;
    ...
    switch (eint_channel)
    {
        ...
        switch (eint_channel)
        {
                ...
                case 3:
                    GPS_GPIO_ModeSetup(64, 1);
                    break;
                case 4:/* there are 2 pin for EINT4:GPIO65 & GPIO71 .need see hw setting*/
                    GPS_GPIO_ModeSetup(65, 1);
                    break;
                case 5:/* there are 2 pin for EINT5:GPIO72 & GPIO104 .need see hw setting*/
                    GPS_GPIO_ModeSetup(72, 2);
                    break;
                case 6:/* there are 2 pin for EINT6:GPIO79 & GPIO102 .need see hw setting*/
                    GPS_GPIO_ModeSetup(102, 2);
                    break;
                case 7:
                    GPS_GPIO_ModeSetup(6, 3);
                    break;
                ...
                case 11:
                    ...
                    break;
                case 12:
                    GPS_GPIO_ModeSetup(1, 4);
                    break;
                case 13:
                    GPS_GPIO_ModeSetup(91, 2);
                    break;
                case 14:
                    GPS_GPIO_ModeSetup(101, 3);
                    break;
...
#endif  /* avoid one EINT number -> multiple GPIO PIN */
#endif /* __SMART_PHONE_MODEM__ */
    return eint_channel;
