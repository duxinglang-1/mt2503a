...
sub scatGen_verno
{
    return "v6.99 -> p7.00 -> p7.01 -> p7.04 -> p7.05 -> p7.09 -> p7.14 -> p7.24" . " + scatFrame". scatFrame::scatFrame_verno() ;
    # p7.24 by Dongming,  20160114, put l1 obj into cached_extsram when zimage false
...
sub layout13
{
    ...
    #2503 project. fota enable, aapmc for bootloader disable, bootloader alwasy suffer memory expired.
    #So, change the defuat bl reserved size.
    if((&sysUtil::is_MT6261_Family($bb)) && 
       (defined &$feature_query_function_ptr('FUNET_ENABLE') and &$feature_query_function_ptr('FUNET_ENABLE') ne 'NONE') && 
       (defined &$feature_query_function_ptr('BLRESERVEDSIZE_AUTOCONFIG') and &$feature_query_function_ptr('BLRESERVEDSIZE_AUTOCONFIG') eq 'FALSE'))
    {
		$EXT_BL_MIN_SIZE = 160 * 1024;
    }
