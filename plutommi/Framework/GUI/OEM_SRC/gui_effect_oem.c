...
static void gui_list_menu_smooth_scrolling_draw_highlight_filler(
				S32 x, 
				S32 y, 
				S32 width, 
				S32 height);
...
static MMI_BOOL gui_ssp_pen_down_on_half_item(
	mmi_pen_point_struct point, 
	S32 new_highlight_item_index);
...
static MMI_BOOL gui_ssp_pen_pre_hdlr(
	U16 dm_control_id, 
	mmi_pen_point_struct point, 
...
static MMI_BOOL gui_ssp_menu_translate_pen_position(
	fixed_list_menu *m, 
	S32 x, 
	S32 y, 
	S32 *item_index);
...
static void gui_matrix_menu_smooth_scrolling_show_item(
	S32 item_index, 
	S32 x, 
	S32 y);
static void gui_ssp_show_matrix_menu (
	S32 offset, 
	S32 x1, 
	S32 y1, 
	S32 x2, 
...
static void gui_matrix_ssp_enter_free_run_recover_mode(
	S32 final_y_offset, 
	S32 momentum);

static gui_menu_type_enum gui_matrix_menu_smooth_scrolling_get_menu_type(void);
static MMI_BOOL gui_matrix_ssp_pen_pre_hdlr(
	U16 dm_control_id, 
	mmi_pen_point_struct point, 
	mmi_pen_event_type_enum pen_event);
...
extern MMI_BOOL gblock_list_animation_highlight;
...
void* gui_list_menu_get_menuitem(S32 index, gui_menu_type_enum menu_type)
{
    void *item = NULL;
...
#if  0//__MMI_GUI_SUPPORT_LIST_SLIDE_TUNE__
/* under construction !*/
#endif
static void gui_effect_slide_run(void)
{
...
/* under construction !*/
            ...
            show_scrollbar = gui_is_list_menu_show_vertical_scrollbar(&MMI_fixed_list_menu);//gui_list_menu_has_scrollbar();
#endif  // __MMI_FTE_SUPPORT__            
            if (!gui_is_fixed_list_menu_empty())//(MMI_fixed_list_menu.n_items > 0)
            {
                ...
                line_height -= MMI_fixed_list_menu.first_displayed_offset;
   				...
   				grid_x = gui_menu_compute_grid_line_x_pos(MMI_fixed_list_menu.x, &MMI_fixed_list_menu);
#if 0
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
                ...
                else if (!gui_is_fixed_list_menu_empty())//(MMI_fixed_list_menu.n_items != 0)
                {
...
/* under construction !*/
                            ...
                            if (g_gui_list_get_menuitem_flags && gui_is_index_in_menu_valid_region(display_index, &MMI_fixed_list_menu))//(display_index >= 0) && (display_index <= MMI_fixed_list_menu.n_items - 1))
                            {
...
/* under construction !*/
            ...
            {          
                ...
                {
                  //  gui_push_clip();
                    gui_push_and_set_clip(g_slide_effect->start_x, 
 ...
 *  void
 *****************************************************************************/
void gui_draw_image_with_alpha(
	S32 x, 
	S32 y, 
	gui_rect *out_rect, 
	PU8 image, 
...
void gui_effect_close(void)
{
    ...
    {   
        ...
        //gdi_layer_push_clip();
        gdi_layer_push_and_set_clip(g_slide_effect->old_x, g_slide_effect->old_y, g_slide_effect->old_x + g_slide_effect->width - 1, g_slide_effect->old_y + g_slide_effect->height - 1);
...
void gui_effect_slide_create(
	...
	S32 height, 
	S32 duration, 
	gui_effect_funcptr_type show_cb, 
	gui_effect_funcptr_type finish_cb, 
	S32 start_frame_index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static mmi_ret  gui_list_menu_smooth_scrolling_key_proc(mmi_event_struct *evt)
{
    ...
    if (key_evt->evt_id == EVT_ID_PRE_KEY)
    {
        ...
        {   
            ...
            #endif
                {
                    if (!gui_is_fixed_list_menu_empty()/*MMI_fixed_list_menu.n_items != 0*/ && !in_dynamic_item_buffer(MMI_fixed_list_menu.highlighted_item))
                        return MMI_RET_KEY_HANDLED;
                }
            }
            else if (g_gui_list_menu_smooth_scrolling_menu_type == GUI_MENU_TYPE_ASYNCDYNAMIC)
            {
                if (!gui_is_fixed_list_menu_empty()/*MMI_fixed_list_menu.n_items != 0*/ && !in_asyncdynamic_item_buffer(MMI_fixed_list_menu.highlighted_item))
                    return MMI_RET_KEY_HANDLED;
            }
        }
    #endif
...
static void gui_list_menu_smooth_scrolling_show_item(S32 item_index, S32 x, S32 y)
{
	...
	gui_menu_auto_adjust_current_displayed_item(&MMI_fixed_list_menu, item_index);
	/*
    if (item_index == MMI_fixed_list_menu.highlighted_item)
    {
        MMI_fixed_list_menu.current_displayed_item = -1;
...
{    
   ...
   // gdi_layer_push_clip();    
    gdi_layer_push_and_set_clip(0, 0, width - 1, height - 1);
...
#if  0//__MMI_GUI_LIST_MENU_SSK_OPTIMIZE__
/* under construction !*/
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
#endif/*defined(__MMI_UI_LIST_SUPPORT_UNEQUAL_HEIGHT_ITEM__)*/
    )
    {
        menu_full_height = 0;
        displayed_items = 0;
...
static MMI_BOOL gui_ssp_is_point_in_menu_rect(S32 x, S32 y)
{
    ...
    return (PEN_CHECK_BOUND(x, y, x1, y1, cm->width, MMI_fixed_list_menu.height));
    //{
    //    return MMI_TRUE;
    //}
 ...
 *****************************************************************************/
static MMI_BOOL gui_ssp_send_pen_event_to_item(
	mmi_pen_event_type_enum pen_event, 
	mmi_pen_point_struct *point, 
	S32 item_index, 
	S32 item_x, 
 ...
 *  void
 *****************************************************************************/
static void gui_ssp_show_list_menu(
	S32 menu_offset_y, 
	S32 x1, 
	S32 y1, 
	S32 x2, 
	S32 y2)
{
   ...
   f = m->normal_filler;
    ...
    if (m->flags & UI_LIST_MENU_INLINE_EDIT)
    {
        ...
        {
            S32 iwidth, iheight;
			gui_menu_auto_adjust_current_displayed_item(m,  m->n_items - 1);
			/*
    ...
    if (m->flags & UI_LIST_MENU_INLINE_EDIT && INLINE_FTE_MULTILINE_HEIGHT > GUI_SSE_MAX_MENU_HIGHLIGHT_HEIGHT)
    {
...
#endif  /* __MMI_FTE_SUPPORT__ */
        {
            ...
            /* use buffer b */
            gui_screen_switch_effect_get_frame_buffer_info(1, &g_gui_effect_layer_buffer, &buf_size);
#endif/*defined(__MMI_SCREEN_SWITCH_EFFECT__)&& !defined(__MMI_REDUCED_UI_BUFFER__)*/
            gdi_layer_create_cf_using_outside_memory(
                GUI_SCREEN_SWITCH_EFFECT_LAYER_COLOR_FORMAT,
                0,
                0,
            ...
            && first_menu_item_index != last_menu_item_index)
        {
            ...
            //    gdi_layer_push_clip();
                ...
                if (wgui_is_wallpaper_on_bottom() == MMI_FALSE)
                {
                    ...
                    gdi_layer_set_source_key(TRUE, GDI_COLOR_TRANSPARENT);    
                    gdi_layer_pop_and_restore_active();
					gui_list_is_hide_menu_callback_executed(result_rect.x, result_rect.y, result_rect.x + result_rect.width - 1, result_rect.y + result_rect.height - 1);
                    #if 0 
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
//#endif/*__LOW_COST_SUPPORT_COMMON__*/
  ...
  //  gdi_layer_push_clip();
   ...
   grid_x =  gui_menu_compute_grid_line_x_pos(x, m);
#if 0
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
static void gui_menuitem_handle_ssp_pen_up()
{
    ...
    {
        if (cm->checkbox_clicked_callback(m->highlighted_item))
        {                    
            gui_list_menu_smooth_scrolling_redraw_list();
        }
        return;
...
#endif/*defined(__MTK_TARGET__) && !defined(__TOUCH_PANEL_CAPACITY__)*/

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	if (gui_is_all_menuitem_in_list_disabled(&MMI_fixed_list_menu))
    {
        return MMI_FALSE;
    }
...
MMI_BOOL gui_ssp_pos_in_menu(S32 x, S32 y)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    return (x >= MMI_fixed_list_menu.x &&
 ...
 *  void 
 *****************************************************************************/
void gui_ssp_set_highlight_threshold(
	U16 highlight_first_threshold, 
	U16 hilight_last_threshold)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#endif/*defined(__MMI_CASCADED_OPTION_MENU__) */
...
MMI_BOOL gui_list_menu_is_support_smooth_scrolling(void)
{
    ...
    if (
		...
		(g_dm_data.s32CatId == MMI_CATEGORY261_ID) || 
 ...
 *  None 
 *****************************************************************************/
void wgui_reset_ssp_and_sync_menu_display_when_freerun(void)
{
     gui_cancel_timer(gui_ssp_update_frame);
