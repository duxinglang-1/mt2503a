...
void gui_status_icon_bar_arrange_oem(void)
{
    ...
    MMI_BOOL bar_baseline;
#if (!defined(__MMI_STATUS_ICONBAR_COSMOS_STYLE__) && defined(__MMI_FTE_SUPPORT__) && (MMI_MAX_SIM_NUM >= 3))
...
/* under construction !*/
    ...
    else
    {
		...
		x2 = wgui_status_icon_bar_icons[bar->icon_list[1]].x - STATUS_ICON_X_GAP;
    }
...
/* under construction !*/
    ...
    {    
        ...
        if (icon_id >= STATUS_ICON_FULL_SMS && icon_id <= STATUS_ICON_SIM3_FULL_SMS)
        {
            wgui_status_icon_bar_icons[STATUS_ICON_SLAVE_FULL_SMS].x = wgui_status_icon_bar_icons[icon_id].x;
            wgui_status_icon_bar_icons[STATUS_ICON_SIM3_FULL_SMS].x = wgui_status_icon_bar_icons[icon_id].x;
            icon_id = STATUS_ICON_SIM3_FULL_SMS;
        }
        #endif  //(MMI_MAX_SIM_NUM >= 4)
#endif
        icon_id = gui_status_icon_bar_search_next_icon(icon_id);
...
if(g_wgui_status_icon_bar_warning_mode)
{
    ...
    if (bar->icon_list != NULL)
    {
        for (i = 0; i < bar->n_icons; i++)
        {
            ...
            #else  //(MMI_MAX_SIM_NUM >= 4)
            else if (icon_id >= STATUS_ICON_FULL_SMS && icon_id <= STATUS_ICON_SIM3_FULL_SMS)
            #endif  //(MMI_MAX_SIM_NUM >= 4)
            {
