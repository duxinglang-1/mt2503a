...
#else//#elif defined(__MMI_DEFAULT_THEME_1__)
UI_filled_area wgui_pop_up_dialog_background =
    {UI_FILLED_AREA_TYPE_COLOR | UI_FILLED_AREA_DOUBLE_BORDER | UI_FILLED_AREA_ROUNDED_BORDER |
UI_FILLED_AREA_SHADOW_DOUBLE_LINE | UI_FILLED_AREA_SHADOW,
    UI_NULL_IMAGE,
...
static void ShowCommonCategoryPopupScreenType2(
        U16 title,
        U16 title_icon,
        U16 left_softkey,
        U16 right_softkey,
 ...
 *  void
 *****************************************************************************/
void wgui_draw_small_screen_background(
	S32 x1, 
	S32 y1, 
	S32 x2, 
	S32 y2, 
    ...
    if (mmi_frm_is_screen_width_height_swapped())
    {
        if (current_MMI_theme->rotated_popup_screen_bg_filler->b != NULL)
        {
            gdi_image_draw_resized(
				...
				(x2 - x1 + 1), 
				(y2 - y1 + 1), 
				current_MMI_theme->rotated_popup_screen_bg_filler->b);
        //    return;
        }
		else
			wgui_draw_small_screen_background(
				...
				x2, 
				y2, 
				current_MMI_theme->rotated_popup_screen_bg_filler);
 ...
 *  ShowCategory63Screen
 * DESCRIPTION
 *  Displays the category63 screen(slim )
 * PARAMETERS
 *  message             [IN]        Message string
 *  message_icon        [IN]        Message icon
 *  history_buffer      [IN]        History buffer
...
void RedrawCalibrationScreen(void)
{
    ...
    //{
        //color c = {0, 0, 0, 100};
        clear_screen_with_color(UI_COLOR_BLACK); /* clear screen with black color in FTE */
    //}
#else
 ...
 *  We should avoid overlapping of image and text
 *****************************************************************************/
U16 UpdateTestControlAreaScreen(
...
#if defined(DM_NEW_POPUPBG_METHOD) || defined(__MMI_FTE_SUPPORT__) || !defined(__GDI_MEMORY_PROFILE_2__)
/*****************************************************************************
 * FUNCTION
...
//        history_buffer);
	...
	#ifdef __GDI_MEMORY_PROFILE_2__
    MMI_multiline_inputbox.flags &= ~UI_MULTI_LINE_INPUT_BOX_DISABLE_BACKGROUND;
	#else/*#ifdef __GDI_MEMORY_PROFILE_2__*/
	MMI_multiline_inputbox.flags |= UI_MULTI_LINE_INPUT_BOX_DISABLE_BACKGROUND;
        ...
        )
{
    ...
    /*----------------------------------------------------------------*/
    set_small_screen();
#if 0//def __MMI_POPUP_NO_SHOW_ICON__
/* under construction !*/
#endif
	dm_data.s32flags = 0;
	if((left_softkey == 0)&&(right_softkey == 0))
...
#endif/*#ifndef __MMI_POPUP_NO_SHOW_ICON__ */
 ...
 *  void
 *****************************************************************************/
void ShowCategory366Screen_int_ex(
        UI_string_type title,
  //      PU8 title_icon,
        UI_string_type left_softkey,
        UI_string_type right_softkey,
...
//    dm_add_title(title, 0);//title_icon
//	dm_add_softkey_str(left_softkey, right_softkey);
 ...
 *  void
 *****************************************************************************/
void wgui_cat6003_show_progress(
	S32 x, 
	S32 y, 
	S32 width, 
	S32 height, 
 ...
 *  void
 *****************************************************************************/
void ShowCategory208Screen_int_ex(
        U16 title,
//        U16 title_icon,
        U16 left_softkey,
        U16 right_softkey,
...
#endif
    dm_reset_context();
    if (((title == 0)) || ((title == CAT66_AREA_NONE)) || is_BW)
    {
        ShowCategory165Screen_int_ex(
            left_softkey,
            right_softkey,
            (UI_string_type) message
#ifndef __MMI_POPUP_NO_SHOW_ICON__             
...
/* under construction !*/
 ...
 //   wgui_title_set_menu_shortcut_number(-1);
  ...
  //  clear_right_softkey();
    ...
    dm_add_title_id(title, 0);//title_icon
...
#endif
#if 0//(defined(__MMI_MAINLCD_320X240__) && defined(__MMI_FTE_SUPPORT__))
/* under construction !*/
/* under construction !*/
/* under construction !*/
#endif
...
//add by huking, for BT dialer special popup

static UI_string_type g_str_cat632_appname;
static UI_string_type g_str_cat632_title;
static UI_string_type g_str_cat632_datetime;

void ExitCategory632Screen(void)
{
  //
	g_str_cat632_datetime = NULL;
	g_str_cat632_appname = NULL;
	g_str_cat632_title = NULL;

	reset_small_screen();
}

static void DrawCat632ControlArea(dm_coordinates *coordinate)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
   // U16 str_appname[] = L"App name...";
	//U16 str_title[] = L"Title...";
    S32 str_width, str_height;
	S32 offset_x,offset_y;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
   offset_x = coordinate->s16X;
   offset_y = coordinate->s16Y;
   ...
   gui_set_font(&MMI_medium_font);//(UI_font_type)gui_font_get_type(4)
   gui_set_text_color(*(current_MMI_theme->list_normal_text_color));
   
   gui_measure_string((UI_string_type) g_str_cat632_appname, &str_width, &str_height);

	if (r2lMMIFlag)
	{
		gui_move_text_cursor(offset_x + str_width, offset_y);
		gui_print_truncated_text(offset_x + str_width, offset_y, coordinate->s16Width, (UI_string_type) g_str_cat632_appname);
	}
	else
	{
		gui_move_text_cursor(offset_x, offset_y);
		gui_print_truncated_text(offset_x, offset_y, coordinate->s16Width, (UI_string_type) g_str_cat632_appname);
	}
   
	//gui_print_text((UI_string_type) g_str_cat632_appname);
    //gui_print_truncated_text();

	offset_y =  str_height + offset_y + CAT632_APPNAME_TITLE_GAP;//3 line gap

	if (r2lMMIFlag)
	{
		gui_move_text_cursor(offset_x + str_width, offset_y);
		gui_print_truncated_text(offset_x + str_width, offset_y, coordinate->s16Width, (UI_string_type) g_str_cat632_title);
	}
	else
	{
		gui_move_text_cursor(offset_x, offset_y);
		gui_print_truncated_text(offset_x, offset_y, coordinate->s16Width, (UI_string_type) g_str_cat632_title);
	} 
   
   // gui_print_text((UI_string_type) g_str_cat632_title);

	gdi_layer_pop_clip();
}

static void DrawCat632ControlArea2(dm_coordinates *coordinate)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	//U16 str_datetime[] = L"2013/06/06 14:12:00";
    S32 str_width, str_height;
	S32 offset_x,offset_y;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
   ...
   gui_set_font(&MMI_small_font);//(UI_font_type)gui_font_get_type(4)
   gui_set_text_color(*(current_MMI_theme->list_normal_text_color));
   
   gui_measure_string((UI_string_type) g_str_cat632_datetime, &str_width, &str_height);//str_datetime

	if (r2lMMIFlag)
	{
		gui_move_text_cursor(offset_x + str_width, offset_y);
		gui_print_truncated_text(offset_x + str_width, offset_y, coordinate->s16Width, (UI_string_type) g_str_cat632_datetime);
	}
	else
	{
		gui_move_text_cursor(offset_x, offset_y);
		gui_print_truncated_text(offset_x, offset_y, coordinate->s16Width, (UI_string_type) g_str_cat632_datetime);
	}
...
//for bt dialer
void wgui_showcategory632_screen(
				...
				UI_string_type str_datetime)
{
	...
	g_str_cat632_appname = str_appname;
	g_str_cat632_title   = str_title;
    g_str_cat632_datetime = str_datetime;
    //just for test
	//str_message = L"特斯图发动发送大发送大发送段发送大发送大发送颠覆";//L"Content...XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX";
	ShowCommonCategoryPopupScreen(str_message);
    dm_add_softkey_str(left_softkey, right_softkey);
    
	MMI_multiline_inputbox.flags &= ~UI_MULTI_LINE_INPUT_BOX_CENTER_JUSTIFY;

    dm_register_category_controlled_callback(DrawCat632ControlArea);
    dm_register_category_controlled2_callback(DrawCat632ControlArea2);

	
	ExitCategoryFunction = ExitCategory632Screen;
    dm_setup_category_functions(dm_redraw_category_screen, dummy_get_history, dummy_get_history_size);
    dm_data.s32ScrId = (S32) GetActiveScreenId();
    dm_data.s32CatId = MMI_CAT632_ID;
#ifdef __MMI_MAINLCD_128X64__
	dm_data.s32flags |= DM_CLEAR_SCREEN_BACKGROUND_COLOR;
#else
    dm_data.s32flags = DM_SET_NEW_LAYER_ACTIVE;
...
//end for BT dialer special popup
