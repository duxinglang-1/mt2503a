...
MMI_BOOL gui_asyncdynamic_list_error = MMI_FALSE;
MMI_BOOL gui_asyncdynamic_list_loading_error = MMI_FALSE;
 ...
 *****************************************************************************/
//add by huking
static void gui_show_asyncdynamic_matrix_menu_bkg(
	fixed_matrix_menu *m,
	S32 x1,
	S32 y1,
	S32 x2,
...
void gui_asyncdynamic_list_menu_locate_previous_item(fixed_list_menu *m)
{
	...
	*/
    ...
    if (m->highlighted_item < m->first_displayed_item)
    {
		...
		gui_asyncdynamic_load_chunk_error(m->highlighted_item, 1);
		/*
		if (gui_asyncdynamic_list_error == 0 &&
...
static void gui_asyncdynamic_list_menu_locate_next_item_helper(fixed_list_menu *m, S32 old_first_displayed_item)
{
	...
	{
		gui_asyncdynamic_load_chunk_error(m->first_displayed_item, old_first_displayed_item - m->first_displayed_item);
		/*
	 if (gui_asyncdynamic_list_error == 0 &&
	...
	gui_asyncdynamic_load_chunk_error(m->highlighted_item, 2);	
	 /*
	 if (gui_asyncdynamic_list_error == 0 &&
 ...
 *  void
 *****************************************************************************/
void gui_asyncdynamic_menu_switch_highlighted_item(void*m1, S32 last_highlighted_item)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        if (gui_is_index_in_menu_valid_region(last_highlighted_item, m1))//((last_highlighted_item >= 0) && (last_highlighted_item < m->n_items))
        {
        ...
        if (gui_is_index_in_menu_valid_region(m->highlighted_item, m1))//((m->highlighted_item >= 0) && (m->highlighted_item < m->n_items))
        {
...
#if defined(__SOCIAL_NETWORK_SUPPORT__) || defined(__MMI_UM_CONVERSATION_BOX_ENHANCE__)
/*****************************************************************************
 * FUNCTION
...
#endif/*defined(__SOCIAL_NETWORK_SUPPORT__) || defined(__MMI_UM_CONVERSATION_BOX_ENHANCE__) */		
...
void gui_matrix_menu_locate_x(fixed_matrix_menu *m,  S32 locate_type)
{
	...
	*/
	...
	{
		gui_asyncdynamic_load_chunk_error(m->first_displayed_row * m->n_columns, m->n_columns);
	/*
	      if (gui_asyncdynamic_list_error == 0 &&
	        !load_chunk_asyncdynamic_item_buffer
...
void gui_show_asyncdynamic_matrix_menu_specific_items(fixed_matrix_menu *m, S32 last_highlight)
{
  ...
  //  UI_filled_area *f;
   ...
   // S32 cx1, cy1, cx2, cy2;
   // S32 tx1, ty1, tx2, ty2;
    S32 ix, iy,  ix2, iy2;//iwidth, iheight,
 //   U8 show_vbar = 0, show_hbar = 0;
   ...
   //    return;
	...
	if(m->n_items <= 0
#ifdef  __MMI_SUPPORT_ASYNCDYNAMIC_MATRIX_MENU__		
		|| matrix_type == GUI_SHOW_ASYNC_MENU && gui_asyncdynamic_list_error
#endif/*__MMI_SUPPORT_ASYNCDYNAMIC_MATRIX_MENU__*/		
		)//asyncdynamic matrix
	{
...
/* under construction !*/
  ...
  //asyncdynamic
#ifdef  __MMI_SUPPORT_ASYNCDYNAMIC_MATRIX_MENU__	  
	if((matrix_type == GUI_SHOW_ASYNC_MENU)||
		(matrix_type == GUI_SHOW_FIXED_MENU))
#endif/*__MMI_SUPPORT_ASYNCDYNAMIC_MATRIX_MENU__*/
	//{
  		gui_show_asyncdynamic_matrix_menu_bkg(m, x1, y1, x2, y2);
#if 0  
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
//cacul x,y gap between matrix items
...
//fixed matrix
   ...
   if(matrix_type == GUI_SHOW_FIXED_MENU)
   	{
#endif/*__MMI_SUPPORT_ASYNCDYNAMIC_MATRIX_MENU__*/   	
		yoff_gap = (height - (m->item_height * m->displayed_rows)) % (m->displayed_rows + 1);
	    if (yoff_gap)
	    {
	        m->height -= yoff_gap;
	    }
    ...
    for (j = 0; j < m->displayed_rows; j++)
    {
        ...
        for (i = 0; i < m->displayed_columns; i++)
        {
            ...
            ix2 = ix + m->item_width - 1;
            iy2 = iy + m->item_height - 1;
            if (ix2 > x2)
            {
                ix2 = x2;
            }
            ...
            //gdi_layer_push_clip();
            gui_push_and_set_clip(ix, iy, ix2, iy2);
            ...
            if(matrix_type == GUI_SHOW_FIXED_MENU)
            {
	            ...
	            if ((MMI_current_menu_type == LIST_MATRIX_MENU) || (((MMI_current_menu_type == MATRIX_MENU)) 
...
/* under construction !*/
						#endif
	                    gdi_layer_push_and_set_clip(ix, iy, ix + m->item_width - 1, iy + m->item_height - 1); 
			...
			else
			{
	            ...
	            {
	            // MAUI_03490040 gui_show_asyncdynamic_matrix_menu_bkg(m, x1, y1, x2, y2);
					gui_show_asyncdynamic_matrix_menu_bkg(m, ix, iy, ix2, iy2);  
	            	#if 0
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
#endif/*__MMI_SUPPORT_ASYNCDYNAMIC_MATRIX_MENU__*/				
            gdi_layer_pop_clip();
            ix += m->item_width + xoff;
        }
        iy += m->item_height + yoff;
    }
...
#ifdef  __MMI_SUPPORT_ASYNCDYNAMIC_MATRIX_MENU__		
		 && ((matrix_type == GUI_SHOW_FIXED_MENU) 		
			||(matrix_type == GUI_SHOW_ASYNC_MENU))
#endif/*__MMI_SUPPORT_ASYNCDYNAMIC_MATRIX_MENU__*/						
			)
...
#ifdef  __MMI_SUPPORT_ASYNCDYNAMIC_MATRIX_MENU__	
#ifdef __MMI_HORIZONTAL_SCROLL_BAR_SUPPORT__    	
	if((matrix_type == GUI_SHOW_FIXED_MENU)&& gui_is_matrix_menu_show_hbar(m->flags))
...
static void gui_asyncdyamic_matrix_menu_scroll_timer_hdlr(void)
{
    ...
    if (!m || gui_asyncdynamic_list_error)
    {
        return;
    }
/*
 ...
 *****************************************************************************/
MMI_BOOL gui_asyncdynamic_list_menu_translate_pen_position(
	fixed_list_menu *m, 
	S32 x, 
	S32 y, 
	S32 *item_index)
 ...
 *  void
 *****************************************************************************/
static void gui_asyncdymaic_list_menu_get_menuitem_position(
	fixed_list_menu *m, 
	S32 item_index, 
	S32 *x, 
	S32 *y)
        ...
        gui_matrix_pen_enum *menu_event)
{
    ...
    *menu_event = GUI_MATRIX_PEN_NONE;
    ...
    if (m->flags & UI_MATRIX_MENU_DISABLE_PEN || gui_asyncdynamic_list_error)
    {
        return MMI_FALSE;
    }
/*
...
/*****************************************************************************
 * FUNCTION
 *  gui_show_asyncdynamic_matrix_menu_bkg
 * DESCRIPTION
 *  Display the asyncdynamic matrix menu.
 * PARAMETERS
 *  m       [IN]        fixed matrix menu object
	...
	S32 y2)
{
	...
	UI_filled_area *f = m->normal_filler;
