 ...
 *  void
 *****************************************************************************/
void cat251_scrolling_text_draw_background(
		S32 x1, 
		S32 y1, 
		S32 x2, 
		S32 y2)
 ...
 *  void
 *****************************************************************************/
void cat251_scrolling_text_draw_backgroundA(
		S32 x1, 
		S32 y1, 
		S32 x2, 
		S32 y2)
...
void DrawCategory251InfoBox(U8 *info_string1, U8 *info_string2)
{
...
#if defined(__MMI_MAINLCD_128X128__)  
    c = UI_COLOR_WHITE;//gui_color(255, 255, 255);
    gui_fill_rectangle(
        0,
        (below_title),
        (UI_device_width - 1),
    ...
    c = UI_COLOR_BLACK;//gui_color(0, 0, 0);
    ...
    else
    {
        ...
        gui_create_scrolling_text(
			...
			cat251_scrolling_text_draw_background, 
			c, 
			UI_COLOR_WHITE);//gui_color(255, 255, 255));  /* 120204 Calvin modified for calculate the height of the info box */
        gui_show_scrolling_text(&scroll_text_cat251);
    }

    gui_measure_string(
		(UI_string_type) info_string2, 
		&w1, 
		&h1);
    ...
    *category_error_flag = 1;
    ...
    if(no_of_icons == 1)
    {
        if(mmi_cat263_display_style == MMI_CAT261_DISPLAY_THREE_LINE)
        {
            ...
            set_fixed_icontext_list_text_coordinates(
				0, 
				(3 + w), 
				((MMI_MULTIROW_MENUITEM_HEIGHT >> 1) - font_height_m) >> 1,
				text_width, 
				font_height_m);
            set_fixed_icontext_list_text_coordinates(
				1, 
				(3 + w), 
				(((MMI_MULTIROW_MENUITEM_HEIGHT >> 1) - font_height_m) >> 1) + (MMI_MULTIROW_MENUITEM_HEIGHT >> 1), 
				text_width, 
				font_height_m);
        }
    ...
    else
    {
        ...
        set_fixed_icontext_list_text_coordinates(
			1, 
			(3 + w), 
			font_height_m + 2*WGUI_CAT261_GAP, 
			text_width, 
			font_height_s);
        set_fixed_icontext_list_icon_coordinates(
			1, 
			(3 + w), 
			font_height_m + font_height_s + 3*WGUI_CAT261_GAP, 
			w1, 
			h1);
        set_fixed_icontext_list_icon_coordinates(
			2, 
    ...
    MMI_fixed_icontext_list_menuitem.flags |= 
    	UI_MENUITEM_RIGHT_JUSTIFY | 
    	UI_MENUITEM_ICONTEXT_LIST_ALL_TEXT_VERTICAL_ALIGN_CENTER |
    	UI_MENUITEM_AUTO_CHOOSE_MARQUEE;
...
#endif/*#if defined(__MMI_EMAIL__) */
...
void DrawCate263CategoryControlArea(dm_coordinates *coordinate)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (!gui_asyncdynamic_list_error && !gui_is_fixed_list_menu_empty())//MMI_fixed_list_menu.n_items > 0)
    {
        DrawCategory251InfoBox(
			category_251_info_text1, 
			category_251_info_text2);
    }
}
...
static void mmi_cat263_set_icontext_list_ext(
				U8 t1, 
				U8 t2, 
				S32 iw, 
				S32 tw, 
...
/* under construction !*/
    ...
    set_fixed_icontext_list_icon_coordinates(
    	...
    	WGUI_CAT263_ICON_GAP_Y, 
    	iw, 
    	MMI_MULTIROW_MENUITEM_HEIGHT - 6);
...
*  void
*****************************************************************************/
static void wgui_cat263_set_display_all(
				S32 text_x, 
				S32 text_width, 
				S32 icon_width, 
				S32 icon1_w, 
...
/* under construction !*/
    ...
    set_fixed_icontext_list_text_coordinates(
    	...
    	0, 
    	text_name_width, 
    	(MMI_MULTIROW_MENUITEM_HEIGHT>>1));   
    set_fixed_icontext_list_text_coordinates(
		1, 
		time_x, 
		0, 
    ...
    MMI_fixed_icontext_list_menuitem.flags |= 
		...
		UI_MENUITEM_AUTO_CHOOSE_MARQUEE;
    MMI_fixed_icontext_list_menuitem.flags |= 
		UI_MENUITEM_RIGHT_JUSTIFY_COL2;
    MMI_fixed_icontext_list_menuitem.flags &= ~(UI_MENUITEM_CENTER_ICON_Y);
...
*  void
*****************************************************************************/
static void wgui_cat263_set_display_none(
				S32 text_x, 
				S32 cb_w, 
				S32 icon2_x, 
				S32 icon_w, 
				...
				S32 list_height)
{
    ...
    set_fixed_icontext_list_icon_coordinates(
    	0, 
    	GUI_ICONTEXT_MENUITEM_ICON_X + cb_w, 
    	0, 
    	icon_w, 
    	MMI_MENUITEM_HEIGHT);
...
*  void
*****************************************************************************/
static void wgui_cat263_create_icontext_menu(
				S32 n_items, 
				S32 n_icons, 
				GetAsyncItemFuncPtr get_item_func, 
				GetAsyncHintFuncPtr get_hint_func)
...
/* under construction !*/
        ...
        set_fixed_icontext_list_text_coordinates(
        	2, 
        	text_x, 
        	WGUI_CAT263_TEXT_GAP_Y, 
        	text_width, 
        	font_height_m);   
        set_fixed_icontext_list_text_coordinates(
			1, 
			text_x, 
			font_height_m + WGUI_CAT263_TEXT_GAP_Y, 
			text_width, 
			font_height_m);
        set_fixed_icontext_list_text_coordinates(
			0, 
			text_x, 
			font_height_m*2 + WGUI_CAT263_TEXT_GAP_Y, 
			text_width, 
			font_height_s);
        MMI_fixed_icontext_list_menuitem.flags &= ~(UI_MENUITEM_CENTER_TEXT_Y);
        ...
        set_fixed_icontext_list_icon_coordinates(
        	0, 
        	GUI_ICONTEXT_MENUITEM_ICON_X, /*3*/
        	0, 
        	w, 
 ...
 *  void
 *****************************************************************************/
static void wgui_cat267_scrolling_text_draw_background(
					S32 x1, 
					S32 y1, 
					S32 x2, 
					S32 y2)
...
/* under construction !*/
    ...
    case MMI_CAT263_DISPLAY_NONE:
        ...
        set_fixed_icontext_list_icon_coordinates(
			0, 
			GUI_ICONTEXT_MENUITEM_ICON_X + check_box_width + 2,
			WGUI_CAT263_ICON_GAP_Y, 
			w, 
			h);
#ifdef __GEMINI__
...
//huking	
        ...
        set_fixed_icontext_list_text_coordinates(
			0, 
			GUI_ICONTEXT_MENUITEM_TEXT_X + MMI_ICONTEXT_MENUITEM_HEIGHT + 3,
			0, 
			text_width, 
			MMI_MENUITEM_HEIGHT);
        if(n_tabs > 0)
...
#endif/*#if defined(__MMI_EMAIL__) || defined(__UNIFIED_MESSAGE_MARK_SEVERAL_SUPPORT__)*/
...
//huking	
    ...
    *category_error_flag = 1;
    ...
    set_fixed_icontext_list_text_coordinates(
		...
		0, 
		text_width, 
		MMI_MENUITEM_HEIGHT);   /* address */
    set_fixed_icontext_list_text_coordinates(
		0, 
		GUI_ICONTEXT_MENUITEM_TEXT_X, 
		MMI_MENUITEM_HEIGHT, 
		text_width, 
		MMI_MENUITEM_HEIGHT); /* subject */
    set_fixed_icontext_list_text_coordinates(
		1, 
...
/* under construction !*/
    ...
    set_fixed_icontext_list_text_coordinates(
    	2, 
    	x + MMI_ICONTEXT_MENUITEM_HEIGHT, 
    	0, 
    	text_width, 
    	MMI_MENUITEM_HEIGHT);
    set_fixed_icontext_list_text_coordinates(
