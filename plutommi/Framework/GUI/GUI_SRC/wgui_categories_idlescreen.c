...
#ifdef __MMI_BT_DIALER_SUPPORT__
WCHAR idle_screen_BT_dialer_paring_name[SRV_BT_CM_BD_FNAME_LEN];
WCHAR pairing_name_output_buf[SRV_BT_CM_BD_FNAME_LEN + 20];
MMI_BOOL idle_screen_BT_dialer_connected = MMI_FALSE;  /* idle screen BT dialer connection status */
...
#if defined(__MMI_TOUCH_IDLESCREEN_SHORTCUTS__) && defined(__UM_SUPPORT__)
...
void SetIdleScreenBTDialerPairingName(const WCHAR *s)
{
    ...
    mmi_wcscpy(idle_screen_BT_dialer_paring_name, s);
}
...
void SetIdleScreenBTDialerConnectionStatus(MMI_BOOL status)
{
	...
	/* Code Body													  */
	/*----------------------------------------------------------------*/
	idle_screen_BT_dialer_connected = status;
}

MMI_BOOL GetIdleScreenBTDialerConnectionStatus()
{
    return idle_screen_BT_dialer_connected;
}
...
/* under construction !*/
    ...
    gdi_layer_set_clip(0, y, UI_device_width - w_inset, y + max_str_h - 1);
    gdi_draw_solid_rect(0, y, UI_device_width - w_inset, y + max_str_h - 1, GDI_COLOR_TRANSPARENT);
    
    if (idle_screen_BT_dialer_connected && idle_screen_BT_dialer_paring_name != NULL)
    {
        mmi_wcscpy(pairing_name_output_buf, idle_screen_BT_dialer_paring_name);
        mmi_wcscat(pairing_name_output_buf, (const WCHAR*)GetString(STR_BT_CONN_POP));
    }
    else
    {
        gdi_layer_pop_clip();
    ...
    real_width = gui_get_string_width(pairing_name_output_buf);
    ...
    else  //static
    {
        ...
        gui_move_text_cursor(x, y + 1);
        gui_print_bordered_text(pairing_name_output_buf);
    }
