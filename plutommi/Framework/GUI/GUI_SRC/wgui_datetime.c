...
static void dt_show_duration2(void);
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void increment_seconds_2(UI_time *t)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    DTGetRTCTime(&curr_time);
    get_time_difference2(&curr_time, &last_duration2_time, &diff_time);
    IncrementTime(*t, diff_time, t );
    last_duration2_time.nSec = curr_time.nSec;
    last_duration2_time.nMin = curr_time.nMin;
    last_duration2_time.nHour = curr_time.nHour;
    last_duration2_time.nDay = curr_time.nDay;
    last_duration2_time.nMonth = curr_time.nMonth;
    last_duration2_time.DayIndex = curr_time.DayIndex;
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void increment_ticks_2(UI_time *t)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    kal_get_time(&current_time);
    secs = (U32) ((current_time - last_duration2_ticks + last_duration2) * 0.004615 + 0.5);
    mins = secs / 60;
    hrs = mins / 60;
    t->nSec = (U8) (secs - mins * 60);
    t->nMin = (U8) (mins - hrs * 60);
...
void show_main_LCD_date_time(void)
{
    ...
    switch (main_LCD_dt_object.flags & DT_FLAGS_TYPE)
    {
        ...
        case DT_NETWORK_SEARCH:
        {
            ...
            gui_measure_string(s, &w, &h);
            x = (UI_device_width >> 1) - (w >> 1);
        #if defined(__MMI_MAINLCD_128X128__)//060606 network search Calvin
        ...
        case DT_TABBED_CALL_SCREEN_2:
        {
            ...
            DTGetRTCTime(&curr_time);
            start_time = &main_LCD_time_object.duration_start_time2;
            ...
            GetTimeDifference(&curr_time, start_time, &duration);
            memcpy(&main_LCD_dt_object.duration2.t, &duration, sizeof(MYTIME));
        ...
        #endif
            {
				x1 = main_LCD_dt_object.duration2.x;
                y1 = main_LCD_dt_object.duration2.y;
				gui_set_font(main_LCD_dt_object.duration2.font);
				duration_string(&main_LCD_dt_object.duration2.t, (UI_string_type) s, main_LCD_dt_object.flags);
				gui_measure_string(s, &w, &h);
				main_LCD_dt_object.duration2.width = (UI_device_width >> 1) + 16;
                main_LCD_dt_object.duration2.height = h;
				x2 = main_LCD_dt_object.duration2.width + x1 - 1;
				y2 = main_LCD_dt_object.duration2.height + y1 - 1;
                if (enable_main_LCD_date_time_display)
				{
                    gui_set_text_color(main_LCD_dt_object.duration2.text_color);
        ...
        case DT_ANALOG_CLOCK_SC:
        {
            ...
            #elif defined(__MMI_BT_BOX_IDLE_SHORTCUTS__)
                gdi_layer_blt_previous(
                    0,
                    (MMI_title_y + MMI_title_height),
                    (UI_device_width - 1),
...
void hide_main_LCD_date_time(void)
{
    ...
    switch (main_LCD_dt_object.flags & DT_FLAGS_TYPE)
    {
        ...
        case DT_MT_CALL_SCREEN:
        {
            ...
            if (y1 > main_LCD_dt_object.time.y)
            {
                y1 = main_LCD_dt_object.time.y;
            }
            ...
            if (y2 < main_LCD_dt_object.time.y + main_LCD_dt_object.time.height)
            {
                y2 = main_LCD_dt_object.time.y + main_LCD_dt_object.time.height - 1;
            }
        ...
        case DT_TABBED_CALL_SCREEN_2:
        {
        ...
        #endif
            {
                ...
                x2 = main_LCD_dt_object.duration2.width + x1 - 1;
                y2 = main_LCD_dt_object.duration2.height + y1 - 1;
                main_LCD_dt_object.duration2.hide_function(x1, y1, x2, y2);
            }
        }
        break;
	#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void set_dt_duration_2(UI_time *t)
{
#ifndef __MMI_DATETIME_SLIM__
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    main_LCD_dt_object.duration2.t = *t; /* set the duration value */
    DTGetRTCTime(&last_duration2_time);
#endif
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void main_LCD_measure_time_duration2_string(S32 *w, S32 *h)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    UI_character_type s[64];
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    gui_set_font(main_LCD_time_object.duration2.font);
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void (*MMI_main_LCD_dt_duration2_hide_function) (S32 x1, S32 y1, S32 x2, S32 y2) = UI_dummy_hide_function;
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void set_main_LCD_dt_duration2_hide_function(void (*f) (S32 x1, S32 y1, S32 x2, S32 y2))
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_main_LCD_dt_duration2_hide_function = f;
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
 ...
 *  void
 *****************************************************************************/
static void dt_show_duration_internal(UI_date_time duration)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    x1 = duration.x;
    y1 = duration.y;
    x2 = duration.width + x1 - 1;
    y2 = duration.height + y1 - 1;
    if (duration.hide_function)
    {
        duration.hide_function(x1, y1, x2, y2);  /* call hide function of call duration */
    }

    gui_set_font(duration.font);
    /* get call duration in string fromat */
    duration_string(&duration.t, (UI_string_type) s, main_LCD_dt_object.flags);
    gui_set_text_color(duration.text_color);
...
static void dt_show_duration(void)
{
	dt_show_duration_internal(main_LCD_dt_object.duration);
...
static void dt_show_duration2(void)
{
	dt_show_duration_internal(main_LCD_dt_object.duration2);
}

void dt_show_two_duration(UI_date_time duration,UI_date_time duration2)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    x3 = duration2.x;
    y3 = duration2.y;
    x4 = duration2.width + x3 - 1;
    y4 = duration2.height + y3 - 1;
    if (duration2.hide_function)
    {
        duration2.hide_function(x3, y3, x4, y4);  /* call hide function of call duration */
    }

    gui_set_font(duration2.font);
    /* get call duration in string fromat */
    duration_string(&duration2.t, (UI_string_type) s2, main_LCD_dt_object.flags);
    gui_set_text_color(duration2.text_color);
    w2 = gui_get_string_width(s2);

    if (mmi_fe_get_r2l_state())
    {
        gui_move_text_cursor(x3 + w2 - 1, y3);
    }
    else
    {
        gui_move_text_cursor(x3, y3);
    }

    gui_set_text_clip(x3, y3, x4, y4);
    /* print call duration */
    gui_print_text(s2);
    gdi_layer_unlock_frame_buffer();
    gdi_layer_blt_previous(x1, y1, x2, y2);	
    gdi_layer_blt_previous(x3, y3, x4, y4);//need check by Datetime owner	
}
#endif
...
void dt_update_duration2(void)
{
    ...
    gui_start_timer(1000, dt_update_duration2);
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
void set_dt_display(U32 flags)
{
    ...
    switch (flags & DT_FLAGS_TYPE)
    {
    ...
    #endif
        {
            ...
            switch (g_clock_type)
            {
                ...
                /* in techno bar, we use image instead of text */
                    ...
                    y = UI_device_height - wgui_cat033_get_shortcut_max_height() - (h + (DT_DISPLAY_GAP << 1)) - 4 - MMI_BUTTON_BAR_HEIGHT;
                #else
                ...
                #if !defined(__MMI_TELEPHONY_SUPPORT__) && defined(__MMI_MAINLCD_176X220__)
                    /* when support bt dialer, not show network info, move datetime on center */
                    y -= 70;
                #endif
        ...
        case DT_IDLE_SCREEN_2:
        {
            ...
            y = DT_DISPLAY_OFFSET_IN_STATUS_BAR_AT_BOTTOM;
        #elif defined(__MMI_BT_BOX_IDLE_SHORTCUTS__)
            /* UI_device_height - short_hight - gap */
            y = 135; //UI_device_height - wgui_cat033_get_shortcut_max_height() - h - (DT_DISPLAY_GAP << 1);
        #else 
            y = UI_device_height - MMI_button_bar_height - h - DT_SOFTKEY_GAP - 1;
        #endif 
    ...
    //#ifndef __MMI_CM_SCREEN_HIDE_DATE_TIME__
		...
		case DT_TABBED_CALL_SCREEN_2:
		{
            ...
            main_LCD_dt_object.duration2.hide_function = MMI_main_LCD_dt_duration2_hide_function;  
            main_LCD_dt_object.duration2.font = gui_font_get_type(GUI_FONT_SIZE_DURATION);
            main_LCD_dt_object.duration2.text_color = *current_MMI_theme->datetime_bar_duration_text_color;
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
	MMI_main_LCD_dt_duration2_hide_function = UI_dummy_hide_function;
	gui_cancel_timer(dt_update_duration2);
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
    gui_cancel_timer(dt_update_duration);
#ifndef __MMI_DATETIME_SLIM__
...
static general_input_show_background(general_input *input)
{
    ...
    else
    {
		...
		if(date_time_input_background_filler.c.r)
		{
             ...
             void (*general_input_key_handler)(S32 vkey_code, S32 key_state))
{
    ...
    SetKeyDownHandler(general_input_previous_character, KEY_LEFT_ARROW);
    SetKeyDownHandler(general_input_next_character, KEY_RIGHT_ARROW);
    SetKeyLongpressHandler(general_input_previous_character, KEY_LEFT_ARROW);
    SetKeyLongpressHandler(general_input_next_character, KEY_RIGHT_ARROW);
    SetKeyRepeatHandler(general_input_previous_character, KEY_LEFT_ARROW);
    SetKeyRepeatHandler(general_input_next_character, KEY_RIGHT_ARROW);
...
void time_input_set_seperator(time_input *d, UI_character_type seperator)
{
    ...
    {// for all languages baseline large then english font's
        mmi_fe_get_string_info_param_struct query;
        S32 text_height_simply = 0, text_ascent_simply = 0, text_descent_simply = 0;

        mmi_fe_get_char_info_of_simply_lang(box1->text_font->size, &text_height_simply, &(text_ascent_simply), &text_descent_simply); 
        memset(&query, 0, sizeof(mmi_fe_get_string_info_param_struct));
        query.String = (U8*)&d->seperator;
        query.n = 1;
        ...
        {
            d->s1_y =   text_ascent_simply - query.baseline;
        }
...
#endif /* __MMI_DATETIME_USE_MULTI_LINE__ */
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void (*MMI_main_LCD_time_duration2_hide_function) (S32 x1, S32 y1, S32 x2, S32 y2) = UI_dummy_hide_function;
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void set_main_LCD_time_duration2_hide_function(void (*f) (S32 x1, S32 y1, S32 x2, S32 y2))
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_main_LCD_time_duration2_hide_function = f;
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void set_time_duration_2(UI_time *t, U32 duration)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    main_LCD_time_object.duration2.t = *t;   /* set the duration value */
    kal_get_time(&last_duration2_ticks);
    last_duration2 = duration;
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void set_duration2_start_time(UI_time *t)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    main_LCD_time_object.duration_start_time2 = *t;   /* set the duration value */
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void get_time_duration_2(UI_time *t)
{
#ifndef __MMI_DATETIME_SLIM__
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    *t = main_LCD_time_object.duration2.t;   /* get the duration value */
#endif
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
 ...
 *  void
 *****************************************************************************/
static void time_display_duration_internal(UI_date_time duration)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    gui_set_font(duration.font);
    /* get call duration in string fromat */
    get_duration_string(&duration.t, (UI_string_type) s, main_LCD_time_object.flags);
    gui_set_text_color(duration.text_color);
    w = gui_get_string_width(s);
    ...
    if (mmi_fe_get_r2l_state())
    {
        ...
        gui_move_text_cursor(x1+((duration.width-w)>>1) + w - 1, y1);
    }
    else
    {
        //gui_move_text_cursor(x1, y1);
        gui_move_text_cursor(x1+((duration.width-w)>>1), y1);
    }
...
{
	time_display_duration_internal(main_LCD_time_object.duration);
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void time_display_duration2(void)
{
	time_display_duration_internal(main_LCD_time_object.duration2);
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void time_update_duration2(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    increment_ticks_2(&main_LCD_time_object.duration2.t);  /* increment secods by 1 */
    time_display_duration2();
    gui_start_timer(300, time_update_duration2);
}
#endif /*__MMI_UCM_BT_CALL_RECORDER__*/
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void set_time_display_2(U32 flags, S32 x, S32 y, S32 width, S32 height)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    main_LCD_time_object.flags = (flags & DT_FLAGS_TYPE);
    main_LCD_time_object.duration2.hide_function = MMI_main_LCD_time_duration2_hide_function;
    main_LCD_time_object.duration2.font = &MMI_large_font;
	main_LCD_time_object.duration2.text_color = *current_MMI_theme->datetime_bar_duration_text_color;
    main_LCD_time_object.duration2.x = x;
    main_LCD_time_object.duration2.y = y;
    main_LCD_time_object.duration2.width = width;
    main_LCD_time_object.duration2.height = height;
    gui_cancel_timer(time_update_duration2);
    if (flags != 0)
    {
        gui_start_timer(300, time_update_duration2);
    }
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void close_main_LCD_time_display_2(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    gui_cancel_timer(time_update_duration2);
    MMI_main_LCD_time_duration2_hide_function = UI_dummy_hide_function;
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void set_main_lcd_duration2_position(U16 x, U16 y)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    main_LCD_dt_object.duration2.x = x;
    main_LCD_dt_object.duration2.y = y;
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
 ...
 *  void
 *****************************************************************************/
static void wgui_datetime_measure_duration_internal(S32 *w, UI_date_time duration)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
{
	wgui_datetime_measure_duration_internal(w ,main_LCD_dt_object.duration);
#if 0
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
#ifdef __MMI_UCM_BT_CALL_RECORDER__
void wgui_datetime_measure_duration2(S32 *w)
{
	wgui_datetime_measure_duration_internal(w ,main_LCD_dt_object.duration2);
}
#endif /* __MMI_UCM_BT_CALL_RECORDER__ */
...
/*****************************************************************************
 * datetime use multi line for input or show info.
 * date, time, IP4, time duration.
 *****************************************************************************/
...
/* Theme data for formatted input objects. Should be obtained from theme resource   */
/* date time input box background color scheme */
UI_filled_area datetime_input_background_filler = {
...
/* date time input box field color scheme */
UI_filled_area datetime_input_field_filler = {
    UI_FILLED_AREA_TYPE_COLOR,
    UI_NULL_IMAGE,
    NULL,
    {255, 255, 255, 100},
...
UI_multi_line_input_box_theme datetime_input_theme = { 
    &datetime_input_field_filler,
    &datetime_input_field_filler,
    &datetime_input_field_filler,
    {0, 0, 0, 100},      //special_text_color;
    {0, 0, 0, 100},      // normal_text_color;
    {128, 128, 128, 100},//disabled_text_color;
    {0, 0, 0, 100},       //selected_text_color;
    {0, 0, 0, 100},  //{255, 255, 255, 100}, // {0, 0, 0, 100},          // //selection_text_color;
    {51, 88, 171, 100},  //{255, 255, 255, 100},    //  //selection_color;
    {0, 0, 0, 100},       //boundary_color;
    {255, 255, 255, 100}, //{255, 0, 0, 100},     //cursor_color;
    &UI_DEFAULT_FONT,
    0,
    0, 
    '*',
...
/* store the value of current date input */
datetime_input *current_datetime_input = NULL;
...
Returns:       none
----------------------------------------------------------------------------*/
void (*current_datetime_input_callback) (void) = UI_dummy_function;
 ...
 *  datetime_get_length
 * DESCRIPTION
 *  get datetime string max length.
 * PARAMETERS
 *  type                  [IN]        datetime input type
 * RETURNS
 *  U16                    length of related type
 *****************************************************************************/
U16 datetime_get_length(datetime_input_type_enum type)
{
    switch(type)
    {
        case DATETIME_INPUT_TYPE_DATE:
            return DATETIME_INPUT_DATE_LENGTH;
            
        case DATETIME_INPUT_TYPE_TIME:
            return DATETIME_INPUT_TIME_LENGTH;   
            
        case DATETIME_INPUT_TYPE_IP4:
            return DATETIME_INPUT_IP4_LENGTH;
            
    #ifdef __DATETIME_MULTI_LINE_SUPPORT_DURATION__            
        case DATETIME_INPUT_TYPE_DURATION:
            return DATETIME_INPUT_DURATION_LENGTH;
    #endif
 ...
 *****************************************************************************/
 //  将buffer 根据 type 拆分
void datetime_get_detail()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U8 text[DATETIME_INPUT_BUF_TEXT_LENGTH * ENCODING_LENGTH];
    ...
    switch(current_datetime_input->type)
    {
        case DATETIME_INPUT_TYPE_DATE:
        {
            wcsncpy((wchar_t *)&text, (wchar_t *)current_datetime_input->text, DATETIME_INPUT_DATE_LENGTH);

            switch (current_datetime_input->detail.date.date_type)// & DATE_INPUT_TYPE_MASK)
            {
            case DATE_INPUT_TYPE_DD_MM_YYYY:
                wcsncpy((wchar_t *)current_datetime_input->detail.date.day, (wchar_t *)&text[0], 2);           
                wcsncpy((wchar_t *)current_datetime_input->detail.date.month, (wchar_t *)&text[6], 2);
                wcsncpy((wchar_t *)current_datetime_input->detail.date.year, (wchar_t *)&text[12], 4);   
                break;
            
            case DATE_INPUT_TYPE_MM_DD_YYYY:
                wcsncpy((wchar_t *)current_datetime_input->detail.date.month, (wchar_t *)&text[0], 2);           
                wcsncpy((wchar_t *)current_datetime_input->detail.date.day, (wchar_t *)&text[6], 2);
                wcsncpy((wchar_t *)current_datetime_input->detail.date.year, (wchar_t *)&text[12], 4);   
                break;
            
            case DATE_INPUT_TYPE_YYYY_MM_DD:
                wcsncpy((wchar_t *)current_datetime_input->detail.date.year, (wchar_t *)&text[0], 4);           
                wcsncpy((wchar_t *)current_datetime_input->detail.date.month, (wchar_t *)&text[10], 2);
                wcsncpy((wchar_t *)current_datetime_input->detail.date.day, (wchar_t *)&text[16], 2);   
                break;
            default:
                break;
            }
        ...
        case DATETIME_INPUT_TYPE_TIME:
        {            
            wcsncpy((wchar_t *)&text, (wchar_t *)current_datetime_input->text, DATETIME_INPUT_TIME_LENGTH);            
            
            wcsncpy((wchar_t *)current_datetime_input->detail.time.hour, (wchar_t *)&text[0], 2);           
            wcsncpy((wchar_t *)current_datetime_input->detail.time.minute, (wchar_t *)&text[6], 2);
            break;
        }
        case DATETIME_INPUT_TYPE_IP4:
        {
            wcsncpy((wchar_t *)&text, (wchar_t *)current_datetime_input->text, DATETIME_INPUT_IP4_LENGTH);            
            
            wcsncpy((wchar_t *)current_datetime_input->detail.ip4.IP1, (wchar_t *)&text[0], 3);        
            wcsncpy((wchar_t *)current_datetime_input->detail.ip4.IP2, (wchar_t *)&text[8], 3);           
            wcsncpy((wchar_t *)current_datetime_input->detail.ip4.IP3, (wchar_t *)&text[16], 3);        
            wcsncpy((wchar_t *)current_datetime_input->detail.ip4.IP4, (wchar_t *)&text[24], 3);   
            break;
        }
        ...
        case DATETIME_INPUT_TYPE_DURATION:
        {
            wcsncpy((wchar_t *)&text, (wchar_t *)current_datetime_input->text, DATETIME_INPUT_DURATION_LENGTH);            
            
            wcsncpy((wchar_t *)current_datetime_input->detail.duration.hour, (wchar_t *)&text[0], 2);           
            wcsncpy((wchar_t *)current_datetime_input->detail.duration.minute, (wchar_t *)&text[6], 2);
            wcsncpy((wchar_t *)current_datetime_input->detail.duration.second, (wchar_t *)&text[12], 2);
            break;
        }
    #endif
        default:
...
/*****************************************************************************
 * FUNCTION
 *  default_inline_date_validation
 * DESCRIPTION
 *  validation function of date input of inline editing
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void default_inline_date_validation()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    Date *date = &current_datetime_input->detail.date;
    
    S32 y = gui_atoi((UI_string_type) date->year);
    S32 y_l = gui_strlen((UI_string_type) date->year);
    S32 m = gui_atoi((UI_string_type) date->month);
    S32 m_l = gui_strlen((UI_string_type) date->month);
    S32 d = gui_atoi((UI_string_type) date->day);
    S32 d_l = gui_strlen((UI_string_type) date->day);

    U8 day = 0;
    BOOL isInvalid = MMI_FALSE;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/

    //year
    if ((y < g_date_year_min) && (y_l > 3))
    {
        y = g_date_year_min;
        isInvalid = MMI_TRUE;
    }
    else if (y > g_date_year_max)
    {
        y = g_date_year_max;
        isInvalid = MMI_TRUE;
    } 
    if(isInvalid)
    {
        gui_sprintf((UI_string_type) date->year, "%04d", y);        
        UI_editor_play_tone_invalid_data();
        isInvalid = MMI_FALSE;
    }

    //month
    if ((m < g_date_month_min) && (m_l > 1))
    {
        m = g_date_month_min;
        gui_sprintf((UI_string_type) date->month, "%02d", m);        
        UI_editor_play_tone_invalid_data();
    }
    else if (m > g_date_month_max)
    {
        m = g_date_month_max;
        gui_sprintf((UI_string_type) date->month, "%02d", m);        
        UI_editor_play_tone_invalid_data();
    ...
    //day
    day = max_num_of_days(y, m);

    if ((d < 1) && (d_l > 1))
    {
        d = 1;
        gui_sprintf((UI_string_type) date->day, "%02d", d);        
        UI_editor_play_tone_invalid_data();
    }
    else if (d > day)
    {
    ...
    switch (current_datetime_input->detail.date.date_type & DATE_INPUT_TYPE_MASK)
    {
        case DATE_INPUT_TYPE_DD_MM_YYYY:
             kal_wsprintf(
              (kal_wchar*)current_datetime_input->text,               
              "%02d%c%02d%c%04d ",
              d,
              current_datetime_input->seperator,
              m,
              current_datetime_input->seperator,
              y);
            break;
...
/*****************************************************************************
 * FUNCTION
 *  default_inline_time_validation
 * DESCRIPTION
 *  validation function of date input of inline editing
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void default_inline_time_validation()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    Times *time = &current_datetime_input->detail.time;    
    S32 h = gui_atoi((UI_string_type) time->hour);
    S32 m = gui_atoi((UI_string_type) time->minute);
    BOOL isInvalid = MMI_FALSE;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ...
    if(isInvalid)
    {        
        gui_sprintf((UI_string_type) time->hour, "%02d", h);  
        UI_editor_play_tone_invalid_data();
        isInvalid = MMI_FALSE;
    }
    ...
    if(isInvalid)
    {    
        gui_sprintf((UI_string_type) time->minute, "%02d", m);       
        UI_editor_play_tone_invalid_data();
    }
...
/*****************************************************************************
 * FUNCTION
 *  default_inline_duration_validation
 * DESCRIPTION
 *  validation function of date input of inline editing
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void default_inline_duration_validation()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    Durations *time = &current_datetime_input->detail.duration;    
    S32 h = gui_atoi((UI_string_type) time->hour);
    S32 m = gui_atoi((UI_string_type) time->minute);
    S32 s = gui_atoi((UI_string_type) time->second);
    BOOL isInvalid = MMI_FALSE;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ...
    if(isInvalid)
    {    
        gui_sprintf((UI_string_type) time->minute, "%02d", s);       
        UI_editor_play_tone_invalid_data();
    }
...
#endif /*__DATETIME_MULTI_LINE_SUPPORT_DURATION__*/


/*****************************************************************************
 * FUNCTION
 *  default_inline_IP4_validation
 * DESCRIPTION
 *  validation function of date input of inline editing
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void default_inline_IP4_validation()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    IP4 *ip = &current_datetime_input->detail.ip4;    
    S32 ip1 = gui_atoi((UI_string_type) ip->IP1);
    S32 ip2 = gui_atoi((UI_string_type) ip->IP2);
    S32 ip3 = gui_atoi((UI_string_type) ip->IP3);
    S32 ip4 = gui_atoi((UI_string_type) ip->IP4);
    BOOL isInvalid = MMI_FALSE;
    U8 i = 0;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    ...
    if(isInvalid)
    {
        gui_sprintf((UI_string_type) ip->IP1, "%03d", ip1);         
        UI_editor_play_tone_invalid_data();
        isInvalid = MMI_FALSE;
    }
    ...
    if(isInvalid)
    {
        gui_sprintf((UI_string_type) ip->IP2, "%03d", ip2);           
        UI_editor_play_tone_invalid_data();
        isInvalid = MMI_FALSE;
    }
    ...
    if(isInvalid)
    {
        gui_sprintf((UI_string_type) ip->IP3, "%03d", ip3);         
        UI_editor_play_tone_invalid_data();
        isInvalid = MMI_FALSE;
    }
    ...
    if(isInvalid)
    {
        gui_sprintf((UI_string_type) ip->IP4, "%03d", ip4);          
        UI_editor_play_tone_invalid_data();
    }    
...
/*****************************************************************************
 * FUNCTION
 *  datetime_validation_callback
 * DESCRIPTION
 *  validation function of date/time/ip4/duration input of inline editing
 * PARAMETERS
 *  text                   [IN]        text in inline
 *  cursor                [IN]        the text cursor point to will show
 *  text_length         [IN]        text length
 * RETURNS
 *  void
 *****************************************************************************/
void datetime_validation_callback(
      UI_buffer_type text,
      UI_buffer_type cursor,
      S32 text_length)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    current_datetime_input->text = text;
    datetime_get_detail();
 ...
 *  create_date_input
 * DESCRIPTION
 *  create date/time/duration/ip4 input box
 * PARAMETERS
 *  d                   [IN]        Date input box
 *  flags               [IN]      
 * RETURNS
        ...
        U32 flags)
{
    ...
    if(d->type < DATETIME_INPUT_TYPE_BASE || d->type > DATETIME_INPUT_TYPE_MAX)
        return;
    
    gui_set_font(datetime_input_theme.text_font);
    
    d->flags = UI_MULTI_LINE_INPUT_BOX_OVERWRITE_MODE | UI_MULTI_LINE_INPUT_BOX_DISABLE_CURSOR_DRAW;
#if defined(__MMI_FTE_SUPPORT__) || defined(__MMI_BASIC_UI_STYLE__)
    d->flags |= UI_MULTI_LINE_INPUT_BOX_DISABLE_BACKGROUND;
#endif

    MMI_multiline_inputbox.flags = d->flags;
    d->current_focus = 0;

    /* Setup default validation functions  */
    d->validation_callback = datetime_validation_callback;
    MMI_multiline_inputbox.validation_callback =  d->validation_callback;
 ...
 *  void
 *****************************************************************************/
void datetime_input_show_background(datetime_input *d)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    gdi_layer_push_clip();
    if (d->flags & GENERAL_INPUT_USE_RESET_CLIP)
    {
        gui_set_clip_preset(
            d->x,
            d->y,
    ...
    else
    {
        ...
        y2 = y1 + d->height - 1;
        /* the default background  */
        gui_draw_filled_area(x1, y1, x2, y2, &datetime_input_background_filler);
    }
    gdi_layer_pop_clip();
}
 ...
 *  void
 *****************************************************************************/
void datetime_input_reset_focus(datetime_input *temp) //(void *temp)//
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    datetime_input *d = (datetime_input *)temp;
    ...
    if (d->is_focus)
    {
        ...
        d->flags = MMI_multiline_inputbox.flags;
    }
}
 ...
 *  void
 *****************************************************************************/
void datetime_input_set_focus(datetime_input *temp) //(void *temp)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (d->is_focus == MMI_FALSE)
    {        
        d->is_focus = MMI_TRUE;

        if(current_datetime_input && d->type != current_datetime_input->type)
        {
            current_datetime_input->is_focus = MMI_FALSE;
        }
        ...
        gui_create_multi_line_input_box_set_buffer(
             ...
             d->width,
             d->height,
             (UI_string_type)d->text,
             datetime_get_length(d->type)* ENCODING_LENGTH,
             (datetime_get_length(d->type) + 1)* ENCODING_LENGTH, 
             0);   

        MMI_multiline_inputbox.flags = UI_MULTI_LINE_INPUT_BOX_OVERWRITE_MODE | UI_MULTI_LINE_INPUT_BOX_DISABLE_CURSOR_DRAW;
    #if defined(__MMI_FTE_SUPPORT__) || defined(__MMI_BASIC_UI_STYLE__)
 ...
 *  void
 *****************************************************************************/
void show_datetime_input(datetime_input *d)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    UI_multi_line_input_box_theme *t;
    ...
    if(d->is_focus)
    {    
        t = current_multi_line_input_box_theme;    
        current_multi_line_input_box_theme = &datetime_input_theme;
        gui_set_multi_line_input_box_theme(&MMI_multiline_inputbox, current_multi_line_input_box_theme);
        datetime_input_show_background(d);

		MMI_multiline_inputbox.ext_flags |= GUI_MULTI_LINE_INPUT_BOX_SINGLINE_STYLE ;
		MMI_multiline_inputbox.flags |= UI_MULTI_LINE_INPUT_BOX_DISABLE_BACKGROUND|UI_MULTI_LINE_INPUT_BOX_DISABLE_SCROLLBAR ;
    #ifdef __MMI_INLINE_HIGHLIGHT_CAPTION__
        MMI_multiline_inputbox.selected_text_color = MMI_multiline_inputbox.normal_text_color;
    #endif
        ...
        current_multi_line_input_box_theme = t;
    }
    else
    {
        gdi_layer_lock_frame_buffer();
    ...
    #if defined(__MMI_IMAGE_CACHE_FOR_ONE_GDI_LAYER__) 
		gui_one_layer_draw_filled_bg_area(d->x, d->y, d->x+d->width-1, d->y + d->height -1);									
    #else
		gui_fill_rectangle(d->x, d->y, d->width, d->height, datetime_input_theme.disabled_filler->c);
    #endif/*__MMI_IMAGE_CACHE_FOR_ONE_GDI_LAYER__*/
        gui_reset_text_clip();
        gui_set_font(&UI_DEFAULT_FONT);  
        gui_set_text_color(datetime_input_theme.normal_text_color);
                       
        if (mmi_fe_get_r2l_state())
        {
            gui_move_text_cursor(d->x + d->width, d->y);
        }
        else
        {
            gui_move_text_cursor(d->x, d->y);
        }
    
        //gui_print_text((UI_string_type)d->text); 
 ...
 *  void
 *****************************************************************************/
void datetime_input_move_to_x_y(datetime_input *d, S32 x, S32 y)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    UI_buffer_type backup_cursor_p = MMI_multiline_inputbox.cursor_p;
    U32            backup_current_focus = d->current_focus;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (PEN_CHECK_BOUND(x, y, d->x, d->y, d->width, d->height))
    {
        wgui_inputs_ml_move_cursor_to_start();        
        datetime_input_reset_focus(d);
        datetime_input_set_focus(d);
        gui_show_multi_line_input_box_ext(&MMI_multiline_inputbox, x, y);
        if((*MMI_multiline_inputbox.cursor_p == '.')
            ||(*MMI_multiline_inputbox.cursor_p == '/')
            ||(*MMI_multiline_inputbox.cursor_p == ':')
            ||(*MMI_multiline_inputbox.cursor_p == ' ')
            ||(*MMI_multiline_inputbox.cursor_p == 0))
        {
            MMI_multiline_inputbox.cursor_p = backup_cursor_p;
            d->current_focus = backup_current_focus;
        }
        else /* need update  d->current_focus */
        {
            d->current_focus = (MMI_multiline_inputbox.cursor_p - MMI_multiline_inputbox.text)/2;
        }
        show_datetime_input(d);
    }    
}
#endif /* defined (__MMI_TOUCH_SCREEN__) */ 
 ...
 *  void
 *****************************************************************************/
void datetime_input_set_seperator(datetime_input *d, UI_character_type seperator)
{
    d->seperator = seperator;
}
 ...
 *  void
 *****************************************************************************/
void datetime_input_set_hide_function(datetime_input *d, void (*f) (S32 x1, S32 y1, S32 x2, S32 y2))
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void set_current_datetime_input(datetime_input *d)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void datetime_input_direct_input(UI_character_type c)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (current_datetime_input == NULL)
    {
        return;
    }
 
    datetime_input_direct_input_nodraw(c);
    
    /* show date input box */
    show_datetime_input(current_datetime_input);
    
    /* call daet input validation function */
    current_datetime_input_callback();
}
 ...
 *  void
 *****************************************************************************/
MMI_BOOL datetime_input_direct_input_nodraw(UI_character_type c)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if ( !gui_multi_line_input_box_test_last_position_overflow(&MMI_multiline_inputbox) && \
        current_datetime_input->current_focus < datetime_get_length(current_datetime_input->type) )
    {
        result = gui_multi_line_input_box_insert_character(&MMI_multiline_inputbox, c);
        current_datetime_input->current_focus++;
        if(current_datetime_input->current_focus == datetime_get_length(current_datetime_input->type))
        {
        ...
        {
            // if cursor position is seperator, move to next pos.
            if ( strcmp((char *)MMI_multiline_inputbox.cursor_p, (char *)&current_datetime_input->seperator) == 0 )
            {
                if (!gui_multi_line_input_box_test_last_position(&MMI_multiline_inputbox) && \
                    (current_datetime_input->current_focus < (U16)(datetime_get_length(current_datetime_input->type) - 1)))
                {
 ...
 *  S32
 *****************************************************************************/
S32 datetime_input_test_last_position(datetime_input *d)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    if (gui_multi_line_input_box_test_last_position(&MMI_multiline_inputbox) || \
        (current_datetime_input->current_focus == datetime_get_length(current_datetime_input->type) - 1) )
    {
        return 1;
    }
    else
 ...
 *  void
 *****************************************************************************/
void datetime_input_delete_character(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void datetime_input_previous_character(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (!gui_multi_line_input_box_test_first_position(&MMI_multiline_inputbox) && current_datetime_input->current_focus > 0)
    {
        gui_multi_line_input_box_previous(&MMI_multiline_inputbox);
        current_datetime_input->current_focus --;

        /* if cursor position is seperator, move to previous pos. */
        if ( strcmp((char *)MMI_multiline_inputbox.cursor_p, (char *)&current_datetime_input->seperator) == 0 && current_datetime_input->current_focus > 0)
        {
            if (!gui_multi_line_input_box_test_first_position(&MMI_multiline_inputbox))
            {
                gui_multi_line_input_box_previous(&MMI_multiline_inputbox);                
 ...
 *  void
 *****************************************************************************/
void datetime_input_first_character(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    wgui_inputs_ml_move_cursor_to_start();
    current_datetime_input->current_focus = 0;
 ...
 *  void
 *****************************************************************************/
void datetime_input_next_character(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if ( !gui_multi_line_input_box_test_last_position(&MMI_multiline_inputbox) && (current_datetime_input->current_focus < (U16)(datetime_get_length(current_datetime_input->type) - 1)))
    {
        gui_multi_line_input_box_next(&MMI_multiline_inputbox);
        current_datetime_input->current_focus++;
 ...
 *  void
 *****************************************************************************/
void datetime_input_toggle_insert_mode(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
static void datetime_input_delete_character_internal(MMI_BOOL delete_all)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void datetime_input_delete_current_character(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    datetime_input_delete_character_internal(MMI_FALSE);
}
 ...
 *  void
 *****************************************************************************/
void datetime_input_delete_all_characters(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    datetime_input_delete_character_internal(MMI_TRUE);
}
 ...
 *  void
 *****************************************************************************/
void datetime_input_numeric_keyboard_input_handler(S32 keyc)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (keyc >= '0' && keyc <= '9')
    {
        datetime_input_direct_input((U8) keyc);
    }
    else if (keyc == 0x08)
    {
        datetime_input_delete_character();
    }
    else if (keyc == 0x1b)
    {
        datetime_input_delete_all_characters();
    }
    /* else if (keyc == 0x0d); */
 ...
 *  void
 *****************************************************************************/
void datetime_input_handle_key_down(MMI_key_code_type k)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    datetime_input_direct_input((UI_character_type) ('0' + k));
}
 ...
 *  void
 *****************************************************************************/
void datetime_input_key_handler(S32 vkey_code, S32 key_state)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (key_state)
    {
        ...
        {
            case 37:
                datetime_input_previous_character();
                break;
            case 38:    /* up */
                break;
            case 39:
                datetime_input_next_character();
                break;
            case 40:    /* down */
                break;
            case 36:    /* home */
            ...
            case 34:    /* page down */
                break;
            case 45:
                datetime_input_toggle_insert_mode();
                break;
            case 46:
                datetime_input_delete_current_character();
                break;
        }
    }
#else /* (MMI_BUILD_TYPE == BUILD_TYPE_X86WIN32) */ 
 ...
 *  void
 *****************************************************************************/
void datetime_input_register_keys(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    register_MMI_key_input_handler();
    register_key_down_handler(datetime_input_handle_key_down);
    register_keyboard_input_handler(datetime_input_numeric_keyboard_input_handler);
    SetKeyHandler(datetime_input_previous_character, KEY_LEFT_ARROW, KEY_EVENT_DOWN);
    SetKeyHandler(datetime_input_next_character, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
    SetKeyHandler(datetime_input_previous_character, KEY_LEFT_ARROW, KEY_LONG_PRESS);
    SetKeyHandler(datetime_input_next_character, KEY_RIGHT_ARROW, KEY_LONG_PRESS);
    SetKeyHandler(datetime_input_previous_character, KEY_LEFT_ARROW, KEY_REPEAT);
    SetKeyHandler(datetime_input_next_character, KEY_RIGHT_ARROW, KEY_REPEAT);
    register_keyboard_key_handler(datetime_input_key_handler);
 ...
 *  void
 *****************************************************************************/
void datetime_input_clear_keys(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void register_datetime_input_callback(void (*f) (void))
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    current_datetime_input_callback = f;
}
 ...
 *  void
 *****************************************************************************/
void clear_datetime_input_callback(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    current_datetime_input_callback = UI_dummy_function;
}
