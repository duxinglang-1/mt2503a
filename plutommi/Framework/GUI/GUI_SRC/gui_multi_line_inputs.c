...
MMI_BOOL g_gui_inputs_enable_read_lang = MMI_FALSE;
mmi_font_engine_read_lang_enum g_gui_inputs_read_lang = MMI_FONT_ENGINE_READ_LANG_NONE;
#endif
...
void  gui_inputs_ml_enable_read_language(mmi_font_engine_read_lang_enum lang)
{
	
	g_gui_inputs_enable_read_lang = MMI_TRUE;
	g_gui_inputs_read_lang = lang;
}

void  gui_inputs_ml_disable_read_language()
{
	g_gui_inputs_enable_read_lang = MMI_FALSE;
	g_gui_inputs_read_lang = MMI_FONT_ENGINE_READ_LANG_NONE;
}
#endif
...
void gui_init_multi_line_show_struct(multi_line_input_box *b, multi_line_input_box_show_struct *bs)
{
    ...
    #else /*__MMI_EDITOR_MULTILINE_SUPPORT_RICH_TEXT__ 00 */

    bs->rich_text.curr_font = *b->text_font;
    bs->rich_text.curr_border = 0;
                            ...
                            else
                            {
                                //gui_draw_filled_area(b->x, bs->y1 - l_height, bs->x2, bs->y2, &f);
                                   ...
                                   if( (b->ext_flags & GUI_MULTI_LINE_INPUT_BOX_SET_BG_COLOR)
                                    ...
                                    {
                                        gui_one_layer_draw_filled_bg_area(b->x, bs->y1 - l_height, bs->x2, bs->y2);
                                    }    
                    ...
                    else
                    {
...
#endif /*__MMI_EDITOR_MULTILINE_SUPPORT_SEMITRANSPARENT_BACKGROUND__*/                        
                        {
                            ...
                            {                             
                                    ...
                                    {
                                        gui_one_layer_draw_filled_bg_area(bs->x1, bs->y1 - l_height, bs->x2, bs->y2);
                                    }    
                                #else /*__MMI_IMAGE_CACHE_FOR_ONE_GDI_LAYER__ */
                                    gui_draw_filled_area(bs->x1, bs->y1 - l_height, bs->x2, bs->y2, &f);
                                #endif/*__MMI_IMAGE_CACHE_FOR_ONE_GDI_LAYER__*/
...
U8 gui_find_multi_line_one_line(multi_line_input_box *b, multi_line_input_box_show_struct *bs)
{
...
#if defined(__MMI_LANG_ZAWGYI_MYANMAR__) && defined(__MMI_LANG_MYANMAR__)
	if (g_gui_inputs_enable_read_lang)
		mmi_fe_enable_read_language(g_gui_inputs_read_lang);
#endif
    ...
    while (g_editor_line.line_char_count < (GUI_INPUTS_MAX_NUM_OF_CHAR_FOR_EACH_LINE/2))
    {
            ...
            && (g_editor_line.line_char_count + g_editor_line.line_char[count].c_info.data_len < (GUI_INPUTS_MAX_NUM_OF_CHAR_FOR_EACH_LINE/2)))
        {
            ...
            {
                b->text_width = g_editor_line.line_width;
            }
        ...
        for(i=0; i<g_editor_line.line_unit_count; i++)
        {
            ...
            if (highlight)
            {
                if (highlight != EDITOR_HILITE_LIST_HIGHLIGHT)
                {
                    ...
                    {  
                        U16 edit_h = gui_get_string_height((UI_string_type) L"1234567890");
                        U16 move_gap = 0;
                        if(char_height > (edit_h + 1))
                        {
                            move_gap = (char_height - edit_h)/2 ;
                            if(move_gap > 3)
                            {
                                move_gap--;
                            }
                            if(char_height > (b->edit_height-1))
                            {
                                move_gap = 0;  
                            }
                        }
                        gui_fill_rectangle(xtx, bs->yty + move_gap, xtx + highlight_width , bs->yty + char_height - gap - move_gap, bs->selector_color);
                    }
                    else if((b->ext_flags & GUI_MULTI_LINE_INPUT_BOX_SINGLINE_STYLE)
                    &&((bs->yty + char_height - gap)> (bs->y2 -2)))
                    {
                        gui_fill_rectangle(xtx, bs->yty, xtx + highlight_width - 1, bs->y2-2, bs->selector_color);
                    }
                    else
#endif /*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__*/
                    {
    ...
    if (g_editor_line.cursor_line_flag)
    {
            ...
            && !(bs->flags & UI_MULTI_LINE_INPUT_BOX_DISABLE_DRAW))
        {
            if (show_cursor)
            {
                ...
                /* show cursor */
#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__
                if(!(b->ext2_flags & UI_MULTI_LINE_INPUT_BOX_SHOW_BLOCK_CURSOR))
#endif /*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__*/          
                {
        ...
        if (show_cursor)
        {
            ...
            if(b->ext_flags & GUI_MULTI_LINE_INPUT_BOX_SINGLINE_STYLE)
            {
                //b->cursor_x = b->cursor_x - b->text_offset_x;
                if (bs->default_direction == BIDI_L)
                {
                    /* normal direction layount*/
                    //bs->xx = bs->x1 + b->text_x + b->text_offset_x;
                    b->cursor_x = b->cursor_x - b->text_offset_x - bs->x1 - b->text_x;
                }
                #ifdef __MMI_BIDI_ALG__ /**/      
                else
                {
                /* from right to left, text_x and text_offset_x is moved to the right*/
                    b->cursor_x = b->cursor_x - b->text_offset_x - bs->x1;
                }
                #endif /* __MMI_BIDI_ALG__ */
            }
            if(b->ext2_flags & UI_MULTI_LINE_INPUT_BOX_SHOW_BLOCK_CURSOR)
...
/* under construction !*/
    ...
    if(b->ext_flags & GUI_MULTI_LINE_INPUT_BOX_SINGLINE_STYLE)
    {
        ...
        #ifdef __MMI_FTE_SUPPORT__
        if (b->flags & UI_MULTI_LINE_INPUT_BOX_DISABLE_BACKGROUND)
        {
