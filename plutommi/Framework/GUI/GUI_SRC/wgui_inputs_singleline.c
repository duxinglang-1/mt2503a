...
/* under construction !*/
    ...
    wgui_inputs_ml_create_set_buffer( buffer, buffer_length, text_length, edit_position);
...
S32 wgui_inputs_sl_multitap_input_string(PU8 str)
{
    ...
    return wgui_inputs_ml_multitap_input_string(str);
...
MMI_BOOL wgui_inputs_sl_multitap_input(UI_character_type c)
{
    ...
    wgui_inputs_ml_multitap_input(c);
...
/* under construction !*/
    ...
    return wgui_inputs_ml_direct_input(c);
...
MMI_BOOL wgui_inputs_sl_direct_input_no_draw(UI_character_type c)
{
    ...
    wgui_inputs_ml_direct_input_no_draw(c);
...
void wgui_inputs_sl_register_keys(void)
{
    ...
    {
        SetKeyDownHandler(wgui_inputs_sl_previous_character, KEY_LEFT_ARROW);
        SetKeyDownHandler(wgui_inputs_sl_next_character, KEY_RIGHT_ARROW);

        SetKeyRepeatHandler(wgui_inputs_sl_handle_left_arrow_repeat, KEY_LEFT_ARROW);
        SetKeyRepeatHandler(wgui_inputs_sl_handle_right_arrow_repeat, KEY_RIGHT_ARROW);
    ...
    {
        SetKeyDownHandler(wgui_inputs_sl_cannot_navigate, KEY_UP_ARROW);
        SetKeyDownHandler(wgui_inputs_sl_cannot_navigate, KEY_DOWN_ARROW);
   }
    ...
    SetKeyDownHandler(wgui_inputs_sl_delete_character, KEY_CLEAR);
    SetKeyLongpressHandler(wgui_inputs_sl_delete_all_characters, KEY_CLEAR);
...
MMI_BOOL get_wgui_inputs_sl_disable_left_right_key_flag(void)
{
    return g_is_disable_left_right_key_register;
}
MMI_BOOL get_wgui_inputs_sl_disable_up_down_key_flagr(void)
{
    return g_is_disable_up_down_key_register;
}
#endif
...
MMI_BOOL wgui_inputs_sl_get_display_status(void)
{
    ...
    return wgui_inputs_ml_get_display_status();
...
void wgui_inputs_sl_register_input_callback(void (*f) (void))
{
    ...
    singleline_inputbox_input_callback = f;
     ...
     wgui_inputs_ml_register_input_callback( f);
...
void wgui_inputs_sl_register_navigate_callback(void (*f) (void))
{
    ...
    wgui_inputs_ml_register_navigate_callback(f);
...
U8 wgui_inputs_sl_is_empty(void)
{
    ...
    return  wgui_inputs_ml_is_empty();
...
U8 wgui_inputs_sl_set_category_history(U16 history_ID, U8 *history_buffer)
{
    ...
    wgui_inputs_ml_set_category_history(history_ID,history_buffer);
...
void wgui_inputs_sl_get_category_history(U16 history_ID, U8 *history_buffer)
{
    ...
    wgui_inputs_ml_get_category_history(history_ID,history_buffer);
...
void wgui_inputs_sl_register_handle_clear_key_up(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    SetKeyUpHandler(wgui_inputs_sl_handle_right_softkey_up, KEY_CLEAR);
#else /* __MMI_EDITOR_SINGLINE_SUPPORT__*/
...
void wgui_inputs_sl_handle_right_softkey_long_press(void)
{
    ...
    {        
        SetKeyRepeatHandler(wgui_inputs_sl_handle_right_softkey_repeat, KEY_CLEAR);
    }
    else
    {        
        set_right_softkey_function(wgui_inputs_sl_handle_right_softkey_touch_repeat, KEY_REPEAT);
        SetKeyRepeatHandler(wgui_inputs_sl_handle_right_softkey_repeat, KEY_RSK); 
    }
    ...
    if (gui_single_line_input_box_get_text_length(&MMI_singleline_inputbox) > 0)
    {
        if (mmi_frm_kbd_is_key_supported(KEY_CLEAR))
        {
            if (wgui_singleline_inputbox_RSK_label_clear == 0)
            {
                ...
                SetKeyDownHandler(wgui_inputs_sl_handle_right_softkey_down, KEY_CLEAR);
                SetKeyUpHandler(UI_dummy_function, KEY_CLEAR);
                SetKeyLongpressHandler(wgui_inputs_sl_handle_right_softkey_long_press, KEY_CLEAR);
...
void wgui_inputs_sl_validation_callback(UI_buffer_type text, UI_buffer_type cursor, S32 text_length)
{
    ...
    wgui_inputs_ml_validation_callback( text, cursor, text_length);
...
S32 wgui_inputs_sl_get_ucs2_count(void)
{
    ...
    return wgui_inputs_ml_get_ucs2_count();
...
S32 wgui_inputs_sl_get_ext_gsm_count(void)
{
    ...
    return wgui_inputs_ml_get_ext_gsm_count();
        ...
        U32 (*msg_call_back) (void * input_box_handle, mmi_imc_message_struct_p msg_ptr))
{
    ...
    gui_single_line_input_box_locate_cursor(&MMI_singleline_inputbox);
#else /* __MMI_EDITOR_SINGLINE_SUPPORT__*/
        U32 (*msg_call_back_input) (void * input_box_handle, mmi_imc_message_struct_p msg_ptr);

        if(msg_call_back == mmi_input_box_msg_call_back_singleline)
        {
            msg_call_back_input = mmi_input_box_msg_call_back_multiline;
        }
        else
        {
           msg_call_back_input =  msg_call_back;
        }
...
U32 mmi_input_box_msg_call_back_singleline(void * input_box_handle, mmi_imc_message_struct_p msg_ptr)
{
    ...
    case MMI_IMC_MESSAGE_INSERT_CHAR:
        ...
        #endif
    #if defined(__MMI_BT_PBAP_CLIENT__)
        kal_wap_trace(MOD_MMI_COMMON_APP, TRACE_GROUP_9, "[myler] mmi_input_box_msg_call_back_singleline\n");

        if (g_phb_pbapc_show_symbol == 1)
        {
            g_wgui_cat199_refresh_when_input_complete = MMI_TRUE;
...
//#endif				   
    ...
    case MMI_IMC_MESSAGE_SET_KEY:
        ...
        if((((mmi_editor_handler_struct *)input_box_handle)->input_box_sub_type) == MMI_EDITOR_QUICK_SEARCH_SINGLE_LINE_INPUT_BOX)
        {
            ...
            SetKeyDownHandler(dynamic_list_goto_previous_item, KEY_UP_ARROW);
            SetKeyDownHandler(dynamic_list_goto_next_item, KEY_DOWN_ARROW);
            SetKeyRepeatHandler(dynamic_list_goto_previous_item, KEY_UP_ARROW);
            SetKeyRepeatHandler(dynamic_list_goto_next_item, KEY_DOWN_ARROW);
        }
			...
			MMI_BOOL (*handler)(gui_multi_line_input_box_change_event_enum event,U16 C))
{
    ...
    wgui_inputs_ml_register_change_event_handler(handler);
...
void wgui_inputs_sl_set_default_text(UI_string_type default_text)
{
    ...
    UI_UNUSED_PARAMETER(default_text);
...
void wgui_inputs_sl_move_cursor(wgui_inputs_cursor_pos_enum pos)
{
    ...
    wgui_inputs_ml_move_cursor(pos);
...
U32 wgui_inputs_sl_insert_string(UI_string_type str, U32 len, MMI_BOOL is_redraw)
{
    ...
    return wgui_inputs_ml_insert_string(str, len, is_redraw);
