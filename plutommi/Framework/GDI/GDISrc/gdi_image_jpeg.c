...
/*****************************************************************************
 * FUNCTION
 *  gdi_image_jpeg_set_exif_info_internal
 * DESCRIPTION
 *****************************************************************************/
GDI_RESULT gdi_image_jpeg_set_exif_info_internal(U8 *filename, EXIF_IFD_ENUM ifd, EXIF_ENTRY *entry_ptr)
{
    GDI_RESULT ret = GDI_FAILED;
#ifdef GDI_USING_JPEG_EXIF
    kal_uint32 wb_size, int_mem_size, ext_mem_size;
    JAIA_BUFFER wb;
    JAIA_HANDLE ws;
    STFSAL fhSrc;
    STFSAL fhDst;
    ...
    void *intMem = NULL;
    void *wsBuf = NULL;
    JAIA_RETURN_STATUS_ENUM status;
    kal_int32 fl;
    kal_wchar dst_name[255] = L"\0";
    fl = app_ucs2_wcslen((U16 *)filename);
    app_ucs2_wcscat(dst_name, (U16 *)filename);
    app_ucs2_wcscat(dst_name, L".cftmp");

    GDI_DEBUG_ASSERT(NULL != entry_ptr);
    fsStat = FSAL_Open(&fhSrc, filename, FSAL_READ);
    if (fsStat != FSAL_OK )
    {
        return GDI_IMAGE_ERR_OPEN_FILE_FAILED;
    }
    fsStat = FSAL_Open(&fhDst, dst_name, FSAL_WRITE);
    if (fsStat != FSAL_OK )
    {
        FSAL_Close(&fhSrc);
        return GDI_IMAGE_ERR_OPEN_FILE_FAILED;
    }
    status = jaiaQueryWorkspaceBuffer(&wb_size);
    if (JAIA_RETURN_OK == status)
    {
        wb.buff_size = wb_size;
        wsBuf = (void *)gdi_alloc_ext_mem(wb_size);
        GDI_DEBUG_ASSERT(NULL != wsBuf);
        wb.buff_addr = wsBuf;
        status = jaiaCreateWorkspace(&ws, &wb);
        if (JAIA_RETURN_OK == status)
        {
        	status = jaiaSetSourceFile(ws, &fhSrc);
            if (JAIA_RETURN_OK == status)
            {
                status = jaiaAddExifEntry(ws, ifd, entry_ptr);
                if (JAIA_RETURN_OK == status)
                {
                    status = jaiaSetDestinationFile(ws, &fhDst);
                    if (JAIA_RETURN_OK == status)
                    {
                        status = jaiaQueryWorkingMemory(ws, &int_mem_size, &ext_mem_size);
                        if (JAIA_RETURN_OK == status)
                        {
                            intMem = (void *)gdi_alloc_ext_mem(int_mem_size);
                            extMem = (void *)gdi_alloc_ext_mem(ext_mem_size);

                            // queried internal or external memory maybe 0, but I don't think both will be 0
                            if (intMem || extMem)
                            {
                                JAIA_BUFFER int_working_buf, ext_working_buf;
                                int_working_buf.buff_addr = intMem;
                                int_working_buf.buff_size = int_mem_size;
                                ext_working_buf.buff_addr = extMem;
                                ext_working_buf.buff_size = ext_mem_size;
                                status = jaiaSetWorkingMemory(ws, &int_working_buf, &ext_working_buf);
                                if (JAIA_RETURN_OK == status)
                                {
                                    status = jaiaStart(ws);
                                    if (JAIA_RETURN_OK == status)
                                    {
                                        status = jaiaReleaseWorkspace(ws);
                                        if (JAIA_RETURN_OK == status)
                                        {
                                            ret = GDI_SUCCEED;
                                        }
    ...
    if (GDI_SUCCEED == ret)
    {
        ...
        FS_Rename(dst_name, (U16*)filename);
    }
    if (wsBuf)
    {
        gdi_free_ext_mem(&wsBuf);
    }
    if (intMem)
    {
...
GDI_RESULT gdi_image_jpeg_set_exif_info(U8 *filename, EXIF_IFD_ENUM ifd, EXIF_ENTRY *entry_ptr)
{
#ifdef GDI_USING_JPEG_EXIF
    return gdi_image_jpeg_set_exif_info_internal(filename, ifd, entry_ptr);
#else
    return GDI_FAILED;
#endif
}
