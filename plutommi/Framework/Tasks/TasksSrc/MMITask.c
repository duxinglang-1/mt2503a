...
/***frank_shao add******/
#if defined(FALL_DETECT_SUPPORT)
#include "mdi_audio.h"
#include "mmi_rp_app_mainmenu_def.h"
#include "TimerEvents.h"
#include "fall_sound.h"
//#include "..\..\custom\drv\Motion_sensor\LSM6DS3_I2C\algorithm.h"


#define FALL_DETECT_TEST_MODE
#define PRINT_RAW_DATA_TEST_MODE

static unsigned char g_fall_detect_is_speaking = 0;
static unsigned char g_fall_detect_img_index = 0;
MMI_ID fall_detect_group_id;


void GGT_mmi_speak_fall_detect_aud_handler(mdi_result result, void *user_data)
{	
	g_fall_detect_is_speaking = 0;
}

void GGT_mmi_speak_fall_detect_aud(void)
{	
	mdi_result result;

	kal_prompt_trace(MOD_PPP,"GGT_mmi_speak_fall_detect_aud\r\n");
	if(g_fall_detect_is_speaking)
		return;
	
	result = mdi_audio_play_string(
        (void*)fall_alarm_sound,
        sizeof(fall_alarm_sound),
        MDI_FORMAT_DAF,
        DEVICE_AUDIO_PLAY_ONCE,
        GGT_mmi_speak_fall_detect_aud_handler,
        NULL);
	kal_prompt_trace(MOD_PPP,"GGT_mmi_speak_fall_detect_aud result:%d,len:%d\r\n",result,sizeof(fall_alarm_sound));
	if(result==MDI_AUDIO_SUCCESS)
	{
		
		g_fall_detect_is_speaking = 1;		
	}
}


void GGT_mmi_stop_speak_fall_detect_aud(void)
{
	if(g_fall_detect_is_speaking)
	{
		g_fall_detect_is_speaking = 0;
...
void GGT_mmi_show_fall_detect_img(void)
{

	#if(UI_DOUBLE_BUFFER_SUPPORT)
    gui_lock_double_buffer();
	#endif 
	gdi_layer_push_and_set_clip(0, 0, 240, 240);
	gdi_image_draw(0, 0, (PU8)GetImage(FALL_DETECT_IMAGE_1+g_fall_detect_img_index));
	g_fall_detect_img_index++;
	if(g_fall_detect_img_index>=3)
	{
		g_fall_detect_img_index = 0;
	}
	gdi_layer_pop_clip();
	#if(UI_DOUBLE_BUFFER_SUPPORT)
    gui_unlock_double_buffer();
    gui_BLT_double_buffer(0, 0, 240, 240);
	#endif /* (UI_DOUBLE_BUFFER_SUPPORT) */ 
	StartTimer(GT_WRT_FALL_DETECT_SCR_TIMER,500,GGT_mmi_show_fall_detect_img);
}
void GGT_mmi_exit_fall_detect_scr(void)
{
	#if 1
	if(mmi_frm_scrn_get_active_id() == SCR_ID_FALL_DETECT)
	{
		mmi_frm_scrn_close_active_id();
		StopTimer(GT_WRT_FALL_DETECT_SCR_TIMER);
		GGT_mmi_stop_speak_fall_detect_aud();
		g_fall_detect_img_index = 0;		
	}
	#else
		GGT_mmi_stop_speak_fall_detect_aud();
...
UI_filled_area fall_detect_scr_background = 
{
...
static U8 displayBuff[512] = {0};
#ifdef FALL_DETECT_TEST_MODE
extern void get_tri_fo_std_thres(double *tri_thr, double *fo_thr, double *std_thr);
#endif
#ifdef PRINT_RAW_DATA_TEST_MODE
...
#endif
void GGT_mmi_fall_detect_real(void)
{
	GT_pro_soc_upl_real_para();
	GGT_mmi_exit_fall_detect_scr();
}

void GGT_mmi_fall_detect_error(void)
{
	GT_pro_soc_upl_real_para();
	GGT_mmi_exit_fall_detect_scr();
}

void GGT_mmi_entry_fall_detect_scr(void)
{
	...
	if (mmi_frm_scrn_enter(
            ...
            SCR_ID_FALL_DETECT,
            NULL,
            (FuncPtr)GGT_mmi_entry_fall_detect_scr,
            MMI_FRM_UNKNOW_SCRN))
    {
    	srv_backlight_turn_on(SRV_BACKLIGHT_SHORT_TIME);
    	g_fall_detect_img_index = 0;
		GGT_mmi_show_fall_detect_img();
		GGT_mmi_speak_fall_detect_aud();
    }
#else
	if (mmi_frm_scrn_enter(
            mmi_frm_group_get_active_id(),
            ...
            MMI_FRM_UNKNOW_SCRN))
    {
    	U8 *guiBuffer,buff[256] = {0};
    	double tri_thr,fo_thr,std_thr,angle,svmg,d_tri_thr,d_fo_thr,d_std_thr;
		U32 data_index;

		double acc_data_x,acc_data_y,acc_data_z,gyro_data_x,gyro_data_y,gyro_data_z,gyro_magn,max_gyro_magn;
		
		srv_backlight_turn_on(SRV_BACKLIGHT_SHORT_TIME);
		
		dbg_print("GGT_mmi_entry_fall_detect_scr\r\n");
	#ifdef FALL_DETECT_TEST_MODE
		get_real_tri_fo_std_thres(&tri_thr, &fo_thr, &std_thr,&angle,&svmg,&data_index);
	#endif
		get_tri_fo_std_thres(&d_tri_thr, &d_fo_thr, &d_std_thr);

	#ifdef PRINT_RAW_DATA_TEST_MODE
		get_test_raw_data(&acc_data_x,&acc_data_y,&acc_data_z,&gyro_data_x,&gyro_data_y,&gyro_data_z,&gyro_magn,&max_gyro_magn);
	#endif
		guiBuffer =  mmi_frm_scrn_get_active_gui_buf();
	
		sprintf(buff,"acc_x:%.6f\r\nacc_y:%.6f\r\nacc_z:%.6f\r\ngyro_x:%.6f\r\ngyro_y:%.6f\r\ngyro_z:%.6f\r\n\r\ngyro_magn:%.2f\r\nmax_g:%.2f",
			acc_data_x,acc_data_y,acc_data_z,gyro_data_x,gyro_data_y,gyro_data_z,gyro_magn,max_gyro_magn);

	#if 0
		sprintf(buff,"tri_thr:%.2f\r\nfo_thr:%.2f\r\nstd_thr:%.4f\r\nangle:%.2f\r\nsvmg:%.2f\r\ndefault thr:\r\n d_tri_thr:%.2f\r\n d_fo_thr:%.2f\r\n d_std_thr:%.2f\r\n",
			tri_thr,fo_thr,std_thr,angle,svmg,pow(d_tri_thr,2),d_fo_thr,d_std_thr);
	#endif

		kal_wsprintf((WCHAR*)displayBuff,"%s",buff);
		...
		//SetKeyHandler(GGT_mmi_fall_detect_real, KEY_LSK, KEY_EVENT_UP);
		//SetKeyHandler(GGT_mmi_fall_detect_error, KEY_POWER, KEY_EVENT_UP);
...
void ls6d_fall_detect_mmi_req(void)
{

#if 1
	ilm_struct *p_send_info;

	p_send_info = allocate_ilm(MOD_DRVKBD);
	p_send_info->src_mod_id = MOD_DRVKBD;
	p_send_info->dest_mod_id = MOD_MMI;
	p_send_info->msg_id = MSG_ID_FALL_DETECT_REQ;
	p_send_info->sap_id = DRIVER_PS_SAP;
	p_send_info->local_para_ptr = NULL;
	p_send_info->peer_buff_ptr = NULL;
	msg_send_ext_queue(p_send_info); 
#endif
}

void ls6d_fall_detect_upl_data_req(void)
{
	...
	p_send_info->src_mod_id = MOD_DRVKBD;
	p_send_info->dest_mod_id = MOD_MMI;
	p_send_info->msg_id = MSG_ID_FALL_DETECT_UPL_DATA_REQ;
	p_send_info->sap_id = DRIVER_PS_SAP;
	p_send_info->local_para_ptr = NULL;
	p_send_info->peer_buff_ptr = NULL;
	msg_send_ext_queue(p_send_info); 
...
/* under construction !*/
                            ...
                            ***************************************/
                                ...
                                break;
									...
									/***frank_shao add******/
							#if defined(FALL_DETECT_SUPPORT)
							case MSG_ID_FALL_DETECT_REQ:
								{
									//extern void GT_pro_soc_upl_real_para(void);
									GT_pro_soc_upl_real_para();
									GGT_mmi_entry_fall_detect_scr();
								}
								break;
							case MSG_ID_FALL_DETECT_UPL_DATA_REQ:
								{
									//extern void GT_pro_soc_upl_real_para(void);
									//GT_pro_soc_upl_real_para();
									GGT_mmi_speak_fall_detect_aud();
									#if defined(LED_AW9106_SUPPORT)
									{
										extern void leds_aw9106_sos(void);
										leds_aw9106_sos();
									}
									#endif
								}
