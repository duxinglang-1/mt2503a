...
#endif /*GDI_USING_SCREEN_SHOT_BUFFER*/
#ifndef GDI_USING_SCREEN_SHOT_BUFFER
    switch (mode)
    {
        case 0:
		case 1:
...
void mmi_frm_init_dump_screen_string(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_frm_set_single_protocol_event_handler(MSG_ID_MMI_EQ_STR_ENABLE_REQ_IND, (PsIntFuncPtr)mmi_enable_str_dump);
	mmi_frm_set_single_protocol_event_handler(MSG_ID_MMI_EQ_STR_REQ_IND, (PsIntFuncPtr)mmi_str_cmd_ind);
#ifdef __MMI_SUPPORT_DUMP_SCREEN_STRING__
...
void mmi_frm_init_inject_string(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/    
	mmi_frm_set_multi_protocol_event_handler(MSG_ID_TST_INJECT_STRING, mmi_proc_inject_string);
    mmi_frm_set_multi_protocol_event_handler(MSG_ID_MMI_EQ_AT_QUICK_INPUT_IND, mmi_proc_at_quick_input);
}
...
/* under construction !*/
    ...
    if (strcmp((const kal_char*)inject_str->string, "[MEM_CHECK]ASM") == 0)
    {
        kal_uint32 asm_app_size = applib_mem_ap_get_total_left_size();
        
        kal_prompt_trace(MOD_MMI, "[MEM_CHECK] Index = %d, ASM remaining size = %d bytes", inject_str->index, asm_app_size);
    }

    if (strcmp((const kal_char*)inject_str->string, "[MEM_CHECK]CTRL_BUF") == 0)
    {
#if defined(DEBUG_KAL) && ( defined(DEBUG_BUF1) || defined(DEBUG_BUF2) )
        kal_uint32 i, result = 0;

        for (i = 0; (pool_info_g[i].pool_id) && (i < KAL_MAX_NUM_BUFF_POOLS); i++)
        {
            result += (pool_info_g[i].pool_id->buff_size * (kal_get_num_free_buffs((kal_poolid)pool_info_g[i].pool_id))); 
        }

        kal_prompt_trace(MOD_MMI, "[MEM_CHECK] Index = %d, CTRL_BUF remaining size = %d bytes", inject_str->index, result);
#else
        kal_prompt_trace(MOD_MMI, "[MEM_CHECK] DEBUG_KAL is undefined.");
#endif
    }
