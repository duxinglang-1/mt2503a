...
static mmi_ret mmi_sys_key_proc(mmi_frm_key_evt_struct *evt)
{
    ...
    {
    	dbg_print("mmi_sys_key_proc:%d",evt->key_code);
        if (evt->key_code == KEY_END && evt->key_type == KEY_EVENT_DOWN)
        {
            if (IS_IN_CALL())
            {
		...
		{	
			extern void GGT_mmi_exit_fall_detect_scr(void);
			GGT_mmi_exit_fall_detect_scr();
		}
		#endif
		#if defined(LED_AW9106_SUPPORT)
		{
			extern void Leds_Aw9106_Key(void);
			extern void leds_aw9106_led_sos_stop(void);
			leds_aw9106_led_sos_stop();
			Leds_Aw9106_Key();
