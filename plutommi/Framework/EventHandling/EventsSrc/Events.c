...
#define FLAG_ARRAY_LENGTH		(MAX_PROTOCOL_EVENT/8 + 1)
...
extern PsFuncPtr protocol_event_handler_funcptr[MAX_PROTOCOL_EVENT];
extern U16	protocol_event_handler_event[MAX_PROTOCOL_EVENT];
extern U8	protocol_event_handler_flagmulti[FLAG_ARRAY_LENGTH];
...
void InitEvents(void)
{
	...
	memset(protocol_event_handler_funcptr, 0, sizeof(PsFuncPtr)*MAX_PROTOCOL_EVENT); 
	memset(protocol_event_handler_event, 0, sizeof(U16)*MAX_PROTOCOL_EVENT);
	memset(protocol_event_handler_flagmulti, 0, sizeof(U8)*FLAG_ARRAY_LENGTH);
	memset(&g_event_table_runtime_info, 0, sizeof(mmi_frm_event_runtime_info_struct));
...
#if (defined(__MMI_ENBALE_SNAPSHOT__) || defined(__CAT_ONLINE_MODE__))
    mmi_frm_set_single_protocol_event_handler(MSG_ID_MMI_EQ_SCREEN_SHOT_REQ_IND, (PsIntFuncPtr)mmi_frm_at_snapshot_req);
#endif // __MMI_ENBALE_SNAPSHOT__
    mmi_frm_set_single_protocol_event_handler(MSG_ID_MMI_EQ_SCRNSIZE_QUERY_REQ_IND, (PsIntFuncPtr)mmi_pen_css_req);
#if defined(__OP11_ATK__) || defined(__AUTO_ATK__)
    mmi_frm_set_single_protocol_event_handler(MSG_ID_MMI_EQ_RAM_USAGE_REQ_IND, (PsIntFuncPtr)mmi_frm_at_ram_usage_ind_hdlr);
    mmi_frm_set_single_protocol_event_handler(MSG_ID_MMI_EQ_LAUNCH_APP_REQ_IND, (PsIntFuncPtr)mmi_frm_at_lauch_app_ind_hdlr);
    mmi_frm_set_single_protocol_event_handler(MSG_ID_MMI_EQ_APP_INFO_OUTPUT_REQ_IND, (PsIntFuncPtr)mmi_frm_at_app_info_output_ind_hdlr);
