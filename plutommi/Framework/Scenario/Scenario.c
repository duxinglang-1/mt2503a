...
#define on_hook(evt, node) mmi_frm_asm_scenario_hook(evt, node)
...
MMI_BOOL mmi_frm_set_active_group(MMI_ID group_id)
{
    ...
    if (shell.state == MMI_SCENARIO_STATE_NONE )
    {
		...
		ret = on_hook(HOOK_EVT_SET_ACTIVE_APP, (base_node_struct *)target_node);
		if (ret != MMI_RET_OK)
		{
			return MMI_FALSE;
		}
 ...
 *  void
 *****************************************************************************/
void  history_buffer_free(void *p)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static void scenario_process_post_evt_ex(U32 type)
{
    ...
    while (post_evt)
    {
...
#endif /* __MMI_FRM_HISTORY__ */
        {
            ...
            {
                MMI_ASSERT(1 == 0);
                /* for 6261 slim if use reopen this, from now on no user use this API*/
                /*
                MMI_ASSERT(shell.state == MMI_SCENARIO_STATE_NONE);
...
static void execute_node_init(base_node_struct *node, mmi_event_struct* param)
{
    ...
    if (_IS_GROUP(node))
    {
        ...
        {
            on_hook(HOOK_EVT_BEFORE_ENTER, (base_node_struct *)node);
...
static void execute_node_post_deinit(base_node_struct *node, mmi_scenario_evt_struct* evt)
{
    ...
    if (_IS_GROUP(node))
    {        
        on_hook(HOOK_EVT_AFTER_DEINIT, (base_node_struct *)node);
...
static void group_inactive(group_node_struct *node, mmi_scenario_evt_struct* evt)
{
    ...
    if (IsBitSet(node->state, MMI_SCENARIO_STATE_ACTIVE_IDX))
    {
        ...
        while(top_child != NULL)
        {
            if (
                #ifndef __MMI_NOT_TAB_SUPPORT__
                ((_IS_TAB(top_child)) && (top_child == (base_node_struct*)shell.active_tab)) ||
                #endif
                (_IS_SCRN(top_child)) && (top_child == (base_node_struct*)shell.active_scrn))
            {
                break;
            }
...
static void group_post_inactive(group_node_struct *node, mmi_scenario_evt_struct* evt)
{
    ...
    on_hook(HOOK_EVT_BEFORE_INACTIVE, (base_node_struct *)node);
    on_hook(HOOK_EVT_AFTER_INACTIVE, (base_node_struct *)node);
...
static void group_post_active(group_node_struct *node, mmi_scenario_evt_struct* evt)
{
	...
	on_hook(HOOK_EVT_BEFORE_ACTIVE, (base_node_struct *)node);
	...
	on_hook(HOOK_EVT_AFTER_ACTIVE, (base_node_struct *)node);
}
...
static mmi_ret group_close(group_node_struct *node, mmi_scenario_evt_struct* evt)
{
    ...
    #endif /* __MMI_FRM_HISTORY__ */
        on_hook (HOOK_EVT_CLOSE_FAIL, (base_node_struct *)node);
...
MMI_ID mmi_frm_group_create (MMI_ID parent_id, MMI_ID group_id, mmi_proc_func proc, void *user_data)
{
    ...
    if (!is_exist && SG_GET_NODE_FROM_DANGLE(group_id) == NULL)
    {
        ...
        on_hook(HOOK_EVT_AFTER_CREATE, (base_node_struct *)node_new);
...
static mmi_ret mmi_frm_group_close_ex (MMI_ID group_id, U32 type, group_node_struct **parent, MMI_BOOL post_to_front)
{
    ...
    on_hook(HOOK_EVT_BEFORE_CLOSE, (base_node_struct *)group);
...
static void scrn_add_history(scrn_node_struct *node)
{
    ...
    if (
        node->type == SCENARIO_SCRN_NODE 
        #ifndef __MMI_NOT_TAB_SUPPORT__
        || node->type == SCENARIO_TAB_PAGE_NODE         
        || node->type == SCENARIO_TAB_NODE
        #endif /* __MMI_NOT_TAB_SUPPORT__ */
        )
    {
        /*
...
static void scrn_active(scrn_node_struct *node, mmi_scenario_evt_struct* evt)
{
    ...
    if (!IsBitSet(node->state, MMI_SCENARIO_STATE_ACTIVE_IDX))
    {
        ...
        if (IsBitReset(shell.state, MMI_SCENARIO_STATE_BACKWARD_IDX))
        {
             ...
             */
         #ifndef __MMI_NOT_TAB_SUPPORT__
            if ((_IS_SCRN(node) || _IS_TAB(node)) && shell.active_tab)
            {
                    ...
                    )
{
    ...
    if (parent)
    {
        ...
        if (shell.active_scrn)
        {
            ...
            else if (shell.active_scrn->id == new_node->id)
            {
                MMI_TRACE(MMI_FW_TRC_G1_FRM, MMI_FRM_SCENARIO_NODE_ENTER,\
    ...
    #endif /* ifndef __MMI_NOT_TAB_SUPPORT__ */
...
static mmi_ret mmi_frm_scrn_close_ex (MMI_ID parent_id, MMI_ID scrn_id, scrn_close_type_enum flag)
{
    ...
    group_node_struct       *group = NULL;
    base_node_struct        *baseNode = NULL;
#endif /* __MMI_NOT_TAB_SUPPORT__ */
    U32                     id = 0;
    mmi_ret                 rst = MMI_RET_ERR;
...
#ifndef __MMI_NOT_TAB_SUPPORT__
    /* check node is screen first*/
    if(screen == NULL)
    {
        screen = _SCRN(SG_GET_NODE_FROM_BG(id));
        /* if bg_scenario has no this screen node, maybe it's tab and page*/
        if(screen == NULL)
        {
            group = _GROUP(SG_GET_NODE_FROM_ROOT(parent_id));
            if(group != NULL)
            {
                baseNode = group->m_tail;
                while(baseNode != NULL)
                {
                    if(SCENARIO_TAB_NODE == baseNode->type)
                    {
                        tab = _TAB(baseNode);
...
mmi_ret mmi_frm_close_to_idle_group(void)
{
	...
	if(shell.state == MMI_SCENARIO_STATE_NONE)
    {
		...
		on_hook(HOOK_EVT_BEFORE_TO_IDLE, (base_node_struct *)pCurrNode);
        ...
        if (start_node == end_node)
       {
        ...
        if (_SCRN(start_node) == shell.active_scrn)
        {
            ...
            if (_SCRN(start_node)->proc)
            {
                ...
                set_scenario_evt(&evt, parent_id, start_node->id, start_node->user_data);
...
typedef MMI_BOOL (*clean_buffer)(void *ptr);

MMI_BOOL mmi_frm_clean_gui_buffer_condiotion(void* ptr)
{
    scrn_node_struct *p_node = (scrn_node_struct*)ptr;
    if(p_node->gui_buf != (U8 *)&g_fake_gui_buf)
    {
        return MMI_TRUE;
    }
    return MMI_FALSE;
...
void mmi_frm_scrn_clean_common_buf (MMI_ID parent_id, MMI_ID scrn_id, TYPE_BUFFER type)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (found &&
        (found->type == SCENARIO_SCRN_NODE
        #ifndef __MMI_NOT_TAB_SUPPORT__
         || found->type == SCENARIO_TAB_PAGE_NODE
        #endif /* __MMI_NOT_TAB_SUPPORT__ */
        ))
    {
        if( (type == GUI_BUFFER_TYPE) && (found->gui_buf) && mmi_frm_clean_gui_buffer_condiotion(found) )
        {
            mfree_fp(found->gui_buf);
            found->gui_buf = NULL;
            found->gui_buf_size = 0;
        }
        else if((type == INPUT_BUFFER_TYPE) && (found->input_buf))
        {
            mfree_fp(found->input_buf);
...
U16 mmi_frm_scrn_get_gui_buf_size(MMI_ID parent_id, MMI_ID scrn_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
U16 mmi_frm_scrn_get_input_buf_size(MMI_ID parent_id, MMI_ID scrn_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
{
    mmi_frm_scrn_clean_common_buf (parent_id, scrn_id, INPUT_BUFFER_TYPE);
}

void mmi_frm_scrn_clean_gui_buf(MMI_ID parent_id, MMI_ID scrn_id)
{
    mmi_frm_scrn_clean_common_buf (parent_id, scrn_id, GUI_BUFFER_TYPE);
}
...
#endif /* #ifndef __MMI_NOT_SMALL_SCRN_SUPPORT__ */
#ifndef __MMI_FRM_HISTORY__ 
...
static mmi_ret scrn_redraw(base_node_struct *node_ptr, void * arg)
{
    ...
    */
    if ((redraw_ctxt_ptr->start_draw == MMI_FALSE)
        #ifndef __MMI_NOT_TAB_SUPPORT__
        || _IS_TAB_PAGE(node_ptr) 
        #endif /* __MMI_NOT_TAB_SUPPORT__ */
        || redraw_ctxt_ptr->skip_group_ptr)
    {
        return MMI_RET_OK;
    }
...
void mmi_frm_dump_showScrnType(scrn_node_struct* pNode, CHAR* scrnType)
{
   if(pNode->attrib & 0x0020)
   {
        memcpy(scrnType, "SMALL SCRN",  11);
   }
   else if(pNode->attrib & 0x0010)
   {
        memcpy(scrnType,  "FULL SCRN", 10);
   }
}
...
void mmi_frm_dump_node_on_modis(void* in, U32 level)
{
    ...
    while(pNode && pNode->type != 0xcc)
    {
        ...
        switch(pNode->type)
        {
            ...
            case SCENARIO_SCRN_NODE:
            {
                mmi_frm_dump_showScrnType(_SCRN(pNode), scrnType);
                kal_printf("id = [0x%04x] (S: [0x%08x]  %s)\n", (U16)(_SCRN(pNode)->id), pNode, scrnType);
                memset(scrnType, 0x00, 20);
            }
            break;
            #ifndef __MMI_NOT_TAB_SUPPORT__
            case SCENARIO_TAB_PAGE_NODE:
        ...
        else if (level == 1)
        {
            ...
            case SCENARIO_SCRN_NODE:            
                ...
                MMI_TRACE(MMI_FW_TRC_G1_FRM, MMI_FRM_SCENARIO_SCRN_NODE_L1,
                        level,
                        (U16)(_SCRN(pNode)->id), _SCRN(pNode)->exit_proc, _SCRN(pNode)->entry_proc,  scrnType, _SCRN(pNode)->user_data, _SCRN(pNode)->proc);
                break;
            default:
                break;
            }
        ...
        else
        {
            ...
            case SCENARIO_SCRN_NODE:            
                ...
                MMI_TRACE(MMI_FW_TRC_G1_FRM, MMI_FRM_SCENARIO_SCRN_NODE_Lx,
                        level, (U16)(_SCRN(pNode)->id), _SCRN(pNode)->exit_proc, _SCRN(pNode)->entry_proc, scrnType, _SCRN(pNode)->user_data, _SCRN(pNode)->proc);
                break;
            default:
                break;
            }
...
#ifdef __MMI_NOT_SMALL_SCRN_SUPPORT__
#define mmi_frm_scenario_is_redrawing() MMI_FALSE
#endif /* __MMI_NOT_SMALL_SCRN_SUPPORT__ */
...
{
	if(MMI_TRUE == mmi_frm_scrn_enter(1001, 2000, MAUI_02220671_exit1, MAUI_02220671_entry1, MMI_FRM_FULL_SCRN))
	{
...
#ifndef __MMI_NOT_TAB_SUPPORT__
void tab_1_exit(mmi_scrn_essential_struct *data)
{
    kal_printf("\n[0x%4x] [0x%4x] EVT_ID_TAB_PAGE_1_EXIT", mmi_frm_scrn_get_active_id(), data->scrn_id);
}

void tab_1_entry(mmi_scrn_essential_struct *data)
{
    if(!mmi_frm_scrn_tab_page_enter(
                        data->group_id, mmi_frm_scrn_get_active_id(),//data->scrn_id, 
                        data->scrn_id, tab_1_exit, tab_1_entry, MMI_FRM_TAB_PAGE))
    {
        return;
    }
    kal_printf("\n[0x%4x] [0x%4x] EVT_ID_TAB_PAGE_1_ENTRY", data->tab_id, data->scrn_id);
}

void tab_2_exit(mmi_scrn_essential_struct *data)
{
    kal_printf("\n[0x%4x] [0x%4x] EVT_ID_TAB_PAGE_2_EXIT", mmi_frm_scrn_get_active_id(), data->scrn_id);
}

void tab_2_entry(mmi_scrn_essential_struct *data)
{
    if(!mmi_frm_scrn_tab_page_enter(
                        data->group_id, mmi_frm_scrn_get_active_id(), 
                        data->scrn_id, tab_2_exit, tab_2_entry, MMI_FRM_TAB_PAGE))
    {
        return;
    }
    kal_printf("\n[0x%4x] [0x%4x] EVT_ID_TAB_PAGE_2_ENTRY", data->tab_id, data->scrn_id);
}

void tab_exit(mmi_scrn_essential_struct *data)
{
    //kal_printf("\n[0x%4x] [0x%4x] EVT_ID_GROUP_ENTER", group_id, scrn_id);
    /* we first inactive page, then inactive tab, currently, data->scrn_id is invalidate*/
    kal_printf("\n[0x%4x] [0x%4x] EVT_ID_TAB_EXIT",data->group_id, data->tab_id);
}



void mmi_frm_tab_enter_2()
{
    mmi_frm_tab_struct tab_page[2] = {0};
    tab_page[0].screen_id = 0x2002;
    tab_page[0].tab_entry_func = tab_1_entry;

    tab_page[1].screen_id = 0x2003;
    tab_page[1].tab_entry_func = tab_2_entry;

    kal_printf("\n[0x%4x] [0x%4x] EVT_ID_TAB_ENTRY", 0x2000, 0x2001);
    
    mmi_frm_scrn_tab_enter(0x2000, 0x2001, tab_exit, mmi_frm_tab_enter_2, &tab_page, 2, 0);
}


void mmi_launch_tab_2()
{
    mmi_frm_group_create_ex(GRP_ID_ROOT, 0x2000, mmi_group_proc_func, 0, 1);
    mmi_frm_tab_enter_2();
}

void mmi_tab_2_change_index()
{
    mmi_frm_scrn_tab_change_page_index(0x2000, 0x2001, 1);
}

void mmi_frm_tab_enter_3()
{
    mmi_frm_tab_struct tab_page[2] = {0};
    tab_page[0].screen_id = 0x3002;
    tab_page[0].tab_entry_func = tab_1_entry;

    tab_page[1].screen_id = 0x3003;
    tab_page[1].tab_entry_func = tab_2_entry;

    kal_printf("\n[0x%4x] [0x%4x] EVT_ID_TAB_ENTRY", 0x2000, 0x3001);
    
    mmi_frm_scrn_tab_enter(0x2000, 0x3001, tab_exit, mmi_frm_tab_enter_3, &tab_page, 2, 0);
}

void mmi_launch_tab_3()
{
    //mmi_frm_group_create_ex(GRP_ID_ROOT, 0x3000, mmi_group_proc_func, 0, 1);
    mmi_frm_tab_enter_3();
}

void mmi_frm_close_tab_3()
{
    mmi_frm_scrn_close(0x2000, 0x3001);
}

void tab_real_entry(mmi_scrn_essential_struct* param)
{
    mmi_frm_tab_struct tab_page[2] = {0};
    tab_page[0].screen_id = 0x2002;
    tab_page[0].tab_entry_func = tab_1_entry;
    ...
    mmi_frm_scrn_tab_enter(param->group_id, param->scrn_id, 
                          tab_exit, mmi_frm_tab_enter_2, &tab_page, 2, 0);
}

/* 
    previous tab_page not inactive
    when group has entered a new tab_page
*/
void MAUI_03429807_1()
{
    mmi_scrn_node_struct node_info;
    
    mmi_frm_close_to_idle_group();

    mmi_frm_group_create_ex(GRP_ID_ROOT, 0x1000, mmi_group_proc_func, 0, 1);
    mmi_frm_scrn_create(0x1000, 0x1100, scrn_leave_proc, 0);
    
    mmi_launch_tab_2();
    mmi_tab_2_change_index();
    
    mmi_launch_tab_3();

    mmi_frm_scrn_get_info(0x2000, 0x2001, &node_info);
    MMI_ASSERT(IsBitSet(node_info.state, MMI_SCENARIO_STATE_INACTIVE_IDX));
    MMI_ASSERT(IsBitReset(node_info.state, MMI_SCENARIO_STATE_ACTIVE_IDX));
    mmi_frm_scrn_get_info(0x2000, 0x3001, &node_info);
    MMI_ASSERT(IsBitReset(node_info.state, MMI_SCENARIO_STATE_INACTIVE_IDX));
    MMI_ASSERT(IsBitSet(node_info.state, MMI_SCENARIO_STATE_ACTIVE_IDX));
...
/* 
    can't change scrn type to tab type
    when group has already a active scrn
*/
void MAUI_03429807_2()
{
    mmi_frm_close_to_idle_group();
    ...
    mmi_frm_scrn_create(0x2000, 0x2005, scrn_leave_proc, 0);

    mmi_frm_scrn_first_enter(0x2000, 0x2001, tab_real_entry, (void*)0x1234);

    MMI_ASSERT(shell.active_tab->id = 0x20002001);
    MMI_ASSERT(shell.active_tab->user_data == 0x00001234);

    mmi_frm_group_create(GRP_ID_ROOT, 0x3000, mmi_group_proc_func, 0, 1);
