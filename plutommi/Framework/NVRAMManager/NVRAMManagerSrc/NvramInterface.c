 ...
 *  Failure return -1;
 *****************************************************************************/
S32 ReadValueSlim(U16 nDataItemId, void *pBuffer, U8 nDataType)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
S32 ReadValue(U16 nDataItemId, void *pBuffer, U8 nDataType, S16 *pError)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    status = ReadValueSlim(nDataItemId, pBuffer, nDataType);
 ...
 *  Failure return -1;
 *****************************************************************************/
S32 WriteValueSlim(U16 nDataItemId, void *pBuffer, U8 nDataType)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
S32 WriteValue(U16 nDataItemId, void *pBuffer, U8 nDataType, S16 *pError)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	status = WriteValueSlim(nDataItemId, pBuffer, nDataType);
 ...
 *****************************************************************************/
S32 ReadRecordSlim(nvram_lid_enum nLID, U16 nRecordId, void *pBuffer, U16 nBufferSize)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
S32 ReadRecord(nvram_lid_enum nLID, U16 nRecordId, void *pBuffer, U16 nBufferSize, S16 *pError)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/  
    res =  ReadRecordSlim(nLID,nRecordId,pBuffer,nBufferSize);
	*pError = NVRAM_READ_SUCCESS;
    return res;
}
 ...
 *****************************************************************************/
 S32 ReadMultiRecordSlim(nvram_lid_enum nLID, U16 nRecordId, void *pBuffer, U16 nBufferSize, U16 nRecordAmount)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
S32 ReadMultiRecord(nvram_lid_enum nLID, U16 nRecordId, void *pBuffer, U16 nBufferSize, U16 nRecordAmount, S16 *pError)
{
    ...
    res = ReadMultiRecordSlim(nLID, nRecordId, pBuffer, nBufferSize, nRecordAmount);
	 *pError = NVRAM_READ_SUCCESS;
    return res;
}
 ...
 *****************************************************************************/
 S32 WriteRecordSlim(nvram_lid_enum nLID, U16 nRecordId, void *pBuffer, U16 nBufferSize)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
S32 WriteRecord(nvram_lid_enum nLID, U16 nRecordId, void *pBuffer, U16 nBufferSize, S16 *pError)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/ 
    res = WriteRecordSlim(nLID, nRecordId, pBuffer, nBufferSize);
	 *pError = NVRAM_WRITE_SUCCESS;
    return res;
}
...
void mmi_frm_nvram_cache_validate(void)
{
   ...
   // S16 error;
    ...
    if (!is_cache_rearrange)
    {
        ...
        for(i = 0; i < sizeof(cache_lid) / sizeof(cache_lid[0]); i++)
        {
			ReadRecordSlim(cache_lid[i], 1, cache_buf[i], NVRAM_CACHE_SIZE);
            
            /*  cache key value */
            for (j = 0; j < cache_key_record[i]; j++)
            {
				ReadRecordSlim(cache_key_lid[i], j + 1, ((U8*)key_pool) + j * cache_key_size[i], cache_key_size[i]);
            }
            ...
            if (result)
            {
				WriteRecordSlim(cache_lid[i], 1, cache_buf[i], NVRAM_CACHE_SIZE);
