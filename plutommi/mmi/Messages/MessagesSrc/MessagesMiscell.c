...
/***frank_shao add******/
extern S32 GT_sms_filter_handle(srv_sms_new_msg_struct *evt,S8* content);
U16 cur_msg_id;

static void GT_delete_sms_callback(srv_sms_callback_struct* callback_data)
{
   kal_prompt_trace(MOD_PPP,"GT_delete_sms_callback=%d",callback_data->result);
}

void GT_delete_cur_message(void)
{
   srv_sms_delete_msg(cur_msg_id,GT_delete_sms_callback,NULL);
}
...
/* under construction !*/
   ...
   {
       GT_sms_filter_handle(new_msg_data,event_info->content);
   }
   cur_msg_id = msg_id;
   return;
	/***frank_shao add******/
