...
static void UnzipImage(void)
{
    ...
    sprintf(tempCommand,"rd /S /Q %s\\PLUTO_ENCODE\\", CUST_IMG_BASE_PATH);
    execute_ext_command(tempCommand);

    sprintf(tempCommand,"md %s\\PLUTO_ENCODE", CUST_IMG_BASE_PATH);
    execute_ext_command(tempCommand);

    sprintf(tempCommand,"rd /S /Q %s\\mainlcd\\", COMMON_IMG_PATH);
    execute_ext_command(tempCommand);

    sprintf(tempCommand,"rd /S /Q %s\\sublcd\\", COMMON_IMG_PATH);
    execute_ext_command(tempCommand);

    sprintf(tempCommand,"7za.exe x -y %s\\image.zip -o%s\\", COMMON_IMG_PATH, CUST_IMG_PATH_ROOT);
    execute_ext_command(tempCommand);

    sprintf(tempCommand,"xcopy /Y /Q /E %s %s\\",CUST_TEMP_PATH,CUST_IMG_PATH_ROOT);
    execute_ext_command(tempCommand);

    sprintf(tempCommand,"rd /S /Q %s\\", CUST_TEMP_PATH);
    execute_ext_command(tempCommand);

    sprintf(tempCommand,"del /Q %s\\image.zip",CUST_IMG_PATH_ROOT);
    execute_ext_command(tempCommand);
