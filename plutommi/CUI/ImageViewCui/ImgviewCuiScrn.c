...
static void cui_iv_update_title_wgui(cui_iv_cntx *pcui);
static void cui_iv_update_RSK_wgui(void);
...
#define CUI_IV_UPDATE_TITLE(pcui)                    cui_iv_update_title_wgui(pcui)
...
static void cui_iv_draw_load_string(void)
{
	...
	gdi_layer_lock_frame_buffer();
#ifdef __MMI_IV_ULTRA_SLIM__
	gdi_draw_solid_rect(0, y1, LCD_WIDTH - 1, y2, GDI_COLOR_BLACK);
#endif
...
static MMI_BOOL cui_iv_still_navigation_down(cui_iv_cntx *pcui, U16 key_code)
{
    ...
    if (pcui->count > 1)
    {
		...
		if (key_code < KEY_UP_ARROW || key_code > KEY_RIGHT_ARROW)
        {   /* down or up */
            return MMI_FALSE;
        }
...
static mmi_ret cui_iv_scrn_key_proc(mmi_event_struct *evt)
{
    ...
    {
#if defined(__MMI_IV_PLUTO_SLIM__) && defined(__MMI_IV_ULTRA_SLIM__)
...
static void cui_iv_init_layer(cui_iv_cntx *pcui)
{
	...
	gdi_layer_create(0, 0, LCD_WIDTH, LCD_HEIGHT, &pscrn_data->h_cache_layer);
#else
	gdi_layer_create_cf_using_outside_memory(GDI_COLOR_FORMAT_16, 0 , 0, LCD_WIDTH, LCD_HEIGHT, &pscrn_data->h_cache_layer, pscrn_data->cacheMemPtr, IV_CACHE_MEMORY_SIZE);
#endif
...
static MMI_BOOL cui_iv_init_memory(cui_iv_cntx *pcui)
{
	...
	pscrn_data->cacheMemPtr = mmi_frm_asm_alloc_nc_r(pcui->cui_gid, IV_CACHE_MEMORY_SIZE);
	if (pscrn_data->cacheMemPtr == NULL) {
		if (pscrn_data->medMemPtr != NULL) {
			mmi_frm_asm_free_r(pcui->cui_gid, pscrn_data->medMemPtr);
		}
		if (pscrn_data->scrMemPtr != NULL) {
...
#ifndef __GDI_MEMORY_PROFILE_2__	
	if (IV_CACHE_MEMORY_SIZE != 0)
	{
		mmi_frm_asm_free_r(pcui->cui_gid, pscrn_data->cacheMemPtr);
	}
#endif
}
...
static void cui_iv_set_blt_layer(const cui_iv_scrndata *pscrn_data, cui_iv_img_stat img_stat)
{
    ...
    {
		/* set blt layers */
		gdi_layer_set_blt_layer_nb_concurrent(pscrn_data->h_disp_layer, pscrn_data->h_cache_layer, 0, 0);
    }
    else
    {
    /* set blt layers */
		gdi_layer_set_blt_layer_nb_concurrent(pscrn_data->h_cache_layer, 0, 0, 0);		
	}
	MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_TRC_IVAPP_SET_BLT_LAYER, pscrn_data->h_disp_layer, pscrn_data->h_cache_layer, 0, 0);
}
...
static void cui_iv_update_screen(cui_iv_cntx *pcui, S32 update_trigger)
{
     ...
     */
	gdi_layer_push_and_set_active(pcui->scrn_data.h_cache_layer);
#ifdef __MMI_IV_ULTRA_SLIM__
	if (pinfo->stat == IV_IMG_STAT_OK)
    { 
...
#ifndef __MMI_IV_ULTRA_SLIM__
    /* blt to LCD */
    cui_iv_set_blt_layer(&pcui->scrn_data, pinfo->stat);
#else
	cui_iv_update_title_wgui(pcui);
	cui_iv_update_RSK_wgui();
	gdi_layer_unlock_frame_buffer();
#endif
...
static mmi_ret cui_iv_scn_proc(mmi_event_struct *evt)
{
	...
	case EVT_ID_WGUI_LSK_CLICK:
		{
			mmi_frm_key_evt_struct key_evt = {0};
			mmi_event_struct *local_evt = (mmi_event_struct *) &key_evt;

			local_evt->evt_id = EVT_ID_ON_KEY;
			key_evt.key_code = KEY_LSK;
			key_evt.key_type = KEY_EVENT_DOWN;
			key_evt.user_data = (void *)pcui;
			pcui->scrn_data.flag |= IV_FLAG_KEY_DOWN;
			cui_iv_scrn_key_proc((mmi_event_struct *) &key_evt);

			local_evt->evt_id = EVT_ID_ON_KEY;
			key_evt.key_code = KEY_LSK;
			key_evt.key_type = KEY_EVENT_UP;
			key_evt.user_data = (void *)pcui;
			cui_iv_scrn_key_proc((mmi_event_struct *) &key_evt);
		return MMI_RET_KEY_HANDLED;
		}
...
static U8 g_imgview_screen_general_ui_control_set[] = {
    1,
    DM_BUTTON_BAR1
};
...
/*****************************************************************************
 * FUNCTION
 *  mmi_imgview_get_screen_general_ui_control_set
 * DESCRIPTION
 * PARAMETERS
 * RETURNS
 *  void 
 *****************************************************************************/
static U8 *mmi_imgview_get_screen_general_ui_control_set(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    return g_imgview_screen_general_ui_control_set;
}


/*****************************************************************************
 * FUNCTION
 *  cui_iv_clear_RSK_wgui
 * DESCRIPTION
 *
 * PARAMETERS
 *  pcui        [?]
 ...
 *  void
 *****************************************************************************/
static void cui_iv_clear_RSK_wgui(WGUI_SOFTKEY_ENUM btn)
{
	/*----------------------------------------------------------------*/
	/* Local Variables												  */
	/*----------------------------------------------------------------*/
	gdi_handle active_layer;
	cui_iv_cntx *pcui = cui_iv_get_context();

	/*----------------------------------------------------------------*/
	/* Code Body													  */
	/*----------------------------------------------------------------*/
	gdi_layer_push_and_set_active(pcui->scrn_data.h_cache_layer);
    gdi_layer_push_and_set_clip(MMI_softkeys[btn].x, MMI_softkeys[btn].y, MMI_softkeys[btn].width, MMI_softkeys[btn].height);
	gdi_layer_clear(GDI_COLOR_TRANSPARENT);
    gdi_layer_pop_clip();
	gdi_layer_pop_and_restore_active();
}
...
/*****************************************************************************
 * FUNCTION
 *  cui_iv_hide_left_softkey
 * DESCRIPTION
 *  hide left softkey handler
 * PARAMETERS
 *  none
 * RETURNS
 *  void
 *****************************************************************************/
void cui_iv_hide_left_softkey(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(pcui && pcui->scrn_data.cur_img_info.stat == IV_IMG_STAT_OK)
    {
//		reset_softkey(MMI_LEFT_SOFTKEY);
//		reset_softkey(MMI_RIGHT_SOFTKEY);
        cui_iv_clear_RSK_wgui(MMI_LEFT_SOFTKEY);
    }
}
...
/*****************************************************************************
 * FUNCTION
 *  cui_iv_hide_right_softkey
 * DESCRIPTION
 *  hide right softkey handler
 * PARAMETERS
 *  none
 * RETURNS
 *  void
 *****************************************************************************/
void cui_iv_hide_right_softkey()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if(pcui && pcui->scrn_data.cur_img_info.stat == IV_IMG_STAT_OK)
    {
        cui_iv_clear_RSK_wgui(MMI_RIGHT_SOFTKEY);
    }
}
...
static void cui_iv_init_pen_handler(cui_iv_cntx *pcui)
{
    ...
    wgui_register_search_control_set_callback_function(mmi_imgview_get_screen_general_ui_control_set);
    register_hide_left_softkey(cui_iv_hide_left_softkey);
    register_hide_right_softkey(cui_iv_hide_right_softkey);
}
...
static void cui_iv_update_title_wgui(cui_iv_cntx *pcui)
{
	...
	gdi_layer_set_source_key(MMI_TRUE, GDI_COLOR_TRANSPARENT);
		...
		mmi_fe_set_antialias(MMI_TRUE);
#endif
/*	ChangeTitleString((U8*) ptitle[0]);
	gdi_layer_get_active(&active_layer);
	wgui_title_move(0,0);
    wgui_title_set_alpha_blend_layer(active_layer);
	wgui_title_set_style(GUI_TITLE_STYLE_DISABLEBG);
	//wgui_title_set_target_layer(active_layer);
	draw_title();
...
static void cui_iv_update_RSK_wgui(void)
{
	...
	//gdi_layer_get_active(&active_layer);
	gdi_layer_push_and_set_active(pcui->scrn_data.h_cache_layer);
	wgui_softkey_set_target_layer(pcui->scrn_data.h_cache_layer);
    gdi_layer_push_and_set_clip(0, 0, LCD_WIDTH-1, LCD_HEIGHT-MMI_SOFTKEY_BAR_HEIGHT-1);
    
    ChangeRightSoftkey(STR_GLOBAL_BACK, NULL);
    show_right_softkey();
    ...
    gdi_layer_unlock_frame_buffer();
	//gdi_layer_blt_previous(0, 0, LCD_WIDTH-1, LCD_HEIGHT-MMI_SOFTKEY_BAR_HEIGHT-1);
}
...
static void cui_iv_imgadp_anim_finish_cb(GDI_RESULT result)
{
    ...
    cui_iv_cntx *pcui;
    cui_iv_scrndata *pscrn;
	cui_iv_imgadp_img_struct *pImg = NULL;
...
static void cui_iv_imgadp_anim_draw(cui_iv_imgadp_img_struct *pImg, WCHAR *pfile)
{
    ...
    pImg->gdiHdl = gdi_anim_nb_draw_resized_file(pImg->x, pImg->y, pImg->w, pImg->h, (U8*)pfile);
    gdi_layer_pop_and_restore_active();
