...
*******************************************************************************/
S32 cui_menu_get_index_from_menu_id_list(mmi_menu_id menu_id);
/*******************************************************************************
...
static void menu_cui_send_event(S32 evt_id, cui_menu_obj_struct *user_data_p, mmi_id current_gid)
{
...
static cui_menu_obj_struct* cui_menu_get_user_data(mmi_id owner_gid)
{
...
static void cui_menu_get_currlist_flags_from_resource(mmi_id owner_gid)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    user_data_p = cui_menu_get_user_data(owner_gid);
...
static void cui_menu_decide_access_by_shortcut(cui_menu_obj_struct* user_data_p, WCHAR* lsk_str)
{
    ...
    if( (user_data_p->currlist_flags >= CUI_MENU_NORMAL_CHECK_BOX_LIST) 
		&& (user_data_p->currlist_flags <= CUI_MENU_RADIO_BUTTON_LIST_WITH_HINTS) || 
...
static MMI_BOOL cui_menu_list_menu_item_callback(S32 index, U32* flag, U32* flag_ex)
{
    ...
    /*----------------------------------------------------------------*/
    mmi_menu_id menu_id;
#ifdef CUI_MENU_SEPARATORS_SUPPORT
    S32 i = 0;
#endif
...
void cui_menu_refresh_currlist(mmi_id owner_gid)
{
    ...
    if (!user_data_p->enable_refresh_screen || 
        user_data_p->menu_type == CUI_MENU_TYPE_OPTION)	
    {
        return;
    }
...
#ifdef CUI_MENU_TITLE_IMAGE_SUPPORT
	    ...
	    if (cui_menu_is_menu_item_hidden(child_menu_id))
        {
            for (j = i + 1; j < user_data_p->currlist_no_of_items; j++)
            {
				...
				if((user_data_p->currlist_flags >= CUI_MENU_LIST_WITH_SINGLE_TEXT_DOUBLE_ICON_ICON_TEXT_ICON) 
					&& (user_data_p->currlist_flags <= CUI_MENU_LIST_WITH_SINGLE_TEXT_DOUBLE_ICON_WITH_HINTS_ICON_TEXT_ICON) )
				{
...
static void cui_menu_common_handler(S32 evt_id)
{
     /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    menu_cui_send_event(evt_id, user_data_p, current_gid);    
...
{   
    ...
    cui_menu_common_handler(EVT_ID_CUI_MENU_ITEM_CSK_SELECT);    
...
static void cui_menu_lsk_handler(void)
{
    ...
    cui_menu_common_handler(EVT_ID_CUI_MENU_ITEM_SELECT);
...
static void cui_menu_rsk_handler(void)
{
    ...
    else
    {
        ...
        menu_cui_send_event(EVT_ID_CUI_MENU_CLOSE_REQUEST, user_data_p, current_gid);
...
static void cui_menu_checkbox_change_state_handler(void)
{
    ...
    menu_cui_send_event(EVT_ID_CUI_MENU_CHECKBOX_CHANGE_STATE, user_data_p, current_gid);      
...
static void cui_menu_hilite_handler(S32 index)
{
    ...
    if(user_data_p->currlist_highlighted_id != ((mmi_menu_id)CUI_MENU_MENUID_NOT_FOUND))
    {
        if (!(user_data_p->currlist_flags & CUI_MENU_NORMAL_CHECK_BOX_LIST)) 
        {
	        ...
	        if(user_data_p->enable_cascading_option_menu)
	        {
	            ...
	            {
                    SetKeyDownHandler(cui_menu_lsk_handler, keyCode);
	            }
	            else
	            {
    ...
    menu_cui_send_event(EVT_ID_CUI_MENU_ITEM_HILITE, user_data_p, current_gid);  
...
static void cui_menu_screen_entry_internal(mmi_id current_gid, cui_menu_obj_struct* user_data_p)
{
    ...
    else /* in case of cascaded option menu */
    {
        if (!(user_data_p->currlist_flags & CUI_MENU_NORMAL_CHECK_BOX_LIST)) 
        {
            ...
            if(user_data_p->current_level_index)
            {
                SetKeyDownHandler(cui_menu_rsk_handler, keyCode);
            }
            else
            {
                ClearKeyHandler(keyCode, KEY_EVENT_DOWN);
...
mmi_ret cui_menu_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        ...
        case EVT_ID_GROUP_INACTIVE:
            {
                ...
                if(user_data_p != NULL)
                {
                    ...
                    if(user_data_p->menu_type == CUI_MENU_TYPE_OPTION)
                    {
                        ...
                        {
                            MMI_TRACE(MMI_FW_TRC_G8_MENU_CUI, MMI_GUI_MENU_CUI_CLOSE_REQUEST_EVT_START);
                            menu_cui_send_event(EVT_ID_CUI_MENU_CLOSE_REQUEST, user_data_p, evt_ext->targ_group);  
                            MMI_TRACE(MMI_FW_TRC_G8_MENU_CUI, MMI_GUI_MENU_CUI_CLOSE_REQUEST_EVT_END);
                        }
        ...
        case EVT_ID_GROUP_DEINIT:
            {   
                menu_cui_send_event(EVT_ID_CUI_MENU_LIST_DEINIT, user_data_p, evt_ext->targ_group);
