...
} cui_sms_send_by_bt_get_list_struct;

extern mmi_sms_bt_cntx_struct *g_sms_bt_ui_p;
extern void cui_sms_pre_device_send_msg();
extern void cui_sms_pre_bt_send_msg();
#endif
...
#ifdef __MMI_BT_MAP_CLIENT__
SMS_HANDLE g_srv_sms_bt_send_handle = 0;
MMI_BOOL g_sms_send_by_bt = MMI_FALSE;
MMI_BOOL g_sms_send_cui_need_device_menu = MMI_TRUE;

cui_sms_send_by_bt_get_list_struct g_cui_sms_get_list_struct;
...
static mmi_ret cui_sms_send_proc(mmi_event_struct* evt)
{
    ...
    cui_phb_pbapc_select_contact_ret_struct* select_ret_result;
#else
    ...
    switch (evt->evt_id)
    {
        ...
        case EVT_ID_PHB_PBAPC_SEL_CONTACT:
            ...
            if (MMI_PHB_PBAPC_SEL_REQ_CLOSE == select_result->rsp_type)
                cui_phb_pbapc_select_contact_close(send_hd->child_gid);
            else
            {
                select_ret_result = (cui_phb_pbapc_select_contact_ret_struct*)select_result->rsp;
        ...
        case EVT_ID_CUI_MENU_LIST_ENTRY:
            {
                ...
                else if (menu_evt->parent_menu_id == MENU_ID_SMS_RECIPIENT_OPT_SEND)
                {
                    ...
                    if (!srv_bt_mapc_adp_is_connected())
                        cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_SMS_SEND_OPT_BY_BT, MMI_TRUE);
                    if (!srv_sim_ctrl_is_available(MMI_SIM1)||
                        !srv_mode_switch_is_network_service_available())
                        cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_SMS_SEND_OPT_BY_SIM1, MMI_TRUE);
                    if (!srv_sim_ctrl_is_available(MMI_SIM2)||
                        !srv_mode_switch_is_network_service_available())
                        cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_SMS_SEND_OPT_BY_SIM2, MMI_TRUE);
                #if (MMI_MAX_SIM_NUM >= 3)
                    if (!srv_sim_ctrl_is_available(MMI_SIM3)||
                        !srv_mode_switch_is_network_service_available())
                        cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_SMS_SEND_OPT_BY_SIM3, MMI_TRUE);
                #if (MMI_MAX_SIM_NUM >= 4)
                    if (!srv_sim_ctrl_is_available(MMI_SIM4)||
                        !srv_mode_switch_is_network_service_available())
                        cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_SMS_SEND_OPT_BY_SIM4, MMI_TRUE);
                #endif
                #endif
                ...
                #else /* (defined __MMI_BT_MAP_CLIENT__) */
                #if (MMI_MAX_SIM_NUM >= 2)
                    if (send_hd->is_direct_send) /* must compiled with __MMI_DUAL_SIM_MASTER__ */
                    {
                        cui_menu_set_currlist_flags(menu_evt->sender_id, CUI_MENU_NORMAL_LIST_WITH_NUMBERED_ICONS);
                    ...
                    #endif /* __MMI_DYNAMIC_SIM_DYNAMIC_UI__ */
                #endif /* MMI_MAX_SIM_NUM >= 2 */
                #endif /* (defined __MMI_BT_MAP_CLIENT__)  */
                }
        ...
        case EVT_ID_CUI_MENU_ITEM_SELECT:
            {
                ...
                #endif /* __MMI_DYNAMIC_SIM_DYNAMIC_UI__ */
                #ifdef __MMI_BT_MAP_CLIENT__  
                    ||(MENU_ID_SMS_SEND_OPT_BY_BT == menu_evt->highlighted_menu_id)  //BT
                #endif
                    )
                {
...
#ifdef __MMI_BT_MAP_CLIENT__
#ifdef __MMI_TELEPHONY_SUPPORT__
    if (srv_sms_is_sms_valid() == MMI_FALSE && MMI_FALSE == srv_bt_mapc_adp_is_connected())
        result = MMI_FALSE;
#else
    if (MMI_FALSE == srv_bt_mapc_adp_is_connected())
        result = MMI_FALSE;
#endif
#else
#ifdef __MMI_TELEPHONY_SUPPORT__
...
static void cui_sms_recipient_highlight_hdlr(S32 index)
{
    ...
    switch (index)
    {
        ...
        #else
        #ifdef __MMI_TELEPHONY_SUPPORT__
            SetCenterSoftkeyFunction(cui_sms_pre_device_send_msg, KEY_EVENT_UP);
        #else
            SetCenterSoftkeyFunction(cui_sms_pre_bt_send_msg, KEY_EVENT_UP);
        #endif
        #endif /* MMI_MAX_SIM_NUM == 2 */
        #endif /* MMI_MAX_SIM_NUM >= 3 */
...
/* under construction !*/
        ...
        #ifdef __MMI_TELEPHONY_SUPPORT__
            //SetKeyHandler(cui_sms_pre_device_send_msg, KEY_SEND, KEY_EVENT_DOWN);
            SetKeyDownHandler(cui_sms_pre_device_send_msg, KEY_SEND);
        #else
            //SetKeyHandler(cui_sms_pre_bt_send_msg, KEY_SEND, KEY_EVENT_DOWN);
            SetKeyDownHandler(cui_sms_pre_bt_send_msg, KEY_SEND);
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
        #endif /* __MMI_DUAL_SIM_MASTER__ */
    #else
        #if (MMI_MAX_SIM_NUM >= 3)
...
/* under construction !*/
            ...
            SetKeyDownHandler(cui_sms_pre_send_msg, KEY_SEND);
        #endif /* __MMI_DUAL_SIM_MASTER__ */
    #endif /* __MMI_BT_MAP_CLIENT__ */
            break;
        }
...
#endif
#else
    handle->child_gid = cui_phb_pbapc_select_contact_create(sms_send_gid);
    if (handle->child_gid != GRP_ID_INVALID)
   {
       cui_phb_pbapc_select_contact_run(handle->child_gid);
   }
#endif
...
void cui_sms_set_send_by_local(MMI_BOOL send_by_local)
{
    g_sms_send_by_bt = !send_by_local;
}

void cui_sms_set_send_cui_need_device_menu(MMI_BOOL need_device_menu)
{
    g_sms_send_cui_need_device_menu = need_device_menu;
}

void cui_sms_pre_bt_send_msg()
{    
...
void cui_sms_pre_device_send_msg()
{
    mmi_id menu_cui_gid;
    mmi_id sms_send_gid;
    cui_sms_send_handle_data_struct *hd_data;
    MMI_BOOL need_cui_menu = MMI_FALSE;
    ...
    if (g_sms_send_cui_need_device_menu)
    {
    ...
    {
        g_sms_send_cui_need_device_menu = MMI_TRUE;
        cui_sms_pre_send_msg();
    }
}

static void cui_sms_recipient_opt_send_by_bt()
{
#if (MMI_MAX_SIM_NUM >= 2)
    cui_sms_set_send_sim_type(SRV_SMS_SIM_1);
#endif
    g_sms_send_by_bt = MMI_TRUE;
    cui_sms_pre_send_msg();
}
...
/* under construction !*/
    ...
    #endif /* __MMI_DYNAMIC_SIM_DYNAMIC_UI__ */ 
    #ifdef __MMI_BT_MAP_CLIENT__
        case MENU_ID_SMS_SEND_OPT_BY_BT:
            cui_sms_recipient_opt_send_by_bt();
            break;
    #endif 
    #else
    #ifdef __MMI_BT_MAP_CLIENT__
...
#ifdef __MMI_BT_MAP_CLIENT__
    if ((MMI_FALSE == g_sms_send_by_bt)&&(sc_number == NULL || strlen((S8*)sc_number) == 0))
#else
...
static void cui_sms_send_msg_fsm_handle(srv_sms_callback_struct *callback_data)
{
    ...
    if (g_sms_send_by_bt)
    {
        if (!cui_sms_send_is_scrn_present(handle->send_gid))
        {
...
void cui_sms_send_by_bt_get_list_callback(srv_sms_callback_struct* callback_data)
{
    cui_sms_send_handle_struct *cui_send_handle;
    
    if (!cui_sms_send_is_scrn_present(g_cui_sms_get_list_struct.sms_send_gid))
    {
        invalid_send_handle = NULL;
        return;
    }
    cui_send_handle = cui_sms_get_send_handle(g_cui_sms_get_list_struct.sms_send_gid);
    #ifdef __MMI_TELEPHONY_SUPPORT__
    if (cui_send_handle->srv_send_handle != NULL)
    {
        srv_sms_free_send_handle(cui_send_handle->srv_send_handle);
        g_cui_sms_get_list_struct.sms_handle = NULL;
        cui_send_handle->srv_send_handle = NULL;
    }
    #endif
	if(callback_data->result == MMI_TRUE)
	{	
	    g_sms_bt_ui_p->last_entered = SRV_SMS_BTMAPC_BOX_UNSENT;
        srv_sms_btmapc_send_msg(g_srv_sms_bt_send_handle, g_cui_sms_get_list_struct.callback, (void*)cui_send_handle);
	}
	else
	{
        srv_sms_btmapc_free_send_handle(g_srv_sms_bt_send_handle);
		mmi_display_popup(GetString(STR_GLOBAL_FAILED), MMI_EVENT_FAILURE);
        mmi_frm_scrn_close(cui_send_handle->send_gid, SCR_ID_SMS_ABORT_SENDING);
        mmi_frm_scrn_close(cui_send_handle->send_gid, SCR_ID_SMS_SENDING);
	}
}
#endif
...
static void cui_sms_send_msg_req(mmi_id sms_send_gid, SrvSmsCallbackFunc callback_func)
{
    ...
    if (hd_data->recipient_info.addr_type == CUI_SMS_ADDR_TYPE_EMAIL_ADDR)
    {
        ...
        srv_sms_btmapc_set_address(g_srv_sms_bt_send_handle, (S8*)srv_sms_get_email_sc_address());        
    #endif
    }
    else
#endif /* __MMI_MESSAGES_SMS_EMAIL__ */ 
    ...
    #ifdef __MMI_BT_MAP_CLIENT__
        g_srv_sms_bt_send_handle = srv_sms_btmapc_get_send_handle();
        srv_sms_btmapc_set_address(g_srv_sms_bt_send_handle, ucs2_addr);        
    #endif
    }
    #ifdef __MMI_TELEPHONY_SUPPORT__
    ...
    #elif defined(__MMI_BT_MAP_CLIENT__)
    cui_sms_set_common_para(hd_data, g_srv_sms_bt_send_handle);
    #endif
    
    {
        ...
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
        #ifdef __MMI_BT_MAP_CLIENT__
            srv_sms_btmapc_set_data_port(g_srv_sms_bt_send_handle, hd_data->src_port, hd_data->dest_port);
        #endif
        }
        ...
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
        #ifdef __MMI_BT_MAP_CLIENT__
            srv_sms_btmapc_set_status_report(g_srv_sms_bt_send_handle, hd_data->status_report);
        #endif
        }
        ...
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
        #ifdef __MMI_BT_MAP_CLIENT__
            srv_sms_btmapc_set_sc_address(g_srv_sms_bt_send_handle, hd_data->sc_address);
        #endif
        }
        ...
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
        #ifdef __MMI_BT_MAP_CLIENT__
            srv_sms_btmapc_set_vp(g_srv_sms_bt_send_handle, hd_data->vp);
        #endif
        }
        ...
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
        #ifdef __MMI_BT_MAP_CLIENT__
            srv_sms_btmapc_set_class_type(g_srv_sms_bt_send_handle, hd_data->class_type);
        #endif
        }
        ...
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
        #ifdef __MMI_BT_MAP_CLIENT__
            srv_sms_btmapc_set_reply_path(g_srv_sms_bt_send_handle, hd_data->reply_path);
        #endif
        }
        ...
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
        #ifdef __MMI_BT_MAP_CLIENT__
            srv_sms_btmapc_set_reply_msg_id(g_srv_sms_bt_send_handle, hd_data->reply_msg_id);
        #endif
        }
        ...
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
        #ifdef __MMI_BT_MAP_CLIENT__
            srv_sms_btmapc_set_send_type(g_srv_sms_bt_send_handle, hd_data->send_type);
        #endif
        }
        ...
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
        #ifdef __MMI_BT_MAP_CLIENT__
            srv_sms_btmapc_set_mms(g_srv_sms_bt_send_handle, MMI_TRUE);
        #endif
        ...
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
        #ifdef __MMI_BT_MAP_CLIENT__
            srv_sms_btmapc_set_mms(g_srv_sms_bt_send_handle, MMI_FALSE);
        #endif
        }
    ...
    if (g_sms_send_by_bt)
    {
        g_cui_sms_get_list_struct.callback = callback_func;
        g_cui_sms_get_list_struct.sms_handle = srv_send_handle;
        g_cui_sms_get_list_struct.sms_send_gid = sms_send_gid; 
        #ifdef __MMI_BT_MAPC_BY_SPP__
        srv_sms_btmapc_get_msg_list(MMI_FALSE, 
            SRV_SMS_BTMAPC_BOX_UNSENT,
            cui_sms_send_by_bt_get_list_callback,
            (void*)&g_cui_sms_get_list_struct);
        #else
            ...
            SMS_HANDLE srv_save_handle)
{
    ...
    #endif /* __MMI_TELEPHONY_SUPPORT__ */
    #ifdef __MMI_BT_MAP_CLIENT__
        srv_sms_btmapc_set_content_dcs(g_srv_sms_bt_send_handle, data->dcs);
    #endif
    }
    ...
    #endif /* __MMI_TELEPHONY_SUPPORT__ */
    #ifdef __MMI_BT_MAP_CLIENT__
        srv_sms_btmapc_set_content(g_srv_sms_bt_send_handle, data->content_ems->textBuffer, 1024);
    #endif
    }
    ...
    #endif /* __MMI_TELEPHONY_SUPPORT__ */
    #ifdef __MMI_BT_MAP_CLIENT__
        srv_sms_btmapc_set_content(g_srv_sms_bt_send_handle, data->content_buff, 1024);
    #endif
    }
    ...
    #endif /* __MMI_TELEPHONY_SUPPORT__ */
    #ifdef __MMI_BT_MAP_CLIENT__
        srv_sms_btmapc_set_content_file(g_srv_sms_bt_send_handle, data->content_file_path);
    #endif
    }
    ...
    #endif /* __MMI_TELEPHONY_SUPPORT__ */
    #ifdef __MMI_BT_MAP_CLIENT__
        srv_sms_btmapc_set_sim_id(g_srv_sms_bt_send_handle, data->sim_id);
    #endif
    }
    ...
    #endif /* __MMI_TELEPHONY_SUPPORT__ */
    #ifdef __MMI_BT_MAP_CLIENT__
        srv_sms_btmapc_set_pid(g_srv_sms_bt_send_handle, data->pid);
    #endif
    }
...
static MMI_BOOL cui_sms_get_send_finish_popup_info(cui_sms_send_handle_struct *handle, U16 *body_str_id, mmi_event_notify_enum *event_id)
{
    ...
    else if(sent_but_save_fail_num == hd_data->send_info.total_num)       
    {
    #ifdef __MMI_BT_MAP_CLIENT__
        if (g_sms_send_by_bt)
            *body_str_id = STR_GLOBAL_SENT;
            ...
            MMI_FRM_SMALL_SCRN))
    {
        ...
        if (g_sms_send_by_bt)   
        {
            ShowCategory66Screen(
                ...
                img_id,
                NULL);
            StartTimer(MESSAGES_INPROGRESS_TIMER_ID, MMI_SMS_UM_BT_PROGRESS_TIME_OUT, cui_sms_sending_time_out);
        }
        else
        {
            ShowCategory66Screen(
...
static void cui_sms_entry_abort_sending(void)
{
            ...
            MMI_FRM_SMALL_SCRN))
    {
        ...
        if (srv_ucm_query_call_count(SRV_UCM_CALL_STATE_ALL, SRV_UCM_CALL_TYPE_NO_CSD, NULL))
        {
            ...
            SetKeyDownHandler(end_key_func, KEY_END);
        }
    }
}
