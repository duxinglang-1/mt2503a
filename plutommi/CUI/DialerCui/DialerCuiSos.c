...
static mmi_ret cui_dialer_sos_check_input_num(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
     ...
     if (cui_dialer_is_active_if_any())
    {
		...
		if((key_evt->evt_id == EVT_ID_PRE_KEY)&&(key_evt->key_type ==KEY_EVENT_DOWN)&&
			((key_evt->key_code >=KEY_0&&key_evt->key_code <=KEY_9)||((key_evt->key_code >=KEY_STAR))||(key_evt->key_code >=KEY_POUND)))
		{
			WCHAR dialer_str[32] = {0};
			WCHAR dialer_str_temp[CUI_DIALER_MAX_DIAL_STR_LEN] = {0};
			kal_wstrncpy(dialer_str_temp, obj->dial_string, CUI_DIALER_MAX_DIAL_STR_LEN);
			
			kal_wsprintf(dialer_str, "%d", key_evt->key_code);
			kal_wstrncat(dialer_str_temp, dialer_str, CUI_DIALER_MAX_DIAL_STR_LEN);

			verify_detail = cui_dialer_is_ecc(dialer_str_temp);
...
static void cui_dialer_sos_on_enter(cui_dialer_obj_struct *obj)
{
	...
	MMI_SCR_SET_KEY_PROC(cui_dialer_sos_check_input_num,NULL);
