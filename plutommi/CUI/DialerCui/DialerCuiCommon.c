...
#if defined(__MMI_VOIP__)
static void cui_dialer_do_send_key_dial_by_voip(void)
{
...
/* under construction !*/
...

    {
		...
		SetKeyLongpressHandler(cui_dialer_do_voice_mail_dial, KEY_1);
    #endif /* __MMI_TELEPHONY_SUPPORT__ */
    }
}
...
extern void dialer_input_dialer_pad_char();
#endif
...
void cui_dialer_set_handler(cui_dialer_obj_struct *obj)
{
    ...
    if (capability->enable_lsk)
    {
			...
			{
				SetLeftSoftkeyFunction(cui_dialer_do_center_key_dial, KEY_EVENT_UP);
			}
		#else /*__MMI_UI_DIALER_PAD_ONLY_KEY__*/
        SetLeftSoftkeyFunction(cui_dialer_do_center_key_dial, KEY_EVENT_UP);
...
#else
		SetCenterSoftkeyFunction(cui_dialer_do_save_contact, KEY_EVENT_UP);
#endif
    }
        ...
        if (obj->type == CUI_DIALER_TYPE_VOIP)
        {
            //SetKeyHandler(
               // cui_dialer_do_send_key_dial_by_voip,
                //KEY_SEND,
                //KEY_EVENT_DOWN);
			SetKeyDownHandler(cui_dialer_do_send_key_dial_by_voip, KEY_SEND);
        }
        else
			#endif
        {
    ...
    if (capability->enable_abbrev_dial)
    {
		...
		SetKeyUpHandler(cui_dialer_do_abbrev_dial, KEY_POUND);
    }
...
/* under construction !*/
    ...
    if (capability->enable_toggle_profile)
    {
        ...
        //    KEY_POUND,
        //    KEY_LONG_PRESS);
		SetKeyLongpressHandler(cui_dialer_do_toggle_silent_profile, KEY_POUND);
    }
        ...
        mmi_search_web_hot_key_is_enabled(MMI_SEARCH_WEB_HOT_KEY_LONG_PRESS_STAR))
    {
		...
		SetKeyLongpressHandler(cui_dialer_do_baidu_search, KEY_STAR);
    }
#endif /* defined(__MMI_SEARCH_WEB__) */
