...
void mdi_gps_set_config(MMI_BOOL supl_en, 
                        U8 *supl_name,
                        U16 supl_port,
                        MMI_BOOL use_tls)
{
    ...
    ilm_struct  *ilm_ptr;
    module_type src_mod;
    gps_supl_ind_struct    *local_param_ptr;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    local_param_ptr = (gps_supl_ind_struct*) construct_local_para(sizeof(gps_supl_ind_struct),TD_CTRL);
    strcpy(local_param_ptr->server, supl_name);
    local_param_ptr->port = supl_port;
    local_param_ptr->use_tls = 1;
    local_param_ptr->account_id = 0;

    src_mod = stack_get_active_module_id();
    ilm_ptr = allocate_ilm(src_mod);
    ilm_ptr->src_mod_id = src_mod;
    ilm_ptr->dest_mod_id = MOD_GPSADAPTOR;
    ilm_ptr->sap_id = MMI_MMI_SAP;
    if (supl_en)
  	{
        ilm_ptr->msg_id = MSG_ID_SUPL_ENABLE;
  	}
  	else
		{
        ilm_ptr->msg_id = MSG_ID_SUPL_DISABLE;
		}
