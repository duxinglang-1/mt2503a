...
VMINT vm_get_sym_entry(char* symbol) 
{
	...
	if(strcmp(symbol,"vm_file_seek") == 0)
		return (VMINT)vm_file_seek;
#endif
#endif
    ...
    if(strcmp(symbol,"vm_graphic_get_img_property") == 0)
        return (VMINT)vm_graphic_get_img_property;        
    ...
    if (pcb->sysEventHandler)
    {
            ...
            || pcb->event_filter->sys_event_filter(msg, param))
        {
			...
			if(resend)
            {
                ...
                vm_log_debug("[SM] _vm_pmng_send_sysevt_msg(%p, %d, %d) - 3", pcb, msg, param);
            }
		}
