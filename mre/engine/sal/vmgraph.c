...
void vm_camera_free_camera_resource(void);
...
static VM_GDI_HANDLE vm_graphic_sal_set_fast_layer(VMINT x, VMINT y, VMINT width, VMINT height, VMINT trans_color_enable, gdi_color trans_color)
{
...
#else
    /* 11C slim, gdi do not has fast layer buffer*/
    if ((calculate_datasize_from_cf(VM_GRAPHIC_COLOR_FORMAT_16, width, height, &data_size) ) < 0 )
        return VM_GDI_FAILED;

    /* allocate App memory from MRE ASM pool */    
    buffer = applib_mem_ap_alloc_framebuffer(APPLIB_MEM_AP_ID_MRE_CORE, data_size);
	...
	if ((ret = gdi_layer_create_using_outside_memory(x, y, width, height, &hdl, buffer, data_size)) == GDI_SUCCEED)
#endif
	{
		if ((handle = set_layer_table(hdl,x,y,width,height,trans_color_enable,trans_color,FALSE,VM_GRAPHIC_COLOR_FORMAT_END,FALSE)) >= 0)
		{
...
static VM_GDI_RESULT vm_graphic_sal_release_layer(gdi_handle salhandle)
{
    ...
    if (layer_trans_tbl[is_fast_layer].handle == salhandle)
        applib_mem_ap_free(layer_trans_tbl[is_fast_layer].buffer);
#endif
	...
	else
	{
		...
		ret = gdi_layer_get_blt_layer_ext(&layers[0], &layers[1], &layers[2], &layers[3]
                                      #ifdef GDI_6_LAYERS
                                          , &layers[4], &layers[5]
                                      #endif
                                          );
		...
		else
		{
			for(i = 0; i < 6; i++)
			{
				if (salhandle == layers[i])
				{
					...
					ret = gdi_layer_set_blt_layer_ext(layers[0], layers[1], layers[2], layers[3]
                                                 #ifdef GDI_6_LAYERS
                                                     , layers[4], layers[5]
                                                 #endif
                                                     );
...
void vm_graphic_flatten_previous_blt_layer_to_base_layer(void)
{
...
#if defined(__MRE_SAL_CAMERA__) && defined(MRE_SUPPORT_CAMERA_FEATURE)
	vm_camera_free_camera_resource();
#endif
	...
	else
	{
		...
		ret = gdi_layer_flatten_with_clipping_ext(base_layer, layers[0], layers[1], layers[2]
		                                      #ifdef GDI_6_LAYERS
		                                          , layers[3], 0
		                                      #endif
		                                          );		
...
static VMINT vm_graphic_async_blt_internal(gdi_handle *layers, VMINT fx1, VMINT fy1, VMINT fx2, VMINT fy2)
{
	...
	gdi_layer_set_source_key(FALSE, 0); 	
	gdi_push_and_set_alpha_blending_source_layer(NULL);
	ret = gdi_layer_flatten_with_clipping_ext(g_vm_blt_cb.async_blt_layer, layers[0], layers[1], layers[2]
                                          #ifdef GDI_6_LAYERS
                                              , layers[3], 0
                                          #endif
                                              );        
		...
		{	
			...
			if (g_vm_blt_cb.async_blt_way == VM_GRAPHIC_FLUSH_WAY_DOUBLE_EX_BUFFER)
			{
				if (g_vm_blt_cb.is_first_blt_flag == 1)
				{
									...
									END_PROTECT_ASYNC_BLT
#endif			
					ret = gdi_layer_blt(layers[0], layers[1], layers[2], layers[3],
											  fx1, fy1, fx2, fy2);			
					g_vm_blt_cb.is_first_blt_flag = 0;
				}
				else
...
VMINT vm_graphic_is_abm_file(VMUINT8 *img_data)
{
	if( img_data[0] == 0x28 && img_data[1] == 0x1)
		return TRUE;
	return FALSE;	
}

VMINT vm_graphic_is_gif_file_mtk_format(VMUINT8 *img_data)
{
	if( img_data[0] == GDI_IMAGE_TYPE_GIF && img_data[8] == 'G' && img_data[9] == 'I' && img_data[10] == 'F')
		return TRUE;
	return FALSE;	
}
...
VMINT vm_graphic_is_jpeg_file_mtk_format(VMUINT8 *img_data)
{
	if( img_data[0] == GDI_IMAGE_TYPE_JPG && img_data[8] == 0xff && img_data[9] == 0xd8 && img_data[10] == 0xff)
		return TRUE;
	return FALSE;	
}
...
static VMINT vm_graphic_get_img_type(VMUINT8 *img_data, VMUINT8 *img_type)
{
    ...
    else if (vm_graphic_is_abm_file(img_data))
    {
        *img_type = GDI_IMAGE_TYPE_ABM;
    }
    else if (vm_graphic_is_gif_file(img_data) || vm_graphic_is_gif_file_mtk_format(img_data))
	{
    ...
    else if (vm_graphic_is_jpeg_file(img_data) || vm_graphic_is_jpeg_file_mtk_format(img_data))
	{
...
static VMINT vm_graphic_decode_internal(vm_graphic_color_famat cf, VMUINT8 *buffer_data , VMINT buffer_len, VMUINT8* img_data, VMINT img_len, VMINT img_width, VMINT img_height,VMUINT8 graphic_type, IHDR_t *ihdr, VMINT resize, VMINT flag)
{
	...
	{
		if ( graphic_type == GDI_IMAGE_TYPE_AB2 || graphic_type == GDI_IMAGE_TYPE_ABM || graphic_type == GDI_IMAGE_TYPE_PNG) 
...
static VMINT gif_get_imgprop(VMUINT8* img_data, VMINT img_len,vm_graphic_imgprop *img_prop)
{
	...
	VMINT img_height = 0;
        ...
        if (vm_graphic_is_gif_file_mtk_format(img_data))
	{
		img_data += 8;
		img_len -= 8;
	}
...
static VMINT vm_graphic_get_imgprop(VMUINT8 img_type, VMUINT8* img_data, VMINT img_len,vm_graphic_imgprop *img_prop)
{
	...
	if ( img_type == GDI_IMAGE_TYPE_AB2 || img_type == GDI_IMAGE_TYPE_ABM || vm_graphic_is_jpeg_file_mtk_format(img_data))
	{
		img_data += 8;
		img_len -= 8;
	}
...
static VMINT vm_graphic_image_decode(VMUINT8 img_type, vm_graphic_color_famat cf, VMUINT8* img_data, VMINT img_len, VMINT flag)
{
	...
	if ( cf == VM_GRAPHIC_COLOR_FORMAT_16 || cf == VM_GRAPHIC_COLOR_FORMAT_24 )
	{
		...
		if ( img_type == GDI_IMAGE_TYPE_AB2  || img_type == GDI_IMAGE_TYPE_ABM) 
...
static VMINT vm_graphic_draw_image_from_memory_internal(VM_GDI_HANDLE dest_layer_handle,VMUINT8* img_data,VMINT img_len,VMINT x,VMINT y,VMINT resized_width,VMINT resized_height,VMBOOL flag, VMINT frame_idx)
{
	...
	if (img_type == GDI_IMAGE_TYPE_AB2  || img_type == GDI_IMAGE_TYPE_ABM || vm_graphic_is_jpeg_file_mtk_format(img_data) || vm_graphic_is_gif_file_mtk_format(img_data))
	{
        img_data += 8;
        img_len -= 8;
	}
...
static VMINT vm_graphic_draw_resized_image_from_file_internal(VM_GDI_HANDLE dest_layer_handle, VMINT x, VMINT y, VMINT resized_width, VMINT resized_height, const VMWSTR filename, VMBOOL flag)
{
	...
	if (flag == TRUE)
	{
		ret = gdi_image_draw_resized_file(x, y, resized_width, resized_height, (VMSTR)filename);
	}
	else
	{
		ret = gdi_image_draw_file(x, y, (VMSTR)filename);
	}
	...
	if (ret != GDI_SUCCEED)
	{
		if (ret == GDI_IMAGE_ERR_UNSUPPORTED_FORMAT)
		    return VM_GDI_ERR_UNSUPPORT_IMAGE_TYPE;
		else
			return VM_GDI_FAILED;
	}
...
VMINT vm_graphic_set_blt_layer(VMINT* layer_handles, VMINT count)
{
    ...
    if(count ==1)
    {
        ret =  gdi_layer_set_blt_layer(SAL_HANDLE(layer_handles[0]),0,0,0);
    }
    else if(count ==2)
    {
        ret =  gdi_layer_set_blt_layer(SAL_HANDLE(layer_handles[0]),SAL_HANDLE(layer_handles[1]),0,0);
    }
    else if(count ==3)
    {
        ret =  gdi_layer_set_blt_layer(SAL_HANDLE(layer_handles[0]),SAL_HANDLE(layer_handles[1]),SAL_HANDLE(layer_handles[2]),0);
    }
        else if(count ==4)
    {
        ret =  gdi_layer_set_blt_layer(SAL_HANDLE(layer_handles[0]),SAL_HANDLE(layer_handles[1]),SAL_HANDLE(layer_handles[2]),SAL_HANDLE(layer_handles[3]));
    }
#ifdef GDI_6_LAYERS
    else if(count ==5)
    {
...
VMINT* vm_graphic_get_blt_layer( VMINT* count)
{
    ...
    gdi_layer_get_blt_layer_ext( &layers[0],&layers[1],&layers[2],&layers[3]
                            #ifdef GDI_6_LAYERS
                                ,&layers[4],&layers[5]
                            #endif
                                );
