...
{
	WCHAR app_name[SRV_MRE_APPMGR_APP_NAME_LEN];
	//char app_dev[SRV_MRE_APPMGR_DEVELOPER_NAME_LEN];
}app_info_struct;
#if defined( __MRE_CUST_PACKAGE_OVERSEA__)
app_info_struct sandbox_slim_app_list[]= {//"MediaTek Inc."
	L"Messenger",
	L"Facebook",
	L"Y! Cricket",
	L"Y! News",
...
#elif defined(__MRE_CUST_PACKAGE_FB21__)
app_info_struct sandbox_slim_app_list[]= {
    L"Messenger",
	L"Facebook",
};
#endif
#if defined( __MRE_CUST_PACKAGE_OVERSEA__)||defined(__MRE_CUST_PACKAGE_FB21__)
vm_is_app_valid_for_sandbox_slim(U16 *app_path)
{
    ...
    if(mmi_ucs2icmp((char *)developer,(char *)(L"MediaTek Inc."))==0)
	{
		...
		for(i=0;i<sizeof(sandbox_slim_app_list)/sizeof(app_info_struct);i++)
		{
			if(mmi_ucs2icmp((char *)name,(char *)(sandbox_slim_app_list[i].app_name))==0)
			{
				return MMI_TRUE;
			}
		}
...
#if defined( __MRE_CUST_PACKAGE_OVERSEA__)||defined(__MRE_CUST_PACKAGE_FB21__)
        if(vm_is_app_valid_for_sandbox_slim((U16 *)filepath)==MMI_FALSE)
		{
			mmi_popup_display((WCHAR*)GetString(STR_MRE_SANDBOX_SLIM_NOT_USE), MMI_EVENT_FAILURE, NULL);
			return;
		}
#endif
