...
VMINT vm_start_mre_app(VMWSTR fileName, vm_mre_app_evt_cb_t vam_cb)
{
	...
	for (i=0; i<MRE_SUPPORT_MAX_APP_NUM; i++)
    {
            mre_mem += g_app_mem_info[i].mem_size;
    }
	#ifdef __MRE_CORE_SM__	
	mre_mem += _vm_sm_get_total_memsize();
    #endif
#endif
    MMI_PRINT(MOD_MRE,TRACE_GROUP_8,"[MRE]start mem %d, %d,%d",total_mem,mre_mem,sysconf.memory_size);
    if (mre_mem + total_mem > sysconf.memory_size)
    {
        error_pos = 3;
        ret = VM_PMNG_NO_MEM;
        goto error;
