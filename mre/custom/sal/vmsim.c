...
VMINT vm_sim_get_home_plmn(VMINT sim, VMCHAR* buffer, VMUINT buffer_size)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 0, sim, __LINE__);
	if (buffer == NULL || buffer_size == 0 || 
	    (sim <= 0 || sim > MMI_SIM_TOTAL))
	{
    	
        MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 1, sim, __LINE__);
		return -2;
	}	
	if (vm_has_sim_card() == FALSE)
	{
        MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 2, sim, __LINE__);
		return -3;
	}
	...
	if (!srv_sim_ctrl_get_home_plmn((mmi_sim_enum)(1<<(sim-1)), (S8 *)ghnum, SRV_MAX_PLMN_LEN + 1))
	{
        MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 3, sim, __LINE__);
	    return -4;
	}
    ...
    strncpy(buffer, ghnum, buffer_size);
#else
    MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 4, sim, __LINE__);
    return -5;
#endif

    MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 5, buffer_size, __LINE__);
...
VMINT vm_nw_info_get_nw_plmn(VMINT sim, VMCHAR* buffer, VMUINT buffer_size)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 10, sim, __LINE__);
    if (buffer == NULL || buffer_size == 0 || 
        (sim <= 0 || sim > MMI_SIM_TOTAL))
    {

        MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 11, sim, __LINE__);
        return -2;
    }	
    if (vm_has_sim_card() == FALSE)
    {
        MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 12, sim, __LINE__);
        return -3;
    }
#ifdef __MMI_TELEPHONY_SUPPORT__
    memset(ghnum, 0x00, SRV_MAX_PLMN_LEN + 1);

    if (!srv_nw_info_get_nw_plmn((mmi_sim_enum)(1<<(sim-1)), (S8 *)ghnum, SRV_MAX_PLMN_LEN + 1))
    {
        MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 13, sim, __LINE__);
        return -4;
    }
    strncpy(buffer, ghnum, buffer_size);
#else
    MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 14, sim, __LINE__);
    return -5;
#endif

    MMI_TRACE(TRACE_GROUP_8, TRC_MRE_VMPAYMENT_195, 15, buffer_size, __LINE__);
    return 0;
}

VMINT vm_get_sim_sym(VMCHAR* func_name)
{
    if(strcmp(func_name,"vm_sim_get_home_plmn") == 0)
        return (VMINT)vm_sim_get_home_plmn;           
    if(strcmp(func_name,"vm_nw_info_get_nw_plmn") == 0)
        return (VMINT)vm_nw_info_get_nw_plmn;           
	return 0;
}
