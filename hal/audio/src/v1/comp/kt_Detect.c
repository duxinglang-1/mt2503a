...
#if defined (__MAGILOUDNESS_SUPPORT__)
...
static struct{
   ...
   uint16         state;
#if defined (__MAGILOUDNESS_SUPPORT__)
   uint16         *pDlPcmBuf_16k;
   uint16         *pDlPcmBuf_8k;
   uint16         pStack_16k[19];
   uint32         DspSphBandFlag;
   uint8          DownSampleFlag;
...
#if defined (__MAGILOUDNESS_SUPPORT__)
void ktDetect_DownSampleInit(void);
void ktDetect_DownSampleUninit(void);

void ktDetect_DownSampleInit(void)
{
    uint8 I;
    kt_detect.DspSphBandFlag = *DSP_SPH_SCH_IMPROVE_CTRL; // Temp
    *DSP_SPH_SCH_IMPROVE_CTRL |= 0x0020; //Setup Speech Scheduler be WB
    
    kt_detect.pDlPcmBuf_16k = (kal_uint16 *)audio_alloc_mem_cacheable(WB_FRAME_LEN << 1); 
    ASSERT(kt_detect.pDlPcmBuf_16k);
    kt_detect.pDlPcmBuf_8k = (kal_uint16 *)audio_alloc_mem_cacheable(WB_FRAME_LEN << 1); 
    ASSERT(kt_detect.pDlPcmBuf_8k);
    
    for (I=0; I<19; I++)
    {
        kt_detect.pStack_16k[I] = 0;	
    }
    kt_detect.DownSampleFlag = 1;
}

void ktDetect_DownSampleUninit(void)
{
    if (kt_detect.DownSampleFlag == 1) {
        if ((kt_detect.DspSphBandFlag & 0x20) != 0x20)
        {	
            *DSP_SPH_SCH_IMPROVE_CTRL &= ~0x20; //Write back to NB
        }
        ASSERT(kt_detect.pDlPcmBuf_16k);
        ASSERT(kt_detect.pDlPcmBuf_8k);
        audio_free_mem( (void **) &kt_detect.pDlPcmBuf_16k);
        audio_free_mem( (void **) &kt_detect.pDlPcmBuf_8k);
        
        kt_detect.DownSampleFlag = 0;
    }
}
...
static void ktDetectDediHisr(void *data)
{
    ...
    *DP2_AUDIO_ASP_TASK4_DM_HANDSHAKE = 0; 
    ...
    {   // Read from uplink
        ...
        if (framesize == WB_FRAME_LEN) {
        	  uint16 * tempAddr;
            int32 I;
            int32 tmp;
            
            // Read into DL_16k memory
            IDMA_ReadFromDSP((kal_uint16 *)kt_detect.pDlPcmBuf_16k, dspAddr, framesize);
            // 16k downsample to 8k
            apm_DownsampleFilter3((kal_uint16 *)kt_detect.pDlPcmBuf_16k, (kal_uint16 *)kt_detect.pDlPcmBuf_8k, kt_detect.pStack_16k, NB_FRAME_LEN<<1); //Sample Rate 16kHz->8kHz
            
            tempAddr = kt_detect.pDlPcmBuf_8k;
            for (I = NB_FRAME_LEN; I > 0; I--)
            {
                tmp = (int32)(int16)*tempAddr++;
