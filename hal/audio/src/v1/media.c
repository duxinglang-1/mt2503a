...
#endif
#if defined (__MAGILOUDNESS_SUPPORT__)
   kal_uint16     *pDlPcmBuf_16k;
   kal_uint16     *pDlPcmBuf_8k;
   kal_uint16     pStack_16k[19];
   kal_uint32     DspSphBandFlag;
   kal_uint8      DownSampleFlag;
#endif
    ...
    {   // all address mode
        ...
        int32 tmp;
          ...
          if (framesize == WB_FRAME_LEN) {
              ...
              IDMA_ReadFromDSP((kal_uint16 *)media.pDlPcmBuf_16k, dspAddr, framesize);
              // 16k downsample to 8k
              apm_DownsampleFilter3((kal_uint16 *)media.pDlPcmBuf_16k, (kal_uint16 *)media.pDlPcmBuf_8k, media.pStack_16k, NB_FRAME_LEN<<1); //Sample Rate 16kHz->8kHz
        	    
              tempAddr = media.pDlPcmBuf_8k;
              for (I = NB_FRAME_LEN; I > 0; I--)
              {
                  tmp = *buf + (int32)(int16)*tempAddr++;
...
void Media_DownSampleInit(void)
{
    uint8 I;
    media.DspSphBandFlag = *DSP_SPH_SCH_IMPROVE_CTRL; // Temp
    *DSP_SPH_SCH_IMPROVE_CTRL |= 0x0020; //Setup Speech Scheduler be WB
    
    media.pDlPcmBuf_16k = (kal_uint16 *)audio_alloc_mem_cacheable(WB_FRAME_LEN << 1); 
    ASSERT(media.pDlPcmBuf_16k);
    media.pDlPcmBuf_8k = (kal_uint16 *)audio_alloc_mem_cacheable(WB_FRAME_LEN << 1); 
    ASSERT(media.pDlPcmBuf_8k);
    
    for (I=0; I<19; I++)
    {
        media.pStack_16k[I] = 0;	
    }
    media.DownSampleFlag = 1;
}

void Media_DownSampleUninit(void)
{
    if (media.DownSampleFlag == 1) {
        if ((media.DspSphBandFlag & 0x20) != 0x20)
        {	
            *DSP_SPH_SCH_IMPROVE_CTRL &= ~0x20; //Write back to NB
        }
        ASSERT(media.pDlPcmBuf_16k);
        ASSERT(media.pDlPcmBuf_8k);
        audio_free_mem( (void **) &media.pDlPcmBuf_16k);
        audio_free_mem( (void **) &media.pDlPcmBuf_8k);
        
        media.DownSampleFlag = 0;
    }
}
