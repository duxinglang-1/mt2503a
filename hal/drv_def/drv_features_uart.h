...
//	#define DRV_UART_DCM_BUG_FIXED// for 6575 bringup
   ...
   #define DRV_UART_TX_RX_CLEAR_BUG  
...
#elif defined(DRV_UART_6250_SERIES)
   #define DRV_UART_BASIC_REG
   #define DRV_UART_VFIFO_EN_REG
   #define DRV_UART1_NO_DMA
   #define DRV_UART2_NO_DMA
   ...
   #define DRV_UART_FRACTIONAL
   #define DRV_UART_DCM_NO_BUG // there is not bug when enable DCM  (liming add --- 3 stage:   stage 1: HW bug,disable DCM when DCM enable   stage2:add a bit to fix this bug   stage3: there is no HW bug )
   //#define DRV_UART_VFIFO_V3//MT6250 use new uart design  and old DMA design
   #define DRV_UART_DMA_VFIFO_CONFIG12   //for MT6250
   #define DRV_UART_DMA_EXTEND
   #define DRV_UART_NEW_UART_AND_OLG_DMA //new uart design + old DMA design
   #define DRV_UART_CLOCK_52M//phase out MT6255 option in uart.c ,use this driver features to define uart clock ,but must kown the new chip's clock when the new chip bring up!!!   
   #define DRV_UART_AUTO_BAUDRATE_BUG// there is a bug on auto baudrate,Rx maybe hang when the DLL and DLM are not 0 before  auto baudrate.//workaround:  clear DLL and DLM before auto baudrate.
   #define DRV_UART_TX_RX_CLEAR_BUG  
	// because of uart tx clear bug:  maybe send dirty byte when clear tx FIFO,   if close FIFO before clear,that's OK
   #define DRV_UART_DMA_ENABLE_HANG_BUG// DMA TX maybe hang when connect META use DMA mode
   ...
   #endif
#elif defined(DRV_UART_6260_SERIES)
   #define DRV_UART_BASIC_REG
   #define DRV_UART_VFIFO_EN_REG
   #define DRV_UART1_NO_DMA
   #define DRV_UART2_NO_DMA
   ...
   #define DRV_UART_FRACTIONAL
   #define DRV_UART_DCM_NO_BUG // there is not bug when enable DCM  (liming add --- 3 stage:   stage 1: HW bug,disable DCM when DCM enable   stage2:add a bit to fix this bug   stage3: there is no HW bug )
   //#define DRV_UART_VFIFO_V3//MT6260 use new uart design  and old DMA design
   #define DRV_UART_DMA_VFIFO_CONFIG13   //for MT6260
   #define DRV_UART_DMA_EXTEND
   #define DRV_UART_NEW_UART_AND_OLG_DMA //new uart design + old DMA design
   #define DRV_UART_CLOCK_52M//phase out MT6255 option in uart.c ,use this driver features to define uart clock ,but must kown the new chip's clock when the new chip bring up!!!   
   #define DRV_UART_AUTO_BAUDRATE_BUG// there is a bug on auto baudrate,Rx maybe hang when the DLL and DLM are not 0 before  auto baudrate.//workaround:  clear DLL and DLM before auto baudrate.
...
#elif defined(DRV_UART_6261_SERIES)
   #define DRV_UART_BASIC_REG
   #define DRV_UART_VFIFO_EN_REG
   #define DRV_UART1_NO_DMA
   #define DRV_UART2_NO_DMA
   ...
   #define DRV_UART_DCM_NO_BUG // there is not bug when enable DCM  (liming add --- 3 stage:   stage 1: HW bug,disable DCM when DCM enable   stage2:add a bit to fix this bug   stage3: there is no HW bug )
   //#define DRV_UART_VFIFO_V3//MT6260 use new uart design  and old DMA design
   #define DRV_UART_DMA_VFIFO_CONFIG14   //for MT6261
   #define DRV_UART_DMA_EXTEND
   #define DRV_UART_NEW_UART_AND_OLG_DMA //new uart design + old DMA design
   #define DRV_UART_CLOCK_52M//phase out MT6255 option in uart.c ,use this driver features to define uart clock ,but must kown the new chip's clock when the new chip bring up!!!   
   #define DRV_UART_AUTO_BAUDRATE_BUG// there is a bug on auto baudrate,Rx maybe hang when the DLL and DLM are not 0 before  auto baudrate.//workaround:  clear DLL and DLM before auto baudrate.
