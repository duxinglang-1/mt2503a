...
#include "drv_features_chip_select.h"
   ...
   */
   ...
   /*
   	when BT and sleep mode are enabled , adc might read small wrong value (ex:vbat = 2v)
   	the option will double check vbat value , if it is not valid (>2.9v) ,reset adc
   */
   #define DRV_ADC_CHECK_IS_VBAT_VALID
  ...
  // #define __CHARGER_WITH_IMMEDIMATE_ADC__
   ...
   #define DRV_ADC_SAMPLE_10bIT	
   #define DRV_ADC_CALIBRATION_EFUSE // For ADC Efuse	
   #define DRV_DIE_TO_DIE_INTERFACE_V2
   #define DRV_MISC_ADC_MEASURE_REMOVE_IRQMASK
   #define DRV_ADC_WAKE_UP_TDMA_TIMER

#elif defined(DRV_ADC_6261_SERIES)
   #define DRV_ADC_BASIC_REG
   #define DRV_ADC_MAX_CH_16
