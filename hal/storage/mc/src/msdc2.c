...
unsigned char MSDC2_EINT_NO;
extern unsigned char MSDC_EINT_NO;
...
void EINT_Callback2(void)
{
    ...
    //
        ...
        cardDetectionEINTPolarity2 = ((cardDetectionEINTPolarity2) ? KAL_FALSE : KAL_TRUE); 
		...
		if(KAL_FALSE == gMSDC2_Handle->mIsPresent){
		    ...
		    MSDC2_SendCardInd(MOD_MMI, 0, MSG_ID_MSDC_CARD_DISPLAY_IND);
		    #endif   
		}
		...
		{
			if (gMSDC2_Handle->msdc_custom_handle.IsCardInd())
				MSDC2_SendCardInd(MOD_FMT, 0, MSG_ID_MSDC_CARD_DETECT_IND);
		}
		else
		{
			MSDC2_SendCardInd(MOD_FMT, 0, MSG_ID_MSDC_CARD_DETECT_IND);
		...
		{
			if (gMSDC2_Handle->msdc_custom_handle.IsCardInd())
				MSDC_SendCardInd(MOD_FMT, 0, MSG_ID_MSDC_CARD_DETECT_IND);
		}
		else
		{
			MSDC2_SendCardInd(MOD_FMT, 0, MSG_ID_MSDC_CARD_DETECT_IND);
