...
{
    kal_uint32 val;
#if defined(MT6250) || defined(MT6260) || defined(MT6261) //Mask SFC AHB Channel 2 and 3 during SFC Mac mode/ Flash busy
    ...
    while (!(SFI_ReadReg32(RW_SFI_MISC_CTL) & SFI_REQ_IDLE));
     ...
     while (!(SFI_ReadReg32(RW_SFI_MISC_CTL) & SFI_REQ_IDLE));
#elif (defined(MT6260) || defined(MT6261) ) &&  !defined(__UBL__) //Wait until SFC AHB Channel 2 become IDLE before enter Macro mode
     while (!(SFI_ReadReg32(RW_SFI_MISC_CTL3) & SFI_CH2_TRANS_IDLE));
     while (!(SFI_ReadReg32(RW_SFI_MISC_CTL) & SFI_REQ_IDLE));
#endif
...
#if (defined(MT6250) &&  !defined(__UBL__)) || ((defined(MT6260) || defined(MT6261) ) &&  !defined(__UBL__))//UnMask SFC AHB Channel 2 and 3 when Flash is not busy
...
#if defined(__TINY_SYS__) //Non-TINY SYS project does not need to enter Deep Power Down in sleep mode
#include "us_timer.h"
#include "csci.h"
extern SF_MTD_Data mtdflash; 
#define DPDReadyTime (100) //Enter and Leave Deep Power Down mode need 100us at most
CSCI_DEFINE_VAR(kal_uint8, NOR_FLASH_LeaveDPDCmd);   //Maui/tiny share memory
CSCI_DEFINE_VAR(kal_uint32, NOR_FLASH_FATStartAddr); //Maui/tiny share memory
CSCI_DEFINE_VAR(kal_uint32, NOR_FLASH_FATStartValue);//Maui/tiny share memory
...
void SF_DAL_DEV_Enter_DPD(void)
{
    ...
    NOR_FLASH_FATStartAddr = ((kal_uint32)D->BaseAddr & 0x1FFFFFFF); //Alice is disabled before enter DPD, access bank1 instead of bank9
    NOR_FLASH_FATStartValue = *(volatile kal_uint32*)NOR_FLASH_FATStartAddr;
...
void SF_DAL_DEV_Leave_DPD(void)
{
    ...
    if(NOR_FLASH_FATStartValue != (*(volatile kal_uint32*)NOR_FLASH_FATStartAddr))
        while(1);

}


#else //Non TINY SYS project does not need to enter Deep Power Down in sleep mode
void SF_DAL_DEV_Enter_DPD(void)
{  
}
