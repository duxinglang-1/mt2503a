...
#pragma arm section code = "SNORCODE"
#include "us_timer.h"
extern SF_MTD_Data mtdflash; 
#define DPDReadyTime (100) //Enter and Leave Deep Power Down mode need 100us at most
kal_uint32 NOR_FLASH_FATStartValue;


void SFUT_DAL_DEV_Enter_DPD(void)
{
    kal_uint32 currtime, nexttime, difftime = 0;
    SF_MTD_Data *D = &mtdflash;
    NOR_FLASH_FATStartValue = *(volatile kal_uint32*)((kal_uint32)D->BaseAddr);
    ...
    while(difftime < DPDReadyTime) //wait 100us for DPD ready
    {
        nexttime=ust_get_current_time();
        difftime=ust_get_duration(currtime, nexttime);
    }      
}

void SFUT_DAL_DEV_Leave_DPD(void)
{
    kal_uint32 currtime, nexttime, difftime = 0;
    SF_MTD_Data *D = &mtdflash;
    kal_uint32 NOR_FLASH_FATStartValue_AfterDPD;
    kal_uint32 NOR_FLASH_FATStartValue_InDPD;

    NOR_FLASH_FATStartValue_InDPD = *(volatile kal_uint32*)((kal_uint32)D->BaseAddr);
    SFI_Dev_Command(D->CS, D->CMD->LeaveDPD);
    ...
    NOR_FLASH_FATStartValue_AfterDPD = *(volatile kal_uint32*)((kal_uint32)D->BaseAddr);
    //FAT start 4 bytes should not be the same in DPD and after DPD
    if(NOR_FLASH_FATStartValue_InDPD == NOR_FLASH_FATStartValue_AfterDPD)
    {
        SFUT_DEBUG("Deep Power Down Test Fail: , Addr: 0x%08X Expect(InDPD): 0x%08X Read(AfterDPD): 0x%08X should not be the same\r\n", (kal_uint32)D->BaseAddr, NOR_FLASH_FATStartValue_InDPD, NOR_FLASH_FATStartValue_AfterDPD);
        ASSERT(0);
    }
    
    //Compare FAT start 4 bytes before and after DPD to make sure DPD ok
    if(NOR_FLASH_FATStartValue != NOR_FLASH_FATStartValue_AfterDPD)
    {
        SFUT_DEBUG("Deep Power Down Test Fail: , Addr: 0x%08X Expect(BeforeDPD): 0x%08X Read(AfterDPD): 0x%08X should be the same\r\n", (kal_uint32)D->BaseAddr, NOR_FLASH_FATStartValue, NOR_FLASH_FATStartValue_AfterDPD);
        ASSERT(0);
    }

}

void SF_DAL_DEV_DPD_TEST(void)
{
//Wait a long time ex: 1s to make flash to enter idle, then test DPD function

  SFUT_DAL_DEV_Enter_DPD();
  SFUT_DAL_DEV_Leave_DPD();
}


void SF_DAL_DEV_DPD_StressTEST(void)
{
//Wait a long time ex: 1s to make flash to enter idle, then test DPD function
  ...
  */
  ...
  for (i=0;sfut_TestPatterns[i].pattern[0]!=PATTERN_END;i++)   {
   ...
   for (prog_len=4; prog_len!=0; prog_len--, address+=PAGE_SIZE) {
        ...
        //MT6260 only have 128byte page progarm, however, 4-byte mode device may have problem during boundary program using 3 byte program
        //Only program 128byte page for 4-byte mode device with 3 byte program test. 
        if((D->AddressCycle == 4) && (prog_len == 3))
        {
            for (i=0; (i+prog_len)<=(PAGE_SIZE/2); i+=prog_len) {
...
#elif defined(MT6255)||defined(MT6250)||defined(MT6280) || defined(MT6260) || defined(MT6261)
...
#if defined(MT6255) || defined(MT6250) || defined(MT6280) || defined(MT6260) || defined(MT6261)
...
#ifdef SFUT_TEST_DPD
        SFUT_DEBUG("Enter/Leave DPD Erase Program Test \r\n");
        SF_DAL_DEV_DPD_TEST();
        SFUT_EraseTest(D, baseaddr);
        SF_DAL_DEV_DPD_TEST();
        SFUT_ProgramTest(D, baseaddr);
...
#ifdef SFUT_TEST_DPD
        SFUT_DEBUG("Enter/Leave DPD ReadTest \r\n");
        SF_DAL_DEV_DPD_TEST();
        SFUT_ReadTest(D, baseaddr-PAGE_SIZE*3);
#endif
#endif
...
#ifdef SFUT_TEST_DPD        
        SFUT_DEBUG("Enter/Leave DPD Stress Test \r\n");       
        SF_DAL_DEV_DPD_StressTEST();
        SFUT_DEBUG("Enter/Leave DPD Stress Test PASS \r\n");  
#endif
...
void SFUT_IntServe(void)
{
   ...
   if(!SFUT_IRQ_ClearReady)
   {
      ...
      #elif (defined(MT6251) || defined(MT6255) || defined(MT6280)  || defined(MT6250) ||defined(MT6260) || defined(MT6261) )
