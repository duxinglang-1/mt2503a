...
/*For MT6250 2CE compatible*/
#define SFI_MAC_SEL                 (0x00000000)   // b28
...
#define SFI_DevMode (SFI_ReadReg32(RW_SFI_DIRECT_CTL)&SFI_QPI_EN)
#define SFI_DevMode_CS1 (SFI_ReadReg32(RW_SFI_DIRECT_CTL2)&SFI_QPI_EN)
...
#define SFI_SetDevMode(CS,x)  do {	\
        if(CS == 0x0) { \
	        if(x != 0) \
             ...
             if(x != 0) \
                SFI_WriteReg32(RW_SFI_DIRECT_CTL2, SFI_ReadReg32(RW_SFI_DIRECT_CTL2) | (SFI_QPI_EN) ); \
             else \
                SFI_WriteReg32(RW_SFI_DIRECT_CTL2, SFI_ReadReg32(RW_SFI_DIRECT_CTL2) & (~(SFI_QPI_EN)) ); \
        } \
	} while(0)
#define SFI_GetWrapMode (SFI_ReadReg32(RW_SFI_MISC_CTL2)&SFI_WRAP_ACC_EN)
//Only for MT6250 MT6260/61
...
#elif (defined(MT6260) || defined(MT6261) ) && !defined(__UBL__) // Bootloader should not toggle Code compressor
#define SFI_MaskAhbChannel(x)  do {	\
	if(x != 0) \
		SFI_WriteReg32(RW_SFI_MISC_CTL3, SFI_ReadReg32(RW_SFI_MISC_CTL3) | (SFI_CH2_TRANS_MASK) ); \
	else \
		SFI_WriteReg32(RW_SFI_MISC_CTL3, SFI_ReadReg32(RW_SFI_MISC_CTL3) & (~(SFI_CH2_TRANS_MASK)) ); \
	} while(0)
