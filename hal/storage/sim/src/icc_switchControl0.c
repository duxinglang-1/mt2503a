...
void SIM_WaitEvent_MT6302(Sim_Card *SIMCARD,kal_uint32 flag, kal_bool unmaskSIMIntr, sim_HW_cb *hw_cb)
{
   ...
   sim_addMsg(0x12345679, flag, SIMCARD->result, retAddr);   
...
void SIM_SetEvent_MT6302(Sim_Card *SIMCARD, kal_uint8 result, sim_HW_cb *hw_cb)
{
	...
	DRV_GET_RET_ADDR(retAddr);
   sim_addMsg(0x12345678, SIMCARD->EvtFlag, drv_get_current_time(), retAddr);
...
void SIM_Reject_MT6302(sim_HW_cb *hw_cb)
{
   ...
   sim_addMsg(0x1234567A, SimCard->EvtFlag, drv_get_current_time(), retAddr);
...
static void SIM_Initialize(kal_uint8 format, kal_uint8 power, sim_HW_cb *hw_cb)
{
	...
	dbg_print("SIM_Initialize power: %d, format: %d, TS_HSK_ENABLE: %d", power, format, SimCard->TS_HSK_ENABLE);
#endif
	...
	if (SimCard->result == SIM_SUCCESS)
	{
		...
		while(1)
		{
			if (!SIM_ProcessATRData(hw_cb))
			{
				...
				{
				   kal_print("RETRY_3V_ON_PTS_ERROR");
					SimCard->Power = SIM_30V;
					SimCard->PTS_check = KAL_TRUE;
					//index = 0;
	...
	if(SimCard->simMagic1 != SimCard->simMagic2){ /*cmd finished before GPT timeout*/
		...
		if(0x60 == SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_SW1_MTK)){
			...
			if(15 < SimCard->sim_nullByteIssueNullCount){
				...
				SIM_DisAllIntr();
				//kal_print("[SIM] : null timeout\n\r");
				DRV_ICC_print(SIM_PRINT_NULL_TIME_OUT, 0, 0, 0, 0, 0);
				 SimCard->timeout = KAL_TRUE;
				SIM_SetEvent_MT6302(SimCard,SIM_NULLTIMEOUT, hw_cb);
			}
			else{
				//kal_print("[SIM] : null byte\n\r");
				DRV_ICC_print(SIM_PRINT_NULL_BYTE, SIM_Reg32(SIM0_BASE_ADDR_MTK + SIM_TOUT_MTK), 0, 0, 0, 0);
			...
			SimCard->sim_nullByteIssueNullCount = 0;
			//kal_print("[SIM] : non-null byte\n\r");
			DRV_ICC_print(SIM_PRINT_NON_NULL_BYTE, SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_SW1_MTK), SIM_Reg32(SIM0_BASE_ADDR_MTK + SIM_TOUT_MTK), 0, 0, 0);
			DRV_ICC_GPTI_StartItem(SimCard->sim_nullByteIssueGPT, 300, sim_nullByteIssueGptTimeoutMT6302, parameter);
		}
   ...
   *rcvSize = SimCard->recDataLen;
   ...
   else
   {
      ...
      SIM_ObtainSW(SW);
	   ...
	   DRV_ICC_print(SIM_PRINT_SIM_CMD_FAIL_STATUS, *(result+2),*(result+3),*(result+4),SIM_Reg(SIM0_BASE_ADDR_MTK + 0x90),SIM_Reg(SIM0_BASE_ADDR_MTK + 0x94));
	   DRV_ICC_print(SIM_PRINT_SIM_CMD_FAIL_STATUS, SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_SW1_MTK),SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_SW2_MTK),SIM_Reg(SIM0_BASE_ADDR_MTK + 0x70),SIM_Reg(SIM0_BASE_ADDR_MTK + 0x74),SIM_Reg(DMA_base + 0x24 + (0x100 * SimCard->sim_dmaport)));
...
static kal_uint16 L1sim_Cmd(kal_uint8  *txData,kal_uint16  txSize,kal_uint8  *result,kal_uint16  *rcvSize, kal_uint8 *Error, sim_HW_cb *hw_cb)
{
   ...
   for(index=0;index<3;index++)
   {
    ...
    if(0x9000 == SW && 0xA4 == txData[1]){
    	//kal_print("[SIM]:0x9000 on select");
