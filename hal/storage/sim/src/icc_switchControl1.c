...
static void usim_activation(sim_HW_cb *hw_cb)
{
	...
	if(SIM_base == hw_cb->mtk_baseAddr){
		...
		SIM_SetBits(0x83010880, 0x0011);
		SIM_SetBits(0x83010888, 0x2);
		kal_sleep_task(20);
		SIM_SetBits(0x83010880, 0x0400);
	}
	else if(SIM2_base == hw_cb->mtk_baseAddr){
		/*enanle VSIM2 to 3V so that we can drive IO low*/
		SIM_SetData(0x83010890, 0x0070, 0x0040);
		SIM_SetBits(0x83010890, 0x0001);
		SIM_SetBits(0x83010898, 0x2);
		kal_sleep_task(20);
		SIM_SetBits(0x83010890, 0x0400);
	}
#elif defined(MT6236)//defined(MT6252) || defined(MT6252H)
	if(SIM_base == hw_cb->mtk_baseAddr){
		/*enanle VSIM to 3V so that we can drive IO low*/
		SIM_SetBits(0x801b0880, 0x0011);
		SIM_SetBits(0x801b0888, 0x2);
		kal_sleep_task(20);
		SIM_SetBits(0x801b0880, 0x0400);
	}
	else if(SIM2_base == hw_cb->mtk_baseAddr){
		/*enanle VSIM2 to 3V so that we can drive IO low*/
		SIM_SetData(0x801b0890, 0x0070, 0x0040);
		SIM_SetBits(0x801b0890, 0x0001);
		SIM_SetBits(0x801b0898, 0x2);
		kal_sleep_task(20);
		SIM_SetBits(0x801b0890, 0x0400);
	}
#elif defined(MT6235) || defined(MT6235B)
	if(SIM_base == hw_cb->mtk_baseAddr){
		/*enanle VSIM to 3V so that we can drive IO low*/
		SIM_SetBits(0x8301080c, 0x2);
		kal_sleep_task(20);
		SIM_ClearBits(0x8301080c, 0x20);
	}
	else if(SIM2_base == hw_cb->mtk_baseAddr){
		/*enanle VSIM2 to 3V so that we can drive IO low*/
		SIM_SetBits(0x83010830, 0x2);
		kal_sleep_task(20);
		SIM_ClearBits(0x83010830, 0x20);
	}
#elif defined(MT6253_3SIM_FTA_PRO3)
	/*turn on MT6306 VSIM4*/
	sim_MT6306_controlPeerSwitchVSIM(hw_cb, 0x3A);
	...
	if(SIM_base == hw_cb->mtk_baseAddr){
		/*enanle VSIM to 3V so that we can drive IO low*/
		SIM_SetBits(0xA0700180, 0x0011);
		SIM_SetBits(0xA0700188, 0x2);
		kal_sleep_task(20);
		SIM_SetBits(0xA0700180, 0x0400);
	}
	else if(SIM2_base == hw_cb->mtk_baseAddr){
		/*enanle VSIM2 to 3V so that we can drive IO low*/
		SIM_SetData(0xA0700190, 0x0070, 0x0040);
		SIM_SetBits(0xA0700190, 0x0001);
		SIM_SetBits(0xA0700198, 0x2);
		kal_sleep_task(20);
		SIM_SetBits(0xA0700190, 0x0400);
	}
#endif
...
#if defined (MT6250)        
   if(SIM_base == hw_cb->mtk_baseAddr){
      SIM_SetData(0xA0700054, 0x0300, 0x0300); //若VSIM1為3V，則需先透過PMU register設定3V(default為1.8V)
   }
#if (2 == SIM_DRV_MTK_INTERFACE_NUM)
   else if(SIM2_base == hw_cb->mtk_baseAddr){
      SIM_SetData(0xA0700054, 0x3000, 0x3000); //若VSIM1為3V，則需先透過PMU register設定3V(default為1.8V)
   }
#endif
   ...
   if(SIM_base == hw_cb->mtk_baseAddr){
      SIM_SetData(0xA0700050, 0x0300, 0x0300); //若VSIM1為3V，則需先透過PMU register設定3V(default為1.8V)
   }
#if (2 == SIM_DRV_MTK_INTERFACE_NUM)
   else if(SIM2_base == hw_cb->mtk_baseAddr){
      SIM_SetData(0xA0700050, 0x3000, 0x3000); //若VSIM1為3V，則需先透過PMU register設定3V(default為1.8V)
   }
#endif
   ...
   if(SIM_base == hw_cb->mtk_baseAddr){
	DRV_ICC_PMU_setVolt2(0, CLASS_B_30V);
      //SIM_ClearBits(0xA0700050, 0x0100); //若VSIM1為3V，則需先透過PMU register設定3V(default為1.8V)
   }
#if (2 == SIM_DRV_MTK_INTERFACE_NUM)
   else if(SIM2_base == hw_cb->mtk_baseAddr){
   	DRV_ICC_PMU_setVolt2(1, CLASS_B_30V);
      //SIM_ClearBits(0xA0700050, 0x1000); //若VSIM2為3V，則需先透過PMU register設定3V(default為1.8V)
   }
#endif
#endif 
...
#endif
	if(KAL_FALSE == sim_MT6302_QuerySIMActive(hw_cb->MT6302PeerInterfaceCb)){
		sim_MT6302_addMsg(0x11030801, hw_cb->simInterface, hw_cb->mtk_baseAddr, 0);

#if defined(MT6252) || defined(MT6252H)
	if(SIM_base == hw_cb->mtk_baseAddr){
		SIM_ClearBits(0x83010880, 0x0001);
		kal_sleep_task(10);
		SIM_ClearBits(0x83010880, 0x0400);
	}
	else if(SIM2_base == hw_cb->mtk_baseAddr){
		SIM_ClearBits(0x83010890, 0x0001);
		kal_sleep_task(10);
		SIM_ClearBits(0x83010890, 0x0400);
	}
#elif defined(MT6236) //defined(MT6252) || defined(MT6252H)
	if(SIM_base == hw_cb->mtk_baseAddr){
		SIM_ClearBits(0x801b0880, 0x0001);
		kal_sleep_task(10);
		SIM_ClearBits(0x83010880, 0x0400);
	}
	else if(SIM2_base == hw_cb->mtk_baseAddr){
		SIM_ClearBits(0x801b0890, 0x0001);
		kal_sleep_task(10);
		SIM_ClearBits(0x801b0890, 0x0400);
	}
#elif defined(MT6235) || defined(MT6235B)
	if(SIM_base == hw_cb->mtk_baseAddr){
		/*enanle VSIM to 3V so that we can drive IO low*/
		SIM_ClearBits(0x8301080c, 0x2);
		kal_sleep_task(20);
		SIM_SetBits(0x8301080c, 0x20);
	}
	else if(SIM2_base == hw_cb->mtk_baseAddr){
		/*enanle VSIM2 to 3V so that we can drive IO low*/
		SIM_ClearBits(0x83010830, 0x2);
		kal_sleep_task(20);
		SIM_SetBits(0x83010830, 0x20);
	}
#elif defined(MT6253_3SIM_FTA_PRO3)
	/*turn off MT6306 VSIM4*/
	sim_MT6306_controlPeerSwitchVSIM(hw_cb, 0x30);
#elif defined(MT6256) || defined(MT6255)
   /* FTA 27.17.2.5 */
	if(SIM_base == hw_cb->mtk_baseAddr){
		SIM_ClearBits(0x801b0880, 0x0001);
		kal_sleep_task(10);
		SIM_ClearBits(0x801b0880, 0x0400);
	}
	else if(SIM2_base == hw_cb->mtk_baseAddr){
		SIM_ClearBits(0x801b0890, 0x0001);
		kal_sleep_task(10);
	...
	if(SIM_base == hw_cb->mtk_baseAddr){
		/*enanle VSIM to 3V so that we can drive IO low*/
		SIM_ClearBits(0xA0700180, 0x0001);
		kal_sleep_task(10);
		SIM_ClearBits(0xA0700180, 0x0400);
	}
	else if(SIM2_base == hw_cb->mtk_baseAddr){
		/*enanle VSIM2 to 3V so that we can drive IO low*/
		SIM_ClearBits(0xA0700190, 0x0001);
		kal_sleep_task(10);
		SIM_ClearBits(0xA0700190, 0x0400);
	}
#endif
	}
	else{
		sim_MT6302_addMsg(0x11030802, hw_cb->simInterface, 0, 0);
	}
