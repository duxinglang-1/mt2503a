...
*   external_global
*/
static void SIM_EINT_HISR_RM_H(void)
{
   ...
   if (simHwCb->polarity == LEVEL_HIGH)
   {
      ...
      if (simHwCb->simHotPlugOut != NULL)
	   ...
	   kal_print("remove SIM");
	   simHwCb->polarity = LEVEL_LOW;
		EINT_Registration(SIM_HOT_SWAP_EINT_NO, KAL_TRUE, simHwCb->polarity, SIM_EINT_HISR_RM_H, KAL_FALSE);
		EINT_Set_Sensitivity(SIM_HOT_SWAP_EINT_NO, KAL_FALSE);
		EINT_SW_Debounce_Modify(SIM_HOT_SWAP_EINT_NO,simHwCb->debounceTime);// 32768Hz, set debounce time
   }
   else
   {
      ...
      if (simHwCb->simHotPlugIn != NULL)
	   ...
	   kal_print("insert SIM");
	   simHwCb->polarity = LEVEL_HIGH;
		EINT_Registration(SIM_HOT_SWAP_EINT_NO, KAL_TRUE, simHwCb->polarity, SIM_EINT_HISR_RM_H, KAL_FALSE);
		EINT_Set_Sensitivity(SIM_HOT_SWAP_EINT_NO, KAL_FALSE);
		EINT_SW_Debounce_Modify(SIM_HOT_SWAP_EINT_NO,simHwCb->debounceTime);// 32768Hz, set debounce time	   
   }   
...
*   external_global
*/
static void SIM_EINT_HISR_RM_L(void)
{
   /* If we have more than 1 EINT, we should 1(HISR)-1(EINT) mapping */
   sim_HW_cb *simHwCb = (sim_HW_cb *)(hwCbArray[sim_get_logicalNum_from_app(regedHotSwapApp)]);

   if (simHwCb->polarity == LEVEL_LOW)
   {
      ...
      if (simHwCb->simHotPlugOut != NULL)
	   ...
	   kal_print("remove SIM");
      simHwCb->polarity = LEVEL_HIGH;
		EINT_Registration(SIM_HOT_SWAP_EINT_NO, KAL_TRUE, simHwCb->polarity, SIM_EINT_HISR_RM_L, KAL_FALSE);
		EINT_Set_Sensitivity(SIM_HOT_SWAP_EINT_NO, KAL_FALSE);
		EINT_SW_Debounce_Modify(SIM_HOT_SWAP_EINT_NO,simHwCb->debounceTime);// 32768Hz, set debounce time
   }
   else
...
#endif 
DRV_ICC_interface_init(SIM_ICC_APPLICATION application)
{
	sim_HW_cb	*hw_cb;
	...
	{ 			 
		...
		switch((value >> 4))
		{
			case 1:
			   /* Get polarity setting for REMOVE */
				...
				kal_sprintf(sim_dbg_str,"[SIM_DRV]EINT INFO:%x, EINTNo:%d, Pol:%d, GPIOSP:%x, simIf;%x, value:%x",SIM_PRINT_INFO_BASE, SIM_HOT_SWAP_EINT_NO, simHwCb->polarity, SIM_Reg(0x80020220), simInterface, value);
				kal_print(sim_dbg_str);
			...
			case 8:				
			default:
				kal_sprintf(sim_dbg_str,"[SIM_DRV]EINT2 INFO:%x, EINTNo:%d, Pol:%d, GPIOSP:%x, simIf;%x, value:%x",SIM_PRINT_INFO_BASE, SIM_HOT_SWAP_EINT_NO, simHwCb->polarity, SIM_Reg(0x80020220), simInterface, value);
				kal_print(sim_dbg_str);
		...
		EINT_Set_Sensitivity(SIM_HOT_SWAP_EINT_NO, KAL_FALSE);
		EINT_SW_Debounce_Modify(SIM_HOT_SWAP_EINT_NO,simHwCb->debounceTime);// 32768Hz, ³]©wdebounce time
	}
#endif    
