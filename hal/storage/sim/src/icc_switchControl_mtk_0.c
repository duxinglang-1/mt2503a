...
/*old method, just copy here and will change it when necessary*/
   ...
   if(SIM_base == hw_cb->mtk_baseAddr){
      ...
      SIM_SetData(0xA0700054, 0x3000, 0x3000); //若VSIM2為3V，則需先透過PMU register設定3V(default為1.8V)
   }
#endif
#elif defined (MT6260)   
   ...
   if(SIM_base == hw_cb->mtk_baseAddr){
      ...
      SIM_SetData(0xA0700050, 0x3000, 0x3000); //若VSIM2為3V，則需先透過PMU register設定3V(default為1.8V)
   }
#endif
#else
   if(SIM_base == hw_cb->mtk_baseAddr){
	...
	else if (power == SIM_18V)
	{
#if defined (MT6250)        
   if(SIM_base == hw_cb->mtk_baseAddr){
      SIM_ClearBits(0xA0700054, 0x0100); //若VSIM1為3V，則需先透過PMU register設定3V(default為1.8V)
   }
#if (2 == SIM_DRV_MTK_INTERFACE_NUM)
   else if(SIM2_base == hw_cb->mtk_baseAddr){
      SIM_ClearBits(0xA0700054, 0x1000); //若VSIM2為3V，則需先透過PMU register設定3V(default為1.8V)
   }
#endif
#elif defined (MT6260)        
   if(SIM_base == hw_cb->mtk_baseAddr){
