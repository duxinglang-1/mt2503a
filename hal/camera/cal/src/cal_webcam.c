    ...
    if (CAL_CTRL_CODE_START & CtrlCode)
    {
        ...
        {  
            ...
            if(!IsHwJpegEncSupport)       /// for SW JPEG Encoder platform
            {
                ...
                // need to use non-cacheable buffer
...
// Jpeg working buffer --> cacheable            
            JpegWrapper.extMemoryAddr=(void *)(((CalWebcamProcessStruct.ExtmemStartAddress2 + WEBCAM_YUV_IMAGE_BUFFER_SIZE + IDP_MEM_CONSUME_CAMERA_CAPTURE_TO_JPEG_EXTMEM)+3)&0xFFFFFFFC);//JPEG 4 byte align
