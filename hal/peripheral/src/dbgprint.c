...
char ble_print_buf[MAXCHARS];
#define USB_DBG_PRINT_PORT	4	 // Use USB as debug port
...
void dbg_print(char *fmt,...)
{
	...
	*bp= 0;
	...
	if(PS_UART_PORT == 4)
	{
		...
		handle = DclSerialPort_Open(USB_DBG_PRINT_PORT,0);

		DclSerialPort_Control(handle, SIO_CMD_GET_OWNER_ID, (DCL_CTRL_DATA_T*)&port_owner);
		BMTPutBytes.puBuffaddr = (kal_uint8 *)print_buf;
		BMTPutBytes.u2Length = len;
		BMTPutBytes.u4OwenrId = port_owner.u4OwenrId;
		DclSerialPort_Control( handle,SIO_CMD_PUT_BYTES,(DCL_CTRL_DATA_T*)&BMTPutBytes);
	}
       else 
       	{
		len = (unsigned short)(bp - print_buf);
	...
	va_end (ap);
#if 0//defined(__GGT_WRITE_LOG_TO_TCARD__)
	   {
		   GT_Log2File(print_buf,len);
	   }
#endif
