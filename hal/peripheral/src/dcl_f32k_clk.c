...
DCL_STATUS DclF32K_Control(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
    ...
    switch (cmd)
    {
        ...
        case F32K_CLK_CMD_SET_EMB_OSC32_MODE:
        {
            F32K_CLK_CTRL_SET_EMB_OSC32_MODE_T *prSetEmbOsc32Mode;
            prSetEmbOsc32Mode = &(data->rSetEmbOsc32Mode);
            return_status = F32K_Set_Emb_OSC32_Mode(prSetEmbOsc32Mode->osc32_mode);
        }
        break;

        case F32K_CLK_CMD_SET_EOSC_CALI_TD:
        {
            F32K_CLK_CTRL_SET_EOSC_CALI_TD_T *prSetEoscCaliTd;
            prSetEoscCaliTd = &(data->rSetEoscCaliTd);
            return_status = F32K_Set_EOSC_CALI_TD(prSetEoscCaliTd->td);
        }
        break;

        case F32K_CLK_CMD_SET_EOSC_CALI_RUN_TIME_EN:
        {
            F32K_CLK_CTRL_SET_EOSC_CALI_RUN_TIME_EN_T *prSetEoscCaliRunTimeEn;
            prSetEoscCaliRunTimeEn = &(data->rSetEoscCaliRunTimeEn);
