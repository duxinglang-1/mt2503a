...
static void bmt_chr_indication(ilm_struct *current_ilm)
{
	...
	kal_bool cable_status;

	//chr_type = bmt_get_chr_type();		
	chr_type = current_ilm->sap_id;
	kal_prompt_trace(MOD_BMT,"chr_type is %d",chr_type);
	if(chr_type == PW_NO_CHR)
		cable_status = CHARGER_PLUG_OUT;
	else
		cable_status = CHARGER_PLUG_IN;			
    ...
    if (cable_status == CHARGER_PLUG_IN)
    {
    ...
    else if(cable_status == CHARGER_PLUG_OUT)
    {
   ...
   while(1)
   {
      ...
      switch(current_ilm.msg_id)
      {
         ...
         case MSG_ID_BMT_CHARGER_IND:
			...
			bmt_chr_indication(&current_ilm);
         	break;
