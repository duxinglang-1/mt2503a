...
BEGIN_TRACE_MAP(MOD_BMT)
    ...
    TRC_MSG(BMT_TOP_OFF_TIMER_EXPIRE_TRC,"Top Off timer(3 hr) expired.")  
    TRC_MSG(BMT_TOP_OFF_TIMER_START_TRC,"Top Off timer(3 hr) starts.")  
    TRC_MSG(BMT_TOP_OFF_TIMER_STOP_TRC,"Top Off timer(3 hr) stops.")  	
	...
	TRC_MSG(BMT_RECHARGE_START_TRC,"ReCharge and Chage state to CHR_FAST.")
	TRC_MSG(BMT_CHARGE_FULL_TRC,"Charging Full, current = %d.")
	...
	TRC_MSG(BMT_SW_ENABLE_SLEEP_MODE,"Enable Sleep mode!")
	TRC_MSG(BMT_SW_DISABLE_SLEEP_MODE,"Disable Sleep mode!")
	...
	TRC_MSG(BMT_PHY_CHECK_RECHARGE_TEMPER_COUNT_TRC, "recharge_temper_count = %d.")
	...
	TRC_MSG(BMT_ADC_SW_RESET_TRC,"ADC SW Reset, count=%d")
	TRC_MSG(BMT_ADC_BUSY_STATUS_TRC,"ADC CON0=%d, busy=%d, retry=%d")
	TRC_MSG(BMT_ADC_RAW_DATA,"ADC vbat adc=%d vbatVol=%d Channel=%d adc=%d ")
	TRC_MSG(BMT_ADC_INVALD_VBAT_RESET,"INVALID VBattery voltage , reset adc %d")
	
	// SW Fuel Gauge
	TRC_MSG(BMT_OAM_DELTA_TIME,"last time =%d systick, now time=%d systick, delta time=%d second ")
	TRC_MSG(BMT_OAM_IMP_TRACKING,"temp_voltage_2=%d,temp_voltage_1=%d,ret_compensate_value=%d,resistance_bat=%d ")
	TRC_MSG(BMT_OAM_INIT_INF,"hw_ocv=%d, hw_ocv_capacity=%d, rtc=%d, D0=%d, oam_v_ocv_init=%d, battemp=%d")
	TRC_MSG(BMT_OAM_INIT_RAW_INF,"%d, %d, %d, %d, %d, %d")
	TRC_MSG(BMT_OAM_RUN_INF1,"ocv1=%d, ocv2=%d, vbat=%d, i1=%d, i2=%d, r1=%d, r2=%d")
	TRC_MSG(BMT_OAM_RUN_RAW_INF1,"%d, %d, %d, %d, %d, %d, %d")
	TRC_MSG(BMT_OAM_RUN_INF2,"car1=%d, car2=%d, bat_capacity=%d, D0=%d")
	TRC_MSG(BMT_OAM_RUN_RAW_INF2,"%d, %d, %d, %d")
	TRC_MSG(BMT_OAM_RESULT_INF,"D1=%d, D2=%d, D3=%d, D4=%d, D5=%d, SOC=%d,UI_SOC=%d")
	TRC_MSG(BMT_OAM_RESULT_RAW_INF,"%d, %d, %d, %d, %d, %d, %d")
	TRC_MSG(BMT_OAM_R_V_TABLE,"<Rbat,VBAT> at %d = <%d,%d> ")
	TRC_MSG(BMT_OAM_DOD_V_TABLE,"<DOD,Voltage> at %d = <%d,%d>")
	TRC_MSG(BMT_OAM_FULL_RESET,"charging full and reset meter")
	TRC_MSG(BMT_OAM_100_TRACKING,"100 tracking, UI_SOC=%d,resetMeter=%d")
	TRC_MSG(BMT_OAM_0_TRACKING,"0 tracking, UI_SOC=%d,resetMeter=%d")
	TRC_MSG(BMT_OAM_RESET_DOD,"UI_SOC=%d, D0=%d")
	TRC_MSG(BMT_OAM_RECHARGE,"Keep UI as 100, UI_SOC=%d")
	TRC_MSG(BMT_OAM_SYNC_SOC,"Sync UI percentage to Real one, UI_SOC=%d, SOC=%d, counter=%d")
	TRC_MSG(BMT_OAM_QMAX_UPDATE,"Qmax update, gFG_columb=%d, D0=%d, Flag=%d, Capacity=%d")
	TRC_MSG(BMT_OAM_SOC_INIT,"RTC_SOC=%d, HW_OCV_SOC=%d, VBAT_SOC=%d")
	TRC_MSG(BMT_OAM_GET_QMAX,"[fgauge_get_Q_max] Q_max = %d")
	TRC_MSG(BMT_OAM_CONSTRUCT_TABLE,"reconstruct table by temperature change from (%d) to (%d)")
	...
	TRC_MSG(BMT_MSG_ADC_PARAM_SLOPE_OFFSET,"BMT_MSG_ADC_PARAM_SLOPE_OFFSET slope: %d offset: %d")
	...
	TRC_MSG(BMT_HIGH_VCHG_ENABLED_TRC,"High Voltage Charger is enabled")	
