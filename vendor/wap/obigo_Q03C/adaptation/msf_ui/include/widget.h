...
#ifdef __MMI_WAP_ANIMATED_GIF_SUPPORT__
#define WIDGET_ANIMATED_GIF_SUPPORT
#endif
//#define WIDGET_DRAW_STATUS_BAR  /* Remove after successfull test */
...
#define WIDGET_STORE_WCACHE_IN_INTERNAL_ADM
#endif

#ifdef WIDGET_STORE_WCACHE_IN_INTERNAL_ADM
#define WIDGET_MODID_INT_ADM (MSF_NUMBER_OF_MODULES)
extern void* widget_internal_adm_malloc(int size);
extern void widget_internal_adm_free(void *ptr);
#endif

#ifdef WIDGET_STORE_WCACHE_IN_INTERNAL_ADM
#define WIDGET_MODID_MAX_INT_ADM (WIDGET_MODID_INT_ADM+1)
#else
#define WIDGET_MODID_MAX_INT_ADM (MSF_NUMBER_OF_MODULES)
#endif
...
#endif /* GDI_USING_HW_JPEG || GDI_USING_HW_JPEG_V2 */ 

#define __WIDGET_MDI_RESIZER_LIMITATION_EVEN_WIDTH_HEIGHT__
...
#if (defined(__MMI_NONBLOCKING_DECODE__) && defined(WIDGET_LARGE_RESOLUTION))
/*
...
#define WIDGET_VIDEO_TWO_LAYER_SUPPORT
...
#ifdef OBIGO_Q03C_MMS_V01
#define WIDGET_BROWSER_V2_WITH_MMS_V01   // Code that is needed to run MMS with Browser V02
#else
#define WIDGET_BROWSER_V2_ONLY
#endif
...
#ifndef WIDGET_STORE_WCACHE_IN_INTERNAL_ADM
 ...
 * *
 */
 /* Changed the value of paint timer from 5 to 10 because the paint timer expiry very fast 
    if timer stop and start event is ongoing so fast, then it might be possible that the previous timer 
    expiry primitive is not processed while another is also started so there might be WAP external queue full happened
     */
#define WIDGET_PAINT_DELAY_TIME              (10)	/* Original value is 100 */
...
/* add mechanism to reduce accessing the file system when create image resource at the background */
#define WIDGET_RAW_FILE_CHECK_TIME                  (500)
...
enum
{
    ...
    //WIDGET_MMS_VIEWER_SCROLL_TIMER,  MAUI_03188709
    WIDGET_PAINTBOX_RELAYOUT_TIMER, /* MAUI_03188709 */
    WIDGET_PAINTBOX_SCROLL_INDICATION_TIMER,
    WIDGET_PIPE_NOTIFY_IMAGE_DELAY_TIMER,
    WIDGET_PIPE_NOTIFY_SOUND_DELAY_TIMER,
    WIDGET_RAW_FILE_IMAGE_CHECK_TIMER,
    WIDGET_TRANSITON_ANIMATION_TIMER,
    WIDGET_MAX_TIMER
};
...
/******************************************************************************
 * Internal configuration and MACRO
 * Widget uses two images for Mouse Pointer Support. One is Mouse Pointer and Other is LinkSelect image (Hand Image). 
 * It might happen that tip of mouse pointer or hand tool doesn't start at (0,0). So in that case to handle drawing errors 
 * these macros have been used. Currently we assume that mouse pointer tip will always be at (0,0) so WIDGET_MOUSE_POINTER_X_OFFSET
 * & WIDGET_MOUSE_POINTER_Y_OFFSET are taken as 0. But hand tool tip starts at (4,0). So WIDGET_LINK_SELECT_TOOL_X_OFFSET is taken as 
 * 4 and WIDGET_LINK_SELECT_TOOL_Y_OFFSET is taken as 0.
 ******************************************************************************/
#ifdef __MMI_WAP_MOUSE_POINTER_SUPPORT__
#define WIDGET_MOUSE_POINTER_X_OFFSET  0
#define WIDGET_MOUSE_POINTER_Y_OFFSET  0
#define WIDGET_LINK_SELECT_TOOL_X_OFFSET  4
#define WIDGET_LINK_SELECT_TOOL_Y_OFFSET  0
#endif
...
/* Used to store the alpha layer and again reset when we exit from widget screen or window */
extern gdi_handle g_old_alpha_layer;
#endif
...
/* under construction !*/
#endif
    /* Focus the paintbox but not blt the screen */
    unsigned int focus_paintbox_without_painting:1;
...
/* under construction !*/
    ...
    /* to know if in back history process */
    kal_bool is_back_history;
...
#ifdef  __MMI_WAP_MOUSE_POINTER_SUPPORT__
    MsfPosition mouse_position;
    MsfSize mouse_pointer_size;
    MsfSize link_select_tool_size;
    MsfSize gdi_layer_size;
    U32 mouse_pointer_handle;
    U32 link_select_tool_handle;
    gdi_handle gdi_layer_handle;
    U32 layer_memory_handle;
    kal_bool is_link_select_image;
    kal_bool is_dialog_focused;
...
/* under construction !*/
    ...
    kal_bool play_tone_and_vibration;
    kal_bool play_only_vibration;   
    kal_bool is_popup_displayed;
...
/* under construction !*/
    ...
    U32 transition_animation_for_MMS;
...
typedef struct
{
...
#ifdef WIDGET_STORE_WCACHE_IN_INTERNAL_ADM
    widget_header_struct *free_obj_list[WIDGET_MODID_MAX_INT_ADM];
#else
    ...
    int pn_image_delay_cnt;
    int pn_image_delay_handle[MAX_WIDGET_PIPE_NOTIFY_SUM/2];
    int pn_image_delay_eventType[MAX_WIDGET_PIPE_NOTIFY_SUM/2];
    int pn_sound_delay_cnt;
    int pn_sound_delay_handle[MAX_WIDGET_PIPE_NOTIFY_SUM/2];
    int pn_sound_delay_eventType[MAX_WIDGET_PIPE_NOTIFY_SUM/2];
...
#endif 
#ifdef __COSMOS_MMI_PACKAGE__
	widget_coordinate_struct coordinate_cntx;
#endif
...
extern void widget_copy_editor_buffer(kal_uint8 *dst, kal_uint8 *src);
...
extern void widget_draw_image_from_queued_op(
                ...
                kal_int16 y_offset);
#ifdef WAP_USE_ASM
extern void widget_draw_op_release(widget_draw_op_struct *draw_op, MSF_UINT8 mod_id);
#else
...
extern void HDIa_widgetExtFreeMarqueeElements(widget_image_struct *img);
...
#ifdef OBIGO_Q03C_MMS_V01
extern void widget_paint_ctk_action(widget_header_struct *parent);

extern void widget_ctk_show_msf_menu(ctk_layout_handle layoutid);
extern void widget_ctk_msf_menu_highlight(ctk_screen_handle scrid, int index);
#endif /* OBIGO_Q03C_MMS_V01 */

extern void widget_ctk_msf_menu_RSK_hdlr(ctk_screen_handle scrid);
extern void widget_ctk_msf_menu_LSK_hdlr(ctk_screen_handle scrid);

extern int widget_get_string_id(MsfStringHandle winHandle);
#ifdef OBIGO_Q03C_MMS_V01
extern ctk_app_handle widget_ctk_appid;
#endif /*  OBIGO_Q03C_MMS_V01 */
 ...
 * V2 Browser
 ****************************/
extern void HDIa_widgetExtResumeRendering(MSF_UINT8 flag);
extern void HDIa_widgetDeviceSetProperties(
        U32 x,
        U32 y,
        U32 w,
        U32 h,
...
extern void widget_paintboxV2_register_keys(void);
extern void widget_paintboxV2_pen_handler(mmi_pen_point_struct point, mmi_pen_event_type_enum type);
extern U16 widget_get_screen_id(void);

extern void widget_common_exit(void);

#ifdef WAP_USE_ASM
extern void* widget_internal_mem_alloc(MSF_UINT8 mod_id, MSF_UINT32 no_of_bytes);
extern void widget_internal_mem_free(MSF_UINT8 mod_id, void * obj_ptr);
#endif

extern void widget_MMI_refresh_screen(void);
...
#define BROWSER_Q03C_DOUBLE_BUFFER
...
#ifdef __COSMOS_MMI_PACKAGE__
extern void widget_blit_render_layer (S32 x1, S32 y1, S32 x2, S32 y2);
extern gdi_handle paintbox_layer_handle;
extern void widget_set_active_layer(void);
extern void widget_reset_active_layer(void);
#endif
