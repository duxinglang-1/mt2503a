...
#else
	{"/mmt", MMS_STORAGE_DEFAULT, "\\@mms_1", 1},
	{"/mms_sys", MMS_STORAGE_DEFAULT, "\\@mms_1\\mms_sys", 1},
	{"/mms_pdu", MMS_STORAGE_DEFAULT, "\\@mms_1\\mms_pdu", 1},
	{"/mms_info", MMS_STORAGE_DEFAULT, "\\@mms_1\\mms_info", 1},
...
#if defined(__MMI_WAP_IMAGE_CACHE_SYSTEM_DRIVE__)
    {WIDGET_RAWFILE_DIRECTORY, WAP_STORAGE_SYSTEM, "\\@wcache\\", 0},
    {WIDGET_RAWFILE_MMS_DIRECTORY, WAP_STORAGE_SYSTEM, "\\@wcache\\", 0},
#elif defined(__MMI_WAP_IMAGE_CACHE_PHONE_DRIVE__)
    {WIDGET_RAWFILE_DIRECTORY, WAP_STORAGE_PUBLIC, "\\@wcache\\", 0},
    {WIDGET_RAWFILE_MMS_DIRECTORY, WAP_STORAGE_PUBLIC, "\\@wcache\\", 0},
#else 
...
void gmms_hdi_file_set_ext_drive(char *src,msf_wap_storage_enum drive)
{
	int i;
	wap_storage_enum drive_n;
	
	if(drive==MSF_WAP_STORAGE_NONE)
		return;
	
	drive_n=WAP_STORAGE_SYSTEM;
	if(drive==MSF_WAP_STORAGE_MEM_CARD_1)
	   drive_n=WAP_STORAGE_REMOVABLE;
	
	if(drive==MSF_WAP_STORAGE_PHONE)
	   drive_n=WAP_STORAGE_PUBLIC;
	
	for (i = 0; i < NUM_FS_MAP_TABLE_ENTRY; i++)
    {
        if(! strcmp(fs_map_table[i].int_name, src))
        {
            fs_map_table[i].ext_drive=drive_n;
            MSF_FILE_MKDIR(src);
        }
	}
    ...
    switch (type)
    {
        ...
        case WAP_STORAGE_PUBLIC:
            ...
            if (public_disk < 0)
            {
                ...
                public_disk = SRV_FMGR_SYSTEM_DRV;    //Ramneek
...
/* SLIM_R starts */
            ...
            else if(fs_map_table[i].ext_drive == WAP_STORAGE_PUBLIC)
            {
                
				sprintf(drive_name, "%c:", getDiskLabel(WAP_STORAGE_PUBLIC)); 
                text = msf_cmmn_str3cat(
                    MSF_MODID_MSM,
                    drive_name,
                    fs_map_table[i].ext_name,
...
/* SLIM_R ends */
            else
            {
                if(storage_id == MSF_MMS_STORAGE_PHONE)
				{
					...
					sprintf(drive_name, "%c:", getDiskLabel(MMS_STORAGE_SYSTEM));   //Ramneek
