...
kal_uint64 mma_get_mms_avaliable_file_space(kal_uint32 *used_bytes, kal_uint8 storage)
{
	...
	if (storage == MMA_MSG_STORAGE_CARD1) /* Check if there is need to check for card plug-in ststus*/
    {
...
#else
            kal_wsprintf(wpath, "%c:\\@mms_1", (char)SRV_FMGR_CARD_DRV);
#endif	/*__MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
            folder_size = FS_GetFolderSize(wpath, FS_COUNT_IN_BYTE, NULL, 0);
            if (folder_size >= 0)
            {
...
#else
	return (MSF_FILE_OPEN_EXT(modId, fileName, mode | HDI_FILE_SET_CUSTOM_BLOCKING, size, MSF_MMS_STORAGE_MEM_CARD_1));
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
}
