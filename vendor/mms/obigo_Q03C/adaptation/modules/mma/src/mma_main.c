...
#define MSB_INDX_SD_CARD_MMS 0x80000000
#endif
    ...
    if ((srv_usb_is_in_mass_storage_mode() || mma_main_is_in_usb_mode()))
    {
        ...
        if (pending_sig != NULL)
        {
            ...
            return;
                    ...
                    if (g_mma_main_cntx.memory_card_plug_in.card_state == MMA_MAIN_MEM_CARD_STATE_PLUG_OUT_PROCESS_PLUG_IN_PENDING)
                    {
                        ...
                        g_mma_main_cntx.memory_card_plug_in.card_state = MMA_MAIN_MEM_CARD_STATE_PLUG_OUT_PROCESS;
						mma_main_handle_mem_card_plug_out_rsp_hdlr(MMA_MSG_STORAGE_CARD1);
...
{  
    ...
    if (g_mma_main_cntx.memory_card_plug_in.card_state == MMA_MAIN_MEM_CARD_STATE_PLUG_OUT_PROCESS)
                    {
        ...
        g_mma_main_cntx.memory_card_plug_in.card_state = MMA_MAIN_MEM_CARD_STATE_PLUG_OUT_PROCESS_PLUG_IN_PENDING;
	...
	else if(g_mma_main_cntx.memory_card_plug_in.card_state == MMA_MAIN_MEM_CARD_STATE_PLUG_IN_PROCESS)
                    {
	/* ignore another plug in over existing plug in */
                        
                    }                 
	else
    if (g_mma_main_cntx.memory_card_plug_in.card_present == MMA_MSG_STORAGE_NONE)
                    {
        ...
        g_mma_main_cntx.memory_card_plug_in.card_present = card_info_p->drv_list[index_1];
        g_mma_main_cntx.memory_card_plug_in.card_state =  MMA_MAIN_MEM_CARD_STATE_PLUG_IN_PROCESS;
...
void mma_main_handle_mem_card_plug_out_rsp_hdlr(mma_msg_storage_type_enum card_enum)
{
    ...
    g_mma_main_cntx.memory_card_plug_in.card_present = MMA_MSG_STORAGE_NONE;
    g_mma_main_cntx.memory_card_plug_in.card_state = MMA_MAIN_MEM_CARD_STATE_NONE;
...
void mma_main_handle_mem_card_plug_in_rsp_hdlr(mma_msg_storage_type_enum card_enum)
{
    ...
    MSF_LOG_MSG_NOARG((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MMA, MMA_MAIN_CARD_PLUG_IN_RSP, "(mma_main.c)mma_main_handle_mem_card_plug_in_rsp_hdlr()"));
            ...
            g_mma_main_cntx.memory_card_plug_in.card_state = MMA_MAIN_MEM_CARD_STATE_PLUG_IN;
...
kal_bool mma_main_init(void)
{
        ...
        msm_get_module_status(MSF_MODID_MMS) == MODULE_STATUS_ACTIVE)
    {
		...
		MSF_FILE_MKDIR_EXT(MMA_FOLDER, MSF_MMS_STORAGE_PHONE);
                ...
                {
                    g_mma_main_cntx.memory_card_plug_in.card_present = MMA_MSG_STORAGE_CARD1;
                    g_mma_main_cntx.memory_card_plug_in.card_state = MMA_MAIN_MEM_CARD_STATE_PLUG_IN;
        }
#endif /* __MMI_MMS_MEMORY_CARD_STORAGE_SUPPORT__ */
    }
...
kal_bool mma_main_is_storage_type_present(mma_msg_storage_type_enum storage_type)
{
   ...
   int index = 0;
	...
	else if (g_mma_main_cntx.memory_card_plug_in.card_present == storage_type &&
            g_mma_main_cntx.memory_card_plug_in.card_state == MMA_MAIN_MEM_CARD_STATE_PLUG_IN)
        {
            return KAL_TRUE;            
        }
...
mma_msg_storage_type_enum mma_get_message_storage(kal_uint32 msgid)
{
        ...
        if (g_mma_main_cntx.memory_card_plug_in.card_present != MMA_MSG_STORAGE_NONE)
            {
              return (mma_msg_storage_type_enum)g_mma_main_cntx.memory_card_plug_in.card_present;
        }
#else 
        if ((g_mma_main_cntx.memory_card_plug_in.card_present != MMA_MSG_STORAGE_NONE) && (msgid & MSB_INDX_SD_CARD_MMS))
        {
            return (mma_msg_storage_type_enum)g_mma_main_cntx.memory_card_plug_in.card_present;
    }
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
...
kal_uint32 mma_main_encode_msg_id(kal_uint8 storage, kal_uint32 o_msgId)
{
	...
	{
        if (g_mma_main_cntx.memory_card_plug_in.card_present == storage)
        {
            return (o_msgId | MSB_INDX_SD_CARD_MMS);            
    }
	}
#endif /* __MMI_MMS_MEMORY_CARD_STORAGE_SUPPORT__ */
