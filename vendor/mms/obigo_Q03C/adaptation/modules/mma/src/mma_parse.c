...
#else
					    externalFilePath = (kal_uint8*) MSF_FILE_GET_EXTERNAL_FILENAME_EXT(MSF_MODID_MMA, virtuelFileName, MSF_MMS_STORAGE_MEM_CARD_1);
            ...
            switch (info->suffix)
            {
                ...
                case MMS_SUFFIX_DELIVERYREPORT:
#endif
                    {
                        ...
                        if ((temp_header_size = MSF_FILE_GETSIZE_EXT(msg_file_path, 
#ifndef __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__
							(msf_mms_storage_enum)translate_mma_storage_to_mms_storage_enum((mma_msg_storage_type_enum)info->storage_type)
#else
...
#ifndef MMA_SML_SUPPORT_AND_EXTRA_ERROR_HANDLE 
					&& info->mms_dsr->body->obj_no <= MMS_SLIM_MAX_OBJ_SUPPORT_IN_PARSE
#endif /* MMA_SML_SUPPORT_AND_EXTRA_ERROR_HANDLE */			
					)
                {                    
					if (info->mms_dsr->body->objects == NULL)
...
/* for postcard */					
...
#else
                                drm_method = mma_drm_get_method_type(-1, vFileName, MSF_MMS_STORAGE_MEM_CARD_1);
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
				...
				info->curr = temp_node;
#if (!defined( MMA_MMS_CREATION_MODE_FREE_ONLY_SUPPORT ) && defined( __MMI_VCARD__ ))
...
#else
                    MMAif_isvcardrestricted(filepath, info->mms_dsr->body->curr_object->size, info->mms_dsr->body->curr_object->offset, MSF_MMS_STORAGE_MEM_CARD_1);
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
                    action = MMA_PARSE_ACTION_DELAYED;
                    MMA_PARSE_FREE(filepath);
                    break;
                }
#endif /* defined( __MMI_VCARD__ ) && !defined( MMA_MMS_CREATION_MODE_FREE_ONLY_SUPPORT ) */
				if(temp_node != NULL)
                {
                ...
                while (obj)
                {
                        ...
                        app_lib_mime_type_value == MIME_TYPE_AUDIO)
                    {
                        if (info->mode != MMA_MODE_EDIT || info->mms_dsr->body->slide_no < wap_custom_get_max_mms_slide())
                        {
                            ...
                            {
#ifndef MMA_SML_SUPPORT_AND_EXTRA_ERROR_HANDLE
                                slide = mma_insert_slide(mma_parse_mod_id, info->mms_dsr->body, MMA_MIXED_MMS_SLIDE_N_OBJ_END_TIME);
#else
                                slide = mma_insert_slide(mma_parse_mod_id, info->mms_dsr->body, 0);
#endif /* MMA_SML_SUPPORT_AND_EXTRA_ERROR_HANDLE */
                            }
						...
						if(info->mode == MMA_MODE_EDIT)
        {
            ...
            if (info->mms_dsr->body->slide_no == 0)  /* Add empty slide */
                        {
#ifndef MMA_SML_SUPPORT_AND_EXTRA_ERROR_HANDLE
                (void)mma_insert_slide(mma_parse_mod_id, info->mms_dsr->body, MMA_MIXED_MMS_SLIDE_N_OBJ_END_TIME);
#else
...
#else
                info->xml_filepath = (kal_wchar *)MSF_FILE_GET_EXTERNAL_FILENAME_EXT(mma_parse_mod_id, file_path, MSF_MMS_STORAGE_MEM_CARD_1 /*Always system "c:\*/);
                info->xml_size = MSF_FILE_GETSIZE_EXT(file_path, MSF_MMS_STORAGE_MEM_CARD_1 /*always system "c:\"*/);
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
            }
            switch (info->result)
            {
