...
#if (defined( MMS_REPORTS_SUPPORT ) && !defined( __MMI_MMS_REPORT_STATUS_SUPPORT__ ))
        case MMS_SUFFIX_READREPORT:
        {
...
#endif /* (defined( MMS_REPORTS_SUPPORT ) && !defined( __MMI_MMS_REPORT_STATUS_SUPPORT__ ) */
        default:
            MSF_LOG_MSG_OFF((MSF_LOG_DETAIL_HIGH, MSF_MODID_MMA, MMA_MSGMGR_CREATE_IMG_ID_FILE_SUFFIX,
                             "(%s) (%d) createListItemImg suffix state (%d)\n", __FILE__, __LINE__, suffix));
            return 0;
...
#if (defined( MMS_REPORTS_SUPPORT ) && defined( __MMI_MMS_REPORT_BOX_SUPPORT__ ))
/*****************************************************************************
 * FUNCTION
...
#endif  /* defined( MMS_REPORTS_SUPPORT ) && defined( __MMI_MMS_REPORT_BOX_SUPPORT__ ) */
...
#endif /*MMA_MMS_ERROR_HNDLNG_AND_RARE_USED_CODE*/
	if ((mma_msgmgr_give_msg_info_to_app() == FALSE))
    {
        mma_send_int_sig(
            &(g_mma_msgmgr_cntx.pending_req_list),
            MMA_SUBMODULE_MSG_MGR,
...
#endif /*MMA_MMS_ERROR_HNDLNG_AND_RARE_USED_CODE*/
	if (mma_msgmgr_check_busy_state_pending_list
...
mma_result_enum mma_msgmgr_add_new_report_notif_req_hdlr( mma_msgmgr_new_message_source_enum new_msg_source, void *data)
{
    ...
    if(new_msg_source == MMA_MSGMGR_GET_FROM_READ_REPORT)
    {
        ...
        msg_list->msg_id = mma_main_encode_msg_id(
                            ...
                            sig_data->msgId);
#else
		msg_list->msg_id = sig_data->msgId;
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
...
#else
		msg_list->msg_id = sig_data->notifId;
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
        msg_list->msg_sim_id = translate_mms_sim_to_mma_sim((kal_uint8)sig_data->simId);
...
#endif /* defined(__MMI_MMS_MEMORY_CARD_STORAGE_SUPPORT__) && !defined(__MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__) */
    {
...
static void mma_msgmgr_get_msg_info_req_hdlr(module_type source, wap_mma_get_msg_info_req_struct *get_msg_info_req)
{
	...
	if (mma_msgmgr_check_busy_state_pending_list(source, MMA_INT_SIG_TYPE_MSG_GET_MSG_INFO_REQ, get_msg_info_req))
        return;
    else
    {
        mma_result_enum result = MMA_RESULT_OK;
...
static void mma_msgmgr_save_req_hdlr(module_type source, wap_mma_save_req_struct *save_req)
{
	...
	if(mma_cvt_mma_folder_to_mms_folder((mma_folder_enum)save_req->box) == -1)
    {
        for(index_of_msg = 0; index_of_msg < save_req->no_of_msg; index_of_msg++)
		{
...
static void mma_msgmgr_delete_req_hdlr(module_type source, wap_mma_delete_req_struct *delete_req)
{
    ...
    if (mma_msgmgr_check_busy_state_pending_list(source, MMA_INT_SIG_TYPE_MSG_DELETE_REQ, delete_req))
    {
        MMA_FREE(delete_result);
        return;
    }
...
#if (!defined( __COSMOS_MMI_PACKAGE__ ) && !defined( __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ ))
...
#endif /* !defined( __COSMOS_MMI_PACKAGE__ ) && !defined( __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ ) */
	...
	else
	{
        ...
        if(result == MMA_RESULT_OK)
		{
			...
			else
			{
				...
				if(
#ifdef MMS_REPORTS_SUPPORT
                                        (msg_info->suffix != MMS_SUFFIX_DELIVERYREPORT) && (msg_info->suffix != MMS_SUFFIX_READREPORT) && 
#endif
...
#else
                char *filename = (char*)MMA_CALLOC(strlen("c:\\@mms_1\\mms_sys\\xxxxxxxx.s") + ENCODING_LENGTH);
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
...
void mma_msgmgr_get_fldr_property_data(MmsListOfProperties *list)
{
...
#else
	msf_mms_storage_enum mms_storage_type = MSF_MMS_STORAGE_MEM_CARD_1;
	mma_msg_storage_type_enum mma_storage_type = MMA_MSG_STORAGE_CARD1;
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
    ...
    while ((i < list->noOfElements) && (list->msgPropertyList[i].msgFolderInfo.msgId))
    {
        ...
        if (((result != MMA_RESULT_OK) && (result != MMA_RESULT_FAIL_UNSUPPORT_MMS_TYPE)) || (property_st->folder == MMA_FOLDER_HIDDEN) || (property_st->folder == MMA_FOLDER_NONE))
        {
    ...
    {    
        mma_check_and_sleep();
        MMSif_getMMSList(MSF_MODID_MMA, (list->startEntry + list->noOfElements - del_count),
            MAX_MMS_GET_PHONE_LIST_COUNT - 1, (MSF_UINT8) mms_storage_type);
            
    } else
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
#ifdef  __MMI_MMS_MEMORY_CARD_STORAGE_SUPPORT__
     if((list->more == TRUE) && MSF_IS_MEM_CARD_PRESNT(MSF_MMS_STORAGE_MEM_CARD_1))
    {
