...
/*****************************************************************************
 * FUNCTION
 *  mma_create_reassemble_object_list
 * DESCRIPTION
 *  Reset the layout information.
 * PARAMETERS
 *  void
 ...
 *  U8 Decimal value.(?)
 *****************************************************************************/
void mma_create_reassemble_object_list(mma_create_info_struct *info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	mma_mms_slide_struct *slide;
	mma_mms_object_struct *obj, *slide_obj = NULL;

	/* update layout: check 1 or 2 regions */
	if(info->mms_dsr->body)
    {
		slide = info->mms_dsr->body->slides;
		info->mms_dsr->body->objects = NULL;
    while (slide)
        {
		if(slide->txt.object)
			slide_obj = slide->txt.object;
		else if(slide->img.object)
			slide_obj = slide->img.object;
		else if(slide->vid.object)
			slide_obj = slide->vid.object;
		else if(slide->aud.object)
			slide_obj = slide->aud.object;
		else if(slide->ref_items && slide->ref_items->object)
			slide_obj = slide->ref_items->object;
		/*Associate in the body object list */
		if(slide_obj)
				{
					if (info->mms_dsr->body->objects == NULL)
					{
						info->mms_dsr->body->objects = obj = slide_obj;
						obj->prev = NULL;
					}
					else
					{
...
#else
			(void)mma_create_reassemble_object_list(info);

#endif /* MMA_SML_SUPPORT_AND_EXTRA_ERROR_HANDLE */		   if(info->mms_dsr->body)
		   {
            obj = info->mms_dsr->body->objects;
            while (obj)
            {
...
#else
                MMSif_createMessage(MSF_MODID_MMA, info->msg_filetype, MSF_MMS_STORAGE_MEM_CARD_1, TRUE);
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
            }
            else
#endif
...
#else
                MMSif_createMessage(MSF_MODID_MMA, info->msg_filetype, MSF_MMS_STORAGE_MEM_CARD_1, FALSE);
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
            }
            switch (info->mode)
            {
...
#else
            MSF_FILE_REMOVE_EXT(path, MSF_MMS_STORAGE_MEM_CARD_1);
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
...
static void mma_create_req_hdlr(kal_bool encode_msg, mma_msgmgr_create_info_struct *create_req_p)
{
...
#else
        preferred_storage = MMA_MSG_STORAGE_CARD1;
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
	...
	else if (card_msg_count >= (kal_uint16)(wap_custom_get_max_mms_num_on_card()))
	{
            mma_create_send_rsp(create_req_p->req_id, MMA_RESULT_FAIL_MAX_MSG_NUM_REACHED, 0);
	}
