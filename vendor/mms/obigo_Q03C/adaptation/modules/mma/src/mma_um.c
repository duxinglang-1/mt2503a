...
#else
        kal_wsprintf(temp_path, "%c:%s", SRV_FMGR_CARD_DRV,"\\@mms_1\\mms_pdu");
...
#else
    sprintf(msg_file_path, "%c:\\@mms_1\\mms_pdu\\%lx.%c", mms_disk, mms_msg_id, (char)MMS_SUFFIX_NOTIFICATION);
#endif /* __MMI_MMS_ONLY_MEMORY_CARD_STORAGE_SUPPORT__ */
    ...
    if(result == MMA_RESULT_OK)
    {
        ...
        if(result == MMA_RESULT_OK)
        {
			...
			if(msg_info->user_can_not_see)
				msg_info->user_can_not_see = KAL_FALSE;
...
#if (defined( MMS_REPORTS_SUPPORT ) && defined( __MMI_MMS_REPORT_STATUS_MENU_SUPPORT__ ))
/*****************************************************************************
 * FUNCTION
...
#endif	/* defined( MMS_REPORTS_SUPPORT ) && defined( __MMI_MMS_REPORT_STATUS_MENU_SUPPORT__ ) */
...
/*#elif defined(__MMI_VIDEO_PLAY_SUPPORT__)
    ...
    if (info->drm_type == MMA_DRM_NONE)
    {
        ...
        #endif /* defined(__MMI_VIDEO_PLAY_SUPPORT__) && !defined(__MMI_MMS_SIMPLIFIED_UI_SUPPORT__)*/ 
#endif
                break;
            default:
                break;
